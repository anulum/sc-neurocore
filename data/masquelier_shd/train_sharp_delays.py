...
if __name__ == "__main__":
    ...
    # Output directory
    out_dir = os.path.join("exp", "SHD", "SNN_axonal_feedforward_delays", "sharp_delays_retrain")
    ...
    for epoch in range(total_epochs):
        ...
        print(
            f"{epoch:>5} {sig:>6.1f} {train_acc:>7.1f}% {val_acc:>7.1f}% {test_acc:>7.1f}% {elapsed:>5.0f}s"
        )
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(
                {"net": model.state_dict(), "acc": val_acc, "epoch": epoch, "sig": sig},
    ...
    best_ckpt = torch.load(
        os.path.join(out_dir, "best.pth"), map_location=device, weights_only=False
    )
    ...
    print(
        f"Best checkpoint (epoch {best_ckpt['epoch']}, SIG={best_ckpt['sig']:.1f}) with sharp delays: {best_sharp_test:.1f}%"
    )
    ...
    print("Original test accuracy (SIG=15): 80.4%")
    print("Original sharp test (SIG=0.1): 58.6%")
