...
// SC-NeuroCore — bounded safety harness for source MAT(1) Q32.32

`default_nettype none
module sc_non_resetting_lif_formal(
    input wire clk,
    input wire rst_n,
...
`ifdef FORMAL
    ...
    always @(posedge clk) begin
        ...
        if (past_valid && $past(!rst_n)) begin
            assert(v == 0 && theta == 0 && refractory == 0 && !event_out);
        end
        if (past_valid && rst_n) begin
            assert(v >= V_MIN && v <= V_MAX);
            assert(theta >= 0 && theta <= THETA_MAX);
