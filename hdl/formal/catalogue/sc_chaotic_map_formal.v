...
// SC-NeuroCore — formal harness for the SC chaotic-map Q8.24 RTL

`default_nettype none

module sc_chaotic_map_formal (
    input wire clk,
    input wire rst_n,
    input wire signed [31:0] I_t
);
    localparam signed [31:0] Q_TEN = 32'sd167772160;
    localparam signed [31:0] X_THRESHOLD = 32'sd8388608;
    wire spike_out;
...
`ifdef FORMAL
    ...
    always @(posedge clk) begin
        ...
        if (past_valid && rst_n && $past(rst_n)) begin
            assert (x_out >= -Q_TEN && x_out <= Q_TEN);
            assert (y_out >= -Q_TEN && y_out <= Q_TEN);
