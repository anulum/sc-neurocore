...
from gen_vmin_lif_lut import (  # noqa: E402
    LUT_RANGE,
    LUT_SIZE,
    Q88_MAX,
    Q88_MIN,
...
class TestQ88:
    ...
    def test_encode_quarter(self):
        assert encode_q88(0.25) == 64

    def test_encode_max_clamp(self):
        assert encode_q88(1e9) == Q88_MAX

    def test_encode_min_clamp(self):
    ...
    def test_q88_scale_constant(self):
...
class TestSoftplus:
    ...
    def test_softplus_large_positive_linear(self):
    ...
    def test_softplus_beta_scaling(self):
...
class TestLUTLookup:
    ...
    def test_lookup_negative_returns_first(self):
        lut = gen_softplus_lut(beta=1.0, size=LUT_SIZE, z_max=LUT_RANGE)
        assert lut_lookup(lut, -100) == lut[0]

    def test_lookup_above_range_returns_input(self):
    ...
    def test_lookup_at_lut_endpoints(self):
        lut = gen_softplus_lut(beta=1.0, size=LUT_SIZE, z_max=LUT_RANGE)
        # z = 0 → first entry
        assert lut_lookup(lut, 0) == lut[0]
    ...
    def test_lookup_accuracy_vs_float(self):
...
class TestVminLifSingleStep:
    ...
    def test_threshold_crossing_triggers_spike(self):
        cfg = VminLifConfig()
        lut = gen_softplus_lut(cfg.beta_v_inf, LUT_SIZE, LUT_RANGE)
    ...
    def test_softplus_floor_prevents_unbounded_negative(self):
        cfg = VminLifConfig()
        lut = gen_softplus_lut(cfg.beta_v_inf, LUT_SIZE, LUT_RANGE)
...
class TestFloatReference:
    def test_float_step_matches_pytorch_dynamics(self):
    ...
    def test_float_step_threshold_crossing(self):
        cfg = VminLifConfig()
...
class TestTrajectoryConsistency:
    def test_constant_input_trajectory(self):
        cfg = VminLifConfig()
        lut = gen_softplus_lut(cfg.beta_v_inf, LUT_SIZE, LUT_RANGE)
    ...
    def test_zero_input_no_runaway(self):
        cfg = VminLifConfig()
        lut = gen_softplus_lut(cfg.beta_v_inf, LUT_SIZE, LUT_RANGE)
        v_q = 0
        for _ in range(100):
...
class TestVerilogHeader:
    def test_header_contains_size_define(self):
    ...
    def test_header_contains_all_entries(self):
        lut = gen_softplus_lut(1.0, LUT_SIZE, LUT_RANGE)
        header = emit_lut_verilog_header(lut)
        for i in range(LUT_SIZE):
            assert f"`define VMIN_LUT_{i:02d}" in header

    def test_header_uses_signed_q88_literals(self):
        lut = gen_softplus_lut(1.0, LUT_SIZE, LUT_RANGE)
        header = emit_lut_verilog_header(lut)
        assert "16'sd" in header

    def test_header_has_provenance_comment(self):
        lut = gen_softplus_lut(1.0, LUT_SIZE, LUT_RANGE)
        header = emit_lut_verilog_header(lut)
