...
// SC-NeuroCore — built-bundle browser-to-server-to-replay export contract
...
const apiPort = configuredPort("SC_NEUROCORE_STUDIO_EXPORT_API_PORT", 18_002);
const uiPort = configuredPort("SC_NEUROCORE_STUDIO_EXPORT_UI_PORT", 15_176);
const apiOrigin = `http://127.0.0.1:${apiPort}`;
const uiOrigin = `http://127.0.0.1:${uiPort}`;

// The export receipt runs against the built bundle served by `vite preview`,
// which proxies /api to a real Studio backend. Run `npm run build` first; the
// `test:e2e:export` script does.
export default defineConfig({
  testDir: "./e2e",
  testMatch: "experiment-export-live.spec.ts",
