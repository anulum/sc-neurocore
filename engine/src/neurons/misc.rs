...
impl CardiacPurkinjeFibre {
    ...
    pub fn step(&mut self, current: f64) -> i32 {
        ...
        // Safety
        self.v = self.v.clamp(-120.0, 60.0);
        if !self.v.is_finite() { self.v = -85.0; }
        if !self.m.is_finite() { self.m = 0.001; }
        if !self.h.is_finite() { self.h = 0.99; }
        if !self.d.is_finite() { self.d = 0.001; }
        if !self.f.is_finite() { self.f = 0.99; }
        if !self.x_r.is_finite() { self.x_r = 0.01; }
        if !self.y.is_finite() { self.y = 0.05; }

        // Spike: V crosses -20 mV upward
        if self.v >= -20.0 && v_prev < -20.0 { 1 } else { 0 }
    }
...
impl SmoothMuscleCell {
    ...
    pub fn step(&mut self, current: f64) -> i32 {
        ...
        if !self.ca.is_finite() { self.ca = 0.1; }
        if !self.ca_store.is_finite() { self.ca_store = 100.0; }

        // "Spike" = slow wave crossing -30 mV
        if self.v >= -30.0 && v_prev < -30.0 { 1 } else { 0 }
    }
...
mod tests {
    ...
    fn cardiac_fires_with_input() {
        ...
        assert!(spikes > 0, "Cardiac Purkinje must fire with input, got {spikes}");
    }
    ...
    fn cardiac_silent_without_input() {
        ...
        assert!(spikes <= 1, "Must be essentially silent without pacemaker, got {spikes}");
    }
    ...
    fn cardiac_six_currents() {
        let n = CardiacPurkinjeFibre::new();
        assert!(n.g_na > 0.0 && n.g_cal > 0.0 && n.g_kr > 0.0
            && n.g_k1 > 0.0 && n.g_f > 0.0 && n.g_l > 0.0,
    ...
    fn smooth_fires_with_input() {
        ...
        let mut spikes = 0;
        for _ in 0..10_000 { spikes += n.step(5.0); }
        assert!(spikes > 0, "Smooth muscle must produce slow waves, got {spikes}");
    }
    ...
    fn smooth_silent_without_input() {
        ...
        let mut spikes = 0;
        for _ in 0..5_000 { spikes += n.step(0.0); }
        assert!(spikes <= 1, "Should be essentially silent without drive, got {spikes}");
    }
    ...
    fn smooth_performance_1k_steps() {
        let start = std::time::Instant::now();
        let mut n = SmoothMuscleCell::new();
        for _ in 0..1_000 { std::hint::black_box(n.step(2.0)); }
        let elapsed = start.elapsed();
        assert!(elapsed.as_millis() < 50, "1k steps must complete in <50ms");
    }
    ...
    fn beta_fires_with_glucose() {
        ...
        let mut spikes = 0;
        for _ in 0..10_000 { spikes += n.step(5.0); }
        assert!(spikes > 0, "Beta cell must burst with high glucose, got {spikes}");
    }
    ...
    fn beta_silent_low_glucose() {
        ...
        let mut spikes = 0;
        for _ in 0..5_000 { spikes += n.step(0.0); }
        assert_eq!(spikes, 0, "Beta cell must be silent at low glucose, got {spikes}");
    }
    ...
    fn beta_ca_rises_with_spiking() {
        ...
        n.atp_level = 0.8;
        let ca0 = n.ca;
        for _ in 0..5_000 { n.step(2.0); }
        assert!(n.ca > ca0, "Ca²⁺ should rise during bursting: ca0={ca0}, ca={}", n.ca);
    }
