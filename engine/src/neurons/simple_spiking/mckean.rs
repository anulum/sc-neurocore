...
// SC-NeuroCore — McKean Neuron Model

//! McKean piecewise-linear neuron dynamics.
...
mod tests {
    ...
    fn default_matches_constructor_state() {
        let default = McKeanNeuron::default();
        let constructed = McKeanNeuron::new();
        assert_eq!(default.v, constructed.v);
    }

    #[test]
    fn simulate_matches_repeated_step() {
        let mut simulated = McKeanNeuron::new();
        let mut repeated = McKeanNeuron::new();
