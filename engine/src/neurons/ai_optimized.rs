...
impl MultiTimescaleNeuron {
    ...
    pub fn step(&mut self, current: f64) -> i32 {
        ...
        self.v_medium +=
            (-self.v_medium + self.alpha * fired as f64) / self.tau_medium * self.dt;
...
impl ContinuousAttractorNeuron {
    pub fn new(n_units: usize) -> Self {
        ...
        for i in 0..n_units {
            for j in 0..n_units {
                let d = (i as f64 - j as f64).abs().min((n_units as f64) - (i as f64 - j as f64).abs());
                weights[i][j] = excitation * (-d * d / (2.0 * sigma_e * sigma_e)).exp() - inhibition;
            }
        }
        Self {
            u: vec![0.0; n_units],
...
mod tests {
    ...
    fn attention_gated_gate_suppresses_low_input() {
        let mut n = AttentionGatedNeuron { w_key: -2.0, ..AttentionGatedNeuron::new() };
