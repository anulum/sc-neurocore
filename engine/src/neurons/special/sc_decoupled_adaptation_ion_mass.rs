...
impl SCDecoupledAdaptationIonMassNeuron {
    ...
    fn valid(&self) -> bool {
        [
            self.v, self.w, self.z, self.g_ca, self.g_na, self.g_k, self.v_ca, self.v_na, self.v_k,
            self.v_l, self.g_l, self.phi, self.tau_k, self.b, self.a_ee, self.v0, self.i_ext,
            self.dt,
        ]
        .iter()
        .all(|x| x.is_finite())
...
impl Default for SCDecoupledAdaptationIonMassNeuron {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn retained_anchor_and_atomic_failure() {
        let mut state = SCDecoupledAdaptationIonMassNeuron::new();
        assert!((state.try_step(0.0).unwrap() - (-0.498_759_341_907_830_5)).abs() < 1e-15);
