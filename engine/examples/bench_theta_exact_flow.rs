...
// SC-NeuroCore — Theta exact-flow Rust benchmark artefact writer

mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::ThetaNeuron;
...
fn run_once() -> (f64, i32, f64) {
    let mut neuron = ThetaNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.theta)
}
...
fn main() {
    ...
    let mut final_thetas = Vec::with_capacity(REPEATS);

    for _ in 0..REPEATS {
        let (ns, spikes, theta) = run_once();
        ns_per_step.push(ns);
        spike_counts.push(spikes);
        final_thetas.push(theta);
    }
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_theta_exact_flow\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
