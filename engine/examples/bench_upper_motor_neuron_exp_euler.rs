...
// SC-NeuroCore — upper motor neuron exponential-Euler Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::UpperMotorNeuron;
use std::hint::black_box;
use std::time::{Instant, SystemTime, UNIX_EPOCH};

const STEPS: usize = 200_000;
const REPEATS: usize = 5;
const CURRENT: f64 = 5.0;

fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = UpperMotorNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.v, neuron.s)
}
...
fn main() {
    ...
    let mut final_ss = Vec::with_capacity(REPEATS);

    for _ in 0..REPEATS {
        let (ns, spikes, v, s) = run_once();
        ns_per_step.push(ns);
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_upper_motor_neuron_exp_euler\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
