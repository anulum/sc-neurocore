...
// SC-NeuroCore — Lapicque exact-flow Rust benchmark artefact writer

mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neuron::LapicqueNeuron;
...
fn run_once() -> (f64, i32, f64) {
    let mut neuron = LapicqueNeuron::new(20.0, 1.0, 1.0, 1.0);
...
fn main() {
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_lapicque_exact_flow\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
