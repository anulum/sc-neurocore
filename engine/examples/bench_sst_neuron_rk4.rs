...
fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = SSTNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.v, neuron.r)
}
...
fn main() {
    ...
    let mut final_rs = Vec::with_capacity(REPEATS);
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_sst_neuron_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
