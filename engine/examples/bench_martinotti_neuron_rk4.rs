...
// SC-NeuroCore — Martinotti RK4 Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::MartinottiNeuron;
...
fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = MartinottiNeuron::default();
...
fn main() {
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_martinotti_neuron_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
