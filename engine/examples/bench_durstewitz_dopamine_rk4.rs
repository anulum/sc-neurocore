...
// SC-NeuroCore — Durstewitz D1 PFC neuron RK4 Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::DurstewitzDopamineNeuron;
...
fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = DurstewitzDopamineNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.v, neuron.h_na)
}
...
fn main() {
    ...
    let mut final_hs = Vec::with_capacity(REPEATS);

    for _ in 0..REPEATS {
        let (ns, spikes, v, h) = run_once();
        ns_per_step.push(ns);
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_durstewitz_dopamine_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
