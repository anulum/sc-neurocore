...
// SC-NeuroCore — De Schutter Purkinje RK4 Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::DeSchutterPurkinjeNeuron;
...
fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = DeSchutterPurkinjeNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.v, neuron.ca)
}
...
fn main() {
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --release --manifest-path engine/Cargo.toml --example bench_de_schutter_purkinje_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
