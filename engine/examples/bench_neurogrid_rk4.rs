...
// SC-NeuroCore — Neurogrid RK4 Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::NeuroGridNeuron;
...
fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = NeuroGridNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.v_s, neuron.v_d)
}
...
fn main() {
    ...
    let mut final_vds = Vec::with_capacity(REPEATS);

    for _ in 0..REPEATS {
        let (ns, spikes, v_s, v_d) = run_once();
        ns_per_step.push(ns);
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --release --manifest-path engine/Cargo.toml --example bench_neurogrid_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
