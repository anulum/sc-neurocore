...
// SC-NeuroCore — Escape-rate exact-flow Rust benchmark artefact writer

mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::EscapeRateNeuron;
use std::hint::black_box;
use std::time::{Instant, SystemTime, UNIX_EPOCH};

const STEPS: usize = 200_000;
const REPEATS: usize = 5;
const CURRENT: f64 = 30.0;

fn run_once(seed: u64) -> (f64, i32, f64) {
    let mut neuron = EscapeRateNeuron::new(seed);
...
fn main() {
    ...
    for repeat in 0..REPEATS {
        let (ns, spikes, v) = run_once(42 + repeat as u64);
        ns_per_step.push(ns);
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_escape_rate_exact_flow\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
