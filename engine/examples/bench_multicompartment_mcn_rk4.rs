...
// SC-NeuroCore — Multicompartment MCN RK4 Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::MulticompartmentMCNNeuron;
use std::hint::black_box;
use std::time::{Instant, SystemTime, UNIX_EPOCH};

const STEPS: usize = 200_000;
const REPEATS: usize = 5;
const CURRENT: f64 = 3.2;

fn run_once() -> (f64, i32, f64, f64, f64) {
    let mut neuron = MulticompartmentMCNNeuron::default();
...
fn main() {
    ...
    let mut final_us = Vec::with_capacity(REPEATS);
    let mut final_basals = Vec::with_capacity(REPEATS);
    let mut final_apicals = Vec::with_capacity(REPEATS);

    for _ in 0..REPEATS {
        let (ns, spikes, u, v_basal, v_apical) = run_once();
        ns_per_step.push(ns);
        spike_counts.push(spikes);
        final_us.push(u);
        final_basals.push(v_basal);
        final_apicals.push(v_apical);
    }
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --release --manifest-path engine/Cargo.toml --example bench_multicompartment_mcn_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
            ...
            "  \"final_us\": {final_us:?},\n",
            "  \"final_basals\": {final_basals:?},\n",
            "  \"final_apicals\": {final_apicals:?}\n",
            "}}\n"
        ),
        timestamp_unix = timestamp_unix,
