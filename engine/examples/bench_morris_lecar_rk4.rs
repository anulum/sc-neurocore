...
// SC-NeuroCore — Morris-Lecar RK4 Rust benchmark artefact writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::MorrisLecarNeuron;
...
fn run_once() -> (f64, i32, f64, f64) {
    let mut neuron = MorrisLecarNeuron::default();
    ...
    (elapsed_ns / STEPS as f64, spikes, neuron.v, neuron.w)
}
...
fn main() {
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --manifest-path engine/Cargo.toml --example bench_morris_lecar_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
