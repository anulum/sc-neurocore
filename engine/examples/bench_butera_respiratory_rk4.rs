...
// SC-NeuroCore — Butera Model 1 source RK4 Rust benchmark writer

#[allow(dead_code)]
mod benchmark_context;

use benchmark_context::{load_average, measurement_context_json, rust_version};
use sc_neurocore_engine::neurons::ButeraRespiratoryNeuron;
use std::hint::black_box;
use std::time::{Instant, SystemTime, UNIX_EPOCH};

const STEPS: usize = 200_000;
const REPEATS: usize = 5;
const CURRENT: f64 = 50.0;

fn run_once() -> (f64, i32, f64, f64, f64) {
    let mut neuron = ButeraRespiratoryNeuron::default();
...
fn main() {
    ...
    for _ in 0..REPEATS {
        let (ns, spikes, v, n, h_nap) = run_once();
        ns_per_step.push(ns);
        spike_counts.push(spikes);
        final_states.push([v, n, h_nap]);
    }
    ...
    println!(
        concat!(
            ...
            "  \"command\": \"cargo run --release --no-default-features --manifest-path engine/Cargo.toml --example bench_butera_respiratory_rk4\",\n",
            "  \"rustc\": \"{rust_version}\",\n",
            "  \"target_os\": \"{os}\",\n",
            "  \"target_arch\": \"{arch}\",\n",
