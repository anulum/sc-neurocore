...
| `onnx` | `onnx` | `dev`, `full`, `research` | Dedicated CI matrix lane `test-optional-extras (onnx-protobuf)` installs `.[dev]` and runs `tests/test_export/test_onnx_exporter.py -q -rs`; JSON export tests run without ONNX, while protobuf export tests skip when ONNX is absent outside that lane. | `tests/test_export/test_onnx_exporter.py`, `tests/test_export/test_onnx_export.py`, `.github/workflows/ci.yml` |
...
| `snntorch` | `snntorch` | Not declared in `pyproject.toml`; install manually for NIR interop experiments. | NIR bridge interop tests skip when snnTorch is absent. | `tests/test_nir_bridge/test_snntorch_interop.py`, `docs/guides/nir_integration.md` |
| `spikingjelly` | `spikingjelly.activation_based` | Not declared in `pyproject.toml`; install manually or from the upstream repository for NIR export support. | NIR bridge round-trip tests skip when SpikingJelly is absent. | `tests/test_nir_bridge/test_spikingjelly_interop.py`, `docs/guides/nir_integration.md` |
