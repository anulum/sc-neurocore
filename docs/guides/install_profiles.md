...
| `pip install "sc-neurocore[studio]"` | Web studio / local design UI | `fastapi`, `uvicorn`, `httpx`, `httpx2`; `nir` for the canvas's NIR export and import; `pint` and `sympy` for candidate-model units and equation diffs |
