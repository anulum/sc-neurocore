...
| Benchmark | Median |
|-----------|-------:|
| pacinian_10k_steps | 837 µs |
| Per step | **83.7 ns** |

The step function evaluates a subtraction (derivative), `abs()`, two linear ODEs, and one comparison. No transcendental functions. Expected cost in the low nanosecond range per step.
...
3. **Fast adaptation (tau_adapt = 5 ms) complements the derivative.** Even if dP/dt is sustained (e.g. linear ramp), the adaptation variable suppresses prolonged firing.
