...
| `exp_if_driven_rk4_doi` | `exp_if` | `universal_dsl` | Independent SC compatibility RK4 re-derivation of the EIF equation with the maintained +30 mV event surface and reset |
...
| `perfect_integrator_constant_current_sawtooth` | `perfect_integrator` | `universal_dsl` | Naud-Gerstner 2012 exact held-current integral with a strict `V > V_T` equality-boundary protocol |
| `sc_perfect_integrator_constant_current_sawtooth` | `sc_perfect_integrator` | `universal_dsl` | Count-neutral retained SC exact-integral trace with the historical inclusive comparator |
...
| `sc_symmetric_quadratic_if_zero_current_analytic` | `sc_symmetric_quadratic_if` | `universal_dsl` | Count-neutral retained SC zero-current Riccati trace at the historical `dt=.01` operating point |
