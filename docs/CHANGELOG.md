...
- Added a schema-driven neuron reference-trace validation harness with immutable
  ...
  corpus covers analytic closed-form `lif`, `lapicque`, `quadratic_if`, and
  spike-bearing `perfect_integrator` traces and is covered by strict mypy checks
  plus 100% exact-file focused coverage; no polyglot kernel or
