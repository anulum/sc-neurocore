...
# SC-NeuroCore — Source-bound five-runtime Jansen–Rit benchmark

"""Measure and fail-closed verify the complete Jansen–Rit runtime set."""
...
DEFAULT_OUTPUT = REPOSITORY / "benchmarks/results/bench_jansen_rit.json"
...
TRACE_KEYS = ("y0", "y3", "y1", "y4", "y2", "y5", "eeg")
FINAL_KEYS = tuple(f"{key}_final" for key in TRACE_KEYS[:6])
INITIAL_STATE = {
    "y0": 0.1,
    "y3": 0.2,
    "y1": 0.3,
...
SOURCE_PATHS = (
    ...
    "engine/src/bindings/jansen_rit.rs",
    "engine/src/neurons/jansen_rit.rs",
    ...
    "src/sc_neurocore/accel/julia/neurons/jansen_rit.jl",
    "src/sc_neurocore/accel/mojo/jansen_rit/__init__.py",
    "src/sc_neurocore/accel/mojo/jansen_rit/jansen_rit.mojo",
    "src/sc_neurocore/accel/rust/safety/jansen_rit.rs",
    "src/sc_neurocore/neurons/model_descriptors/JansenRitUnit.toml",
    "src/sc_neurocore/neurons/model_schemas/jansen_rit.json",
    "src/sc_neurocore/neurons/model_schemas/jansen_rit.toml",
...
def _binary_hashes() -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "go_shared_library": _binary_record(
            REPOSITORY / "src/sc_neurocore/accel/go/jansen_rit/libjansen_rit.so"
        ),
        "mojo_shared_library": _binary_record(
            REPOSITORY / "src/sc_neurocore/accel/mojo/jansen_rit/libjansen_rit.so"
        ),
    }
...
def _run_backend(backend: str, n_steps: int) -> BenchmarkResult:
    return JansenRitUnit(**INITIAL_STATE).simulate(_drive(n_steps), backend=backend)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "parity_contract": {
            "observable": "seven complete float64 traces and six final states",
        ...
        "workload": {
            ...
            "input_formula": "220 + 80*sin(step*0.037) + 20*cos(step*0.011)",
