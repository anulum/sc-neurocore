...
# SC-NeuroCore — Source-bound five-runtime resonate-and-fire benchmark

"""Measure and fail-closed verify the complete resonate-and-fire runtime set."""
...
DEFAULT_OUTPUT = REPOSITORY / "benchmarks/results/bench_resonate_and_fire.json"
...
TRACE_KEYS = ("x", "y", "spikes")
FINAL_KEYS = ("x_final", "y_final")
INITIAL_STATE = {
    "x": 0.13,
    "y": -0.27,
    "b": -0.8,
...
SOURCE_PATHS = (
    ...
    "engine/Cargo.toml",
    "engine/src/bindings/resonate_and_fire.rs",
    ...
    "src/sc_neurocore/accel/go/resonate_and_fire/resonate_and_fire.go",
    "src/sc_neurocore/accel/go/services/resonate_and_fire.go",
    "src/sc_neurocore/accel/go/services/resonate_and_fire_test.go",
    "src/sc_neurocore/accel/julia/neurons/__init__.py",
    "src/sc_neurocore/accel/julia/neurons/resonate_and_fire.jl",
    "src/sc_neurocore/accel/mojo/kernels/resonate_and_fire.mojo",
    "src/sc_neurocore/accel/mojo/pixi.lock",
    "src/sc_neurocore/accel/mojo/pixi.toml",
    "src/sc_neurocore/accel/mojo/resonate_and_fire/__init__.py",
    "src/sc_neurocore/accel/mojo/resonate_and_fire/resonate_and_fire.mojo",
    "src/sc_neurocore/accel/resonate_and_fire.py",
    "src/sc_neurocore/accel/rust/safety/resonate_and_fire.rs",
    "src/sc_neurocore/neurons/model_descriptors/ResonateAndFireNeuron.toml",
    "src/sc_neurocore/neurons/model_schemas/resonate_fire.json",
    "src/sc_neurocore/neurons/model_schemas/resonate_fire.toml",
...
def _binary_hashes() -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "go_shared_library": _binary_record(
            REPOSITORY / "src/sc_neurocore/accel/go/resonate_and_fire/libresonate_and_fire.so"
        ),
        "mojo_shared_library": _binary_record(
            REPOSITORY / "src/sc_neurocore/accel/mojo/resonate_and_fire/libresonate_and_fire.so"
        ),
    }
...
def _run_backend(backend: str, n_steps: int) -> BenchmarkResult:
    return ResonateAndFireNeuron(**INITIAL_STATE).simulate(_drive(n_steps), backend=backend)
...
def _go_binary_metadata() -> dict[str, object]:
    path = REPOSITORY / "src/sc_neurocore/accel/go/resonate_and_fire/libresonate_and_fire.so"
    completed = subprocess.run(
...
def _verify_rust_safety() -> dict[str, object]:
    source = REPOSITORY / "src/sc_neurocore/accel/rust/safety/resonate_and_fire.rs"
    with tempfile.TemporaryDirectory(prefix="resonate-fire-rust-") as directory:
        binary = Path(directory) / "resonate_and_fire_tests"
    ...
    return {
        "command": [
            ...
            "-o",
            "<temporary>/resonate_and_fire_tests",
        ],
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "benchmark": "ResonateAndFireNeuron five-runtime exact-flow batch",
        "kernel": KERNEL,
        "timestamp_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "command": (
            "taskset -c <cpu> env PYTHONPATH=$WHEEL_SITE:src:. python "
            "benchmarks/bench_model_resonate_and_fire.py --json "
            "benchmarks/results/bench_resonate_and_fire.json"
        ),
        ...
        "workload": {
            ...
            "input_formula": "4.5 + 1.4*sin(step*0.037) + 0.3*cos(step*0.011)",
