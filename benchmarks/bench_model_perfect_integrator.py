...
Missing backends or unpinned execution fail unless explicitly acknowledged.
"""
...
SOURCE_PATHS = (
    ...
    "src/sc_neurocore/accel/mojo/kernels/perfect_integrator.mojo",
...
def _probe_backend(backend: str) -> tuple[bool, str]:
    ...
    if backend == "rust":
        return (
            backends._HAS_RUST,
            "" if backends._HAS_RUST else "Rust engine PerfectIntegrator symbol unavailable",
        )
    if backend == "julia":
        available = backends.ensure_julia_loaded()
        return (
            available,
            "" if available else "juliacall or Perfect Integrator Julia module unavailable",
        )
    if backend == "go":
        available = backends.ensure_go_loaded()
        return available, "" if available else "compiled Go libperfect_integrator.so unavailable"
    available = backends.ensure_mojo_loaded()
    return available, "" if available else "compiled Mojo libperfect_integrator.so unavailable"
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Controlled Perfect Integrator five-backend benchmark"
    )
    ...
    print(f"Perfect Integrator benchmark: {N_STEPS} steps x {N_REPEATS} repeats")
