...
# SC-NeuroCore — Multicompartment MCN RK4 multi-backend local regression benchmark
...
REPEATS = 5
CURRENT = 3.2
OUTPUT = Path("benchmarks/results/local_python_2026-06-26_multicompartment_mcn_rk4.json")
REPO_ROOT = Path(__file__).resolve().parents[1]
GO_BENCH_RE = re.compile(r"^BenchmarkMulticompartmentMCNRK4-\d+\s+\d+\s+([0-9.]+)\s+ns/op")
GO_SPIKES_RE = re.compile(r"\s([0-9.]+)\s+spikes(?:\s|$)")
SOURCE_HASH_PATHS = {
    "benchmarks/bench_model_multicompartment_mcn.py": REPO_ROOT
    / "benchmarks/bench_model_multicompartment_mcn.py",
    ...
    / "src/sc_neurocore/accel/go/services/multicompartment_mcn.go",
    "src/sc_neurocore/accel/go/services/multicompartment_mcn_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/multicompartment_mcn_test.go",
    "src/sc_neurocore/accel/julia/neurons/multicompartment_mcn.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/multicompartment_mcn.jl",
    "src/sc_neurocore/accel/mojo/kernels/multicompartment_mcn.mojo": REPO_ROOT
    / "src/sc_neurocore/accel/mojo/kernels/multicompartment_mcn.mojo",
}
...
def _run_once(backend: str) -> dict[str, object]:
    neuron: _StepNeuron = MulticompartmentMCNNeuron()
    spikes = 0
    ...
    return {
        ...
        "spikes": spikes,
        "ending_state": [float(neuron.u), float(neuron.v_basal), float(neuron.v_apical)],
    }
...
def _run_rust_backend() -> dict[str, object]:
    command = [
        ...
        "engine/Cargo.toml",
        "--example",
        "bench_multicompartment_mcn_rk4",
    ]
    try:
...
def _run_go_backend() -> dict[str, object]:
    command = [
        ...
        "^$",
        "-bench",
        "BenchmarkMulticompartmentMCNRK4$",
        "-benchtime",
        "200000x",
        "-count",
        str(REPEATS),
    ...
    if not values:
        return {
            "backend": "go",
            "skipped": True,
            "reason": "Go benchmark output did not include BenchmarkMulticompartmentMCNRK4 ns/op rows",
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/multicompartment_mcn.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = MulticompartmentMcnAccel.MulticompartmentMCNNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        spikes += MulticompartmentMcnAccel.step!(s, CURRENT)
    end
    elapsed = time_ns() - start
    return elapsed / STEPS, spikes, s.u, s.v_basal, s.v_apical
end
...
println("final_us=", join([r[3] for r in results], ","))
println("final_basals=", join([r[4] for r in results], ","))
println("final_apicals=", join([r[5] for r in results], ","))
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <multicompartment_mcn rk4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        ...
        "final_us": [float(value) for value in fields["final_us"].split(",")],
        "final_basals": [float(value) for value in fields["final_basals"].split(",")],
        "final_apicals": [float(value) for value in fields["final_apicals"].split(",")],
    }
...
def _run_mojo_backend() -> dict[str, object]:
    program = textwrap.dedent(
        f"""
        from multicompartment_mcn import MulticompartmentMCN
    ...
    return {
        "backend": "mojo",
        "command": "mojo run --disable-warnings -I src/sc_neurocore/accel/mojo/kernels <temp multicompartment_mcn benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": statistics.median(values),
...
def main() -> None:
    ...
    payload = {
        ...
        "benchmark": "MulticompartmentMCNNeuron candidate-first RK4 three-compartment step",
        "timestamp_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "command": "PYTHONPATH=src .venv/bin/python benchmarks/bench_model_multicompartment_mcn.py",
