...
# SC-NeuroCore — Python/Rust benchmark for Traub-Miles 1991 RK4 dynamics

"""Measure Traub-Miles Python reference and Rust PyO3 class throughput."""
...
PARITY_STEPS = 2_000
CURRENT = 5.0
INTEGRATOR = "ten_substep_candidate_first_rk4"
...
def _run_python(n_steps: int) -> tuple[float, dict[str, float | int]]:
    ...
    return wall, {"spikes": spikes, "v": neuron.v, "m": neuron.m, "h": neuron.h, "n": neuron.n}
...
def _probe_rust_class() -> type[Any] | None:
    try:
        ...
        return None
    cls = getattr(module, "TraubMilesNeuron", None)
...
def _parity() -> dict[str, float | str]:
    ...
    return {
        "status": "measured",
        "max_abs_delta": max(
            abs(float(py_state[name]) - float(rust_state[name])) for name in ("v", "m", "h", "n")
        ),
    }
...
def main() -> int:
    ...
    out_path = RESULTS_DIR / "bench_traub_miles.json"
