...
# SC-NeuroCore — PVFastSpiking RK4 multi-backend local regression benchmark
...
REPEATS = 5
CURRENT = 2.0
OUTPUT = Path("benchmarks/results/local_python_2026-06-24_pv_fast_spiking_rk4.json")
REPO_ROOT = Path(__file__).resolve().parents[1]
GO_BENCH_RE = re.compile(r"^BenchmarkPVFastSpikingRK4-\d+\s+\d+\s+([0-9.]+)\s+ns/op")
GO_SPIKES_RE = re.compile(r"\s([0-9.]+)\s+spikes(?:\s|$)")
SOURCE_HASH_PATHS = {
    "benchmarks/bench_model_pv_fast_spiking_neuron.py": REPO_ROOT
    / "benchmarks/bench_model_pv_fast_spiking_neuron.py",
    ...
    / "src/sc_neurocore/accel/go/services/pv_fast_spiking_neuron.go",
    "src/sc_neurocore/accel/go/services/pv_fast_spiking_neuron_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/pv_fast_spiking_neuron_test.go",
    "src/sc_neurocore/accel/julia/neurons/pv_fast_spiking_neuron.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/pv_fast_spiking_neuron.jl",
    "src/sc_neurocore/accel/mojo/kernels/pv_fast_spiking_neuron.mojo": REPO_ROOT
    / "src/sc_neurocore/accel/mojo/kernels/pv_fast_spiking_neuron.mojo",
}
...
def _run_once(backend: str) -> dict[str, object]:
    neuron: _StepNeuron = PVFastSpikingNeuron()
    spikes = 0
...
def _run_go_backend() -> dict[str, object]:
    ...
    if not values:
        return {
            "backend": "go",
            "skipped": True,
            "reason": "Go benchmark output did not include BenchmarkPVFastSpikingRK4 ns/op rows",
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/pv_fast_spiking_neuron.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = PvFastSpikingNeuronAccel.PVFastSpikingNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        spikes += PvFastSpikingNeuronAccel.step!(s, CURRENT)
    end
...
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <pv_fast_spiking_neuron rk4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
...
def _run_mojo_backend() -> dict[str, object]:
    program = textwrap.dedent(
        f"""
        from pv_fast_spiking_neuron import PVFastSpiking
    ...
    return {
        "backend": "mojo",
        "command": "mojo run --disable-warnings -I src/sc_neurocore/accel/mojo/kernels <temp pv_fast_spiking_neuron benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": statistics.median(values),
...
def _require_all_backends(payloads: list[dict[str, object]]) -> None:
    ...
    if skipped:
        raise RuntimeError("PVFastSpiking benchmark requires every backend: " + "; ".join(skipped))
...
def main() -> None:
    ...
    payload = {
        ...
        "benchmark": "PVFastSpikingNeuron candidate-first RK4 Wang-Buzsaki + Kv3 fast-spiking step",
        "timestamp_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "command": "PYTHONPATH=src .venv/bin/python benchmarks/bench_model_pv_fast_spiking_neuron.py",
