...
# SC-NeuroCore — Neurogrid RK4 multi-backend local regression benchmark
...
SOURCE_HASH_PATHS = {
    ...
    / "src/sc_neurocore/accel/go/services/neurogrid.go",
    "src/sc_neurocore/accel/go/services/neurogrid_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/neurogrid_test.go",
    "src/sc_neurocore/accel/julia/neurons/neurogrid.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/neurogrid.jl",
    "src/sc_neurocore/accel/mojo/kernels/neurogrid.mojo": REPO_ROOT
...
def _run_once(backend: str) -> dict[str, object]:
    neuron: _StepNeuron = NeuroGridNeuron()
    spikes = 0
    ...
    return {
        ...
        "spikes": spikes,
        "ending_state": [float(neuron.v_s), float(neuron.v_d)],
    }
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/neurogrid.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = NeurogridAccel.NeuroGridNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        spikes += NeurogridAccel.step!(s, CURRENT)
    end
    elapsed = time_ns() - start
    return elapsed / STEPS, spikes, s.v_s, s.v_d
end
...
println("final_vds=", join([r[4] for r in results], ","))
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <neurogrid rk4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        ...
        "final_vds": [float(value) for value in fields["final_vds"].split(",")],
    }
