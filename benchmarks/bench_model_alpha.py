...
# SC-NeuroCore — Source-bound five-runtime alpha-synapse benchmark

"""Measure and fail-closed verify the complete alpha-synapse runtime set."""
...
DEFAULT_OUTPUT = REPOSITORY / "benchmarks/results/bench_alpha.json"
...
TRACE_KEYS = ("v", "a_exc", "i_exc", "a_inh", "i_inh", "spikes")
FINAL_KEYS = ("v_final", "a_exc_final", "i_exc_final", "a_inh_final", "i_inh_final")
INITIAL_STATE = {
    "v": 0.15,
    "a_exc": 0.08,
    "i_exc": 0.05,
...
SOURCE_PATHS = (
    ...
    "src/sc_neurocore/accel/go/alpha/__init__.py",
    "src/sc_neurocore/accel/go/alpha/alpha.go",
    "src/sc_neurocore/accel/go/services/alpha.go",
    "src/sc_neurocore/accel/go/services/alpha_test.go",
    "src/sc_neurocore/accel/julia/neurons/__init__.py",
    "src/sc_neurocore/accel/julia/neurons/alpha.jl",
    "src/sc_neurocore/accel/mojo/kernels/alpha.mojo",
    "src/sc_neurocore/accel/mojo/pixi.lock",
    "src/sc_neurocore/accel/mojo/pixi.toml",
    "src/sc_neurocore/accel/mojo/alpha/__init__.py",
    "src/sc_neurocore/accel/mojo/alpha/alpha.mojo",
    "src/sc_neurocore/accel/alpha.py",
    "src/sc_neurocore/accel/rust/safety/alpha.rs",
    "src/sc_neurocore/neurons/model_descriptors/AlphaNeuron.toml",
    "src/sc_neurocore/neurons/model_schemas/alpha.json",
    "src/sc_neurocore/neurons/model_schemas/alpha.toml",
    "src/sc_neurocore/neurons/models/alpha.py",
    "src/sc_neurocore/neurons/reference_trace_data/alpha_dual_synapse_doi.json",
...
def _binary_hashes() -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "go_shared_library": _binary_record(
            REPOSITORY / "src/sc_neurocore/accel/go/alpha/libalpha.so"
        ),
        "mojo_shared_library": _binary_record(
            REPOSITORY / "src/sc_neurocore/accel/mojo/alpha/libalpha.so"
        ),
    }


def _drive(n_steps: int) -> tuple[FloatArray, FloatArray]:
    index: FloatArray = np.arange(n_steps, dtype=np.float64)
    return (
        2.0 + 0.8 * np.sin(index * 0.037) + 0.2 * np.cos(index * 0.011),
...
def _run_backend(backend: str, n_steps: int) -> BenchmarkResult:
    exc, inh = _drive(n_steps)
    return AlphaNeuron(**INITIAL_STATE).simulate(exc, inh, backend=backend)
...
def _go_binary_metadata() -> dict[str, object]:
    path = REPOSITORY / "src/sc_neurocore/accel/go/alpha/libalpha.so"
    completed = subprocess.run(
...
def _verify_rust_safety() -> dict[str, object]:
    source = REPOSITORY / "src/sc_neurocore/accel/rust/safety/alpha.rs"
    with tempfile.TemporaryDirectory(prefix="alpha-rust-") as directory:
        binary = Path(directory) / "alpha_tests"
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "benchmark": "AlphaNeuron five-runtime exact-flow batch",
        "kernel": KERNEL,
        "timestamp_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "command": (
            "taskset -c <cpu> env PYTHONPATH=$WHEEL_SITE:src:. python "
            "benchmarks/bench_model_alpha.py --json "
            "benchmarks/results/bench_alpha.json"
        ),
        "evidence_class": "local_regression_single_cpu_affinity_non_exclusive",
        "production_speed_claim": False,
        "hardware_measurement_claimed": False,
        "parity_contract": {
            "observable": "six complete traces, five final states, and spike count",
        ...
        "workload": {
            ...
            "input_formula": "exc: 2.0 + 0.8*sin(step*0.037) + 0.2*cos(step*0.011); inh: 0.7 + 0.3*cos(step*0.021)",
