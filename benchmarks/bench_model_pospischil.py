...
# SC-NeuroCore — Pospischil RK4 multi-backend local regression benchmark
...
REPEATS = 5
CURRENT = 5.0
OUTPUT = Path("benchmarks/results/local_python_2026-06-23_pospischil_rk4.json")
REPO_ROOT = Path(__file__).resolve().parents[1]
GO_BENCH_RE = re.compile(r"^BenchmarkPospischilRK4(?:-\d+)?\s+\d+\s+([0-9.]+)\s+ns/op")
GO_SPIKES_RE = re.compile(r"\s([0-9.]+)\s+spikes(?:\s|$)")
SOURCE_HASH_PATHS = {
    "benchmarks/bench_model_pospischil.py": REPO_ROOT / "benchmarks/bench_model_pospischil.py",
    "engine/Cargo.toml": REPO_ROOT / "engine/Cargo.toml",
    "engine/examples/bench_pospischil_rk4.rs": REPO_ROOT
    / "engine/examples/bench_pospischil_rk4.rs",
    "engine/src/neurons/biophysical.rs": REPO_ROOT / "engine/src/neurons/biophysical.rs",
    "src/sc_neurocore/neurons/models/pospischil.py": REPO_ROOT
    / "src/sc_neurocore/neurons/models/pospischil.py",
    "src/sc_neurocore/accel/go/services/pospischil.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/pospischil.go",
    "src/sc_neurocore/accel/go/services/pospischil_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/pospischil_test.go",
    "src/sc_neurocore/accel/julia/neurons/pospischil.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/pospischil.jl",
    "src/sc_neurocore/accel/mojo/kernels/pospischil.mojo": REPO_ROOT
    / "src/sc_neurocore/accel/mojo/kernels/pospischil.mojo",
}
...
def _run_once(backend: str) -> dict[str, object]:
    neuron: _StepNeuron = PospischilNeuron()
    spikes = 0
...
def _run_go_backend() -> dict[str, object]:
    ...
    if not values:
        return {
            "backend": "go",
            "skipped": True,
            "reason": "Go benchmark output did not include BenchmarkPospischilRK4 ns/op rows",
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/pospischil.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = PospischilAccel.PospischilNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        spikes += PospischilAccel.step!(s, CURRENT)
    end
...
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <pospischil rk4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
...
def _run_mojo_backend() -> dict[str, object]:
    program = textwrap.dedent(
        f"""
        from pospischil import Pospischil
    ...
    return {
        "backend": "mojo",
        "command": "mojo run --disable-warnings -I src/sc_neurocore/accel/mojo/kernels <temp pospischil benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": statistics.median(values),
...
def _require_all_backends(payloads: list[dict[str, object]]) -> None:
    ...
    if skipped:
        raise RuntimeError("Pospischil benchmark requires every backend: " + "; ".join(skipped))
...
def main() -> None:
    ...
    payload = {
        ...
        "benchmark": "PospischilNeuron candidate-first RK4 Hodgkin-Huxley step",
        "timestamp_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "command": "PYTHONPATH=src .venv/bin/python benchmarks/bench_model_pospischil.py",
