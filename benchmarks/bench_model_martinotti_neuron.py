...
# SC-NeuroCore — Martinotti RK4 multi-backend local regression benchmark
...
REPEATS = 5
CURRENT = 5.0
OUTPUT = Path("benchmarks/results/local_python_2026-06-24_martinotti_neuron_rk4.json")
...
SOURCE_HASH_PATHS = {
    ...
    / "src/sc_neurocore/accel/go/services/martinotti_neuron.go",
    "src/sc_neurocore/accel/go/services/martinotti_neuron_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/martinotti_neuron_test.go",
    "src/sc_neurocore/accel/julia/neurons/martinotti_neuron.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/martinotti_neuron.jl",
    "src/sc_neurocore/accel/mojo/kernels/martinotti_neuron.mojo": REPO_ROOT
    / "src/sc_neurocore/accel/mojo/kernels/martinotti_neuron.mojo",
}
...
def _run_once(backend: str) -> dict[str, object]:
    neuron: _StepNeuron = MartinottiNeuron()
    spikes = 0
...
def _run_go_backend() -> dict[str, object]:
    ...
    if not values:
        return {
            "backend": "go",
            "skipped": True,
            "reason": "Go benchmark output did not include BenchmarkMartinottiRK4 ns/op rows",
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/martinotti_neuron.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = MartinottiNeuronAccel.MartinottiNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        spikes += MartinottiNeuronAccel.step!(s, CURRENT)
    end
...
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <martinotti_neuron rk4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
...
def _run_mojo_backend() -> dict[str, object]:
    program = textwrap.dedent(
        f"""
        from martinotti_neuron import Martinotti
    ...
    return {
        "backend": "mojo",
        "command": "mojo run --disable-warnings -I src/sc_neurocore/accel/mojo/kernels <temp martinotti_neuron benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": statistics.median(values),
...
def _require_all_backends(payloads: list[dict[str, object]]) -> None:
    ...
    if skipped:
        raise RuntimeError("Martinotti benchmark requires every backend: " + "; ".join(skipped))
