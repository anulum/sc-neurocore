...
# SC-NeuroCore — Wilson 1999 polynomial cortical model multi-language benchmark

"""Multi-language benchmark for ``WilsonHRNeuron.simulate``.
...
Usage::
    ...
    python benchmarks/bench_wilson_hr_simulate.py --json benchmarks/results/bench_wilson_hr_simulate.json
...
def _probe_rust() -> tuple[bool, str]:
    ok = wilson_hr._HAS_RUST
    return (ok, "" if ok else "engine wheel lacks the symbol")


def _probe_julia() -> tuple[bool, str]:
    ok = wilson_hr._ensure_julia_loaded()
    return (ok, "" if ok else "juliacall/.jl unavailable")


def _probe_go() -> tuple[bool, str]:
    ok = wilson_hr._ensure_go_loaded()
    return (ok, "" if ok else "accel/go/neurons/wilson_hr/libwilsonhr.so not built")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Wilson 1999 cortical multi-language benchmark.")
