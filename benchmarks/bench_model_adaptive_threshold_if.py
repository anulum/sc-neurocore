...
# SC-NeuroCore — AdaptiveThresholdIFNeuron exact-relaxation benchmark
...
def main() -> None:
    ...
    payload = {
        "benchmark": "AdaptiveThresholdIFNeuron Python exact-relaxation step",
        "timestamp_utc": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "command": "PYTHONPATH=src .venv/bin/python benchmarks/bench_model_adaptive_threshold_if.py",
    ...
    output = args.output
    if output is None:
        output = Path("benchmarks/results/local_python_2026-06-01_adaptive_threshold_if.json")
