...
# SC-NeuroCore — Source-bound five-runtime adaptive-threshold benchmark

"""Measure and fail-closed verify the complete adaptive-threshold runtime set."""
...
DEFAULT_OUTPUT = REPOSITORY / "benchmarks/results/bench_adaptive_threshold_if.json"
...
SOURCE_PATHS = (
    ...
    "engine/Cargo.toml",
    "engine/src/bindings/adaptive_threshold_if.rs",
    ...
    "src/sc_neurocore/accel/go/adaptive_threshold_if/adaptive_threshold_if.go",
    "src/sc_neurocore/accel/go/services/adaptive_threshold_if.go",
    "src/sc_neurocore/accel/go/services/adaptive_threshold_if_test.go",
    "src/sc_neurocore/accel/julia/neurons/__init__.py",
    "src/sc_neurocore/accel/julia/neurons/adaptive_threshold_if.jl",
    "src/sc_neurocore/accel/mojo/kernels/adaptive_threshold_if.mojo",
    "src/sc_neurocore/accel/mojo/pixi.lock",
    "src/sc_neurocore/accel/mojo/pixi.toml",
    "src/sc_neurocore/accel/mojo/adaptive_threshold_if/__init__.py",
    "src/sc_neurocore/accel/mojo/adaptive_threshold_if/adaptive_threshold_if.mojo",
    "src/sc_neurocore/accel/adaptive_threshold_if.py",
    "src/sc_neurocore/accel/rust/safety/adaptive_threshold_if.rs",
    "src/sc_neurocore/neurons/model_descriptors/AdaptiveThresholdIFNeuron.toml",
    "src/sc_neurocore/neurons/model_schemas/adaptive_threshold_if.json",
    "src/sc_neurocore/neurons/model_schemas/adaptive_threshold_if.toml",
    "src/sc_neurocore/neurons/models/adaptive_threshold_if.py",
    "src/sc_neurocore/neurons/reference_trace_data/adaptive_threshold_if_tonic_adaptation_doi.json",
    "tools/build_accel_backends.py",
    "tools/check_mojo_isa_baseline.py",
)

FloatArray: TypeAlias = npt.NDArray[np.float64]
BenchmarkResult: TypeAlias = dict[str, FloatArray | float | int]
...
def _binary_hashes() -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "go_shared_library": _binary_record(
            REPOSITORY
            / "src/sc_neurocore/accel/go/adaptive_threshold_if/libadaptive_threshold_if.so"
        ),
        "mojo_shared_library": _binary_record(
            REPOSITORY
            / "src/sc_neurocore/accel/mojo/adaptive_threshold_if/libadaptive_threshold_if.so"
        ),
    }
...
def _run_backend(backend: str, n_steps: int) -> BenchmarkResult:
    return AdaptiveThresholdIFNeuron(**INITIAL_STATE).simulate(_drive(n_steps), backend=backend)
...
def _go_binary_metadata() -> dict[str, object]:
    path = (
        REPOSITORY / "src/sc_neurocore/accel/go/adaptive_threshold_if/libadaptive_threshold_if.so"
    )
    completed = subprocess.run(
...
def _verify_rust_safety() -> dict[str, object]:
    source = REPOSITORY / "src/sc_neurocore/accel/rust/safety/adaptive_threshold_if.rs"
    with tempfile.TemporaryDirectory(prefix="adaptive-threshold-rust-") as directory:
        binary = Path(directory) / "adaptive_threshold_if_tests"
    ...
    return {
        "command": [
            ...
            "-o",
            "<temporary>/adaptive_threshold_if_tests",
        ],
...
def _backend_record(
    ...
    reference_event_count = int(reference["spike_count"])
    ...
    return {
        ...
        "spike_count": event_count,
        "spike_count_matches_python": event_count == reference_event_count,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    for backend in BACKENDS:
        ...
        parity_failed |= not bool(record["spike_count_matches_python"])
    if parity_failed:
    ...
    report: dict[str, Any] = {
        ...
        "benchmark": "AdaptiveThresholdIFNeuron five-runtime exact-relaxation batch",
        "kernel": KERNEL,
        "timestamp_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "command": (
            "taskset -c <cpu> env PYTHONPATH=$WHEEL_SITE:src:. python "
            "benchmarks/bench_model_adaptive_threshold_if.py --json "
            "benchmarks/results/bench_adaptive_threshold_if.json"
        ),
        "evidence_class": "local_regression_single_cpu_affinity_non_exclusive",
        "production_speed_claim": False,
        "hardware_measurement_claimed": False,
        "parity_contract": {
            "observable": "three complete traces, two final states, and spike count",
            "comparison": {name: PARITY_ATOL[name] for name in BACKENDS},
            "reason": "independent compiler transcendental evaluation may differ by bounded ulps",
        },
        "workload": {
            "n_steps": args.steps,
            "repeats": args.repeats,
            "warmup_steps": min(WARMUP_STEPS, args.steps),
            "input_formula": "22.0 + 6.0*sin(step*0.037) + 1.5*cos(step*0.011)",
