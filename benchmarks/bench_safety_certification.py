...
# SC-NeuroCore — reproducible safety certification benchmark

"""Compare safety-evidence import and package-generation cost across source trees."""
...
_SCHEMA_VERSION = "sc-neurocore.safety-certification-benchmark.v1"
_CHILD_PROGRAM = """
import json
import resource
import statistics
...
for _ in range(10):
    ...
    operation_samples.append(time.perf_counter_ns() - started)
generation_ns = int(statistics.median(operation_samples))
    ...
    json.dumps(
        {
            ...
            "max_rss_kib": rss,
            "checklist_count": len(package.checklist),
        }
    )
)
""".strip()
...
class Variant:
    """One source checkout measured by the safety certification probe."""
...
def _source_digest(root: Path) -> str:
    """Hash the complete imported safety package surface."""
    safety = root / "src" / "sc_neurocore" / "safety_cert"
    files = sorted(safety.glob("*.py"))
    if not files:
        raise ValueError(f"no safety certification source surface found below {root}")
...
def _git_metadata(variant: Variant) -> dict[str, object]:
    ...
    return {
        ...
        "git_head": git_head,
        "safety_cert_source_sha256": _source_digest(variant.root),
        "safety_cert_surface_dirty": dirty,
    }
...
def _sample(
    ...
    *,
    ...
    generation_ns = payload.get("generation_ns")
    checklist_count = payload.get("checklist_count")
    ...
    if not isinstance(generation_ns, int) or isinstance(generation_ns, bool):
        raise RuntimeError(f"{variant.label} probe emitted non-integer metrics")
    if not isinstance(max_rss_kib, int) or isinstance(max_rss_kib, bool):
        raise RuntimeError(f"{variant.label} probe emitted non-integer metrics")
    if checklist_count != 7:
        raise RuntimeError(f"{variant.label} safety package returned an invalid checklist")
    return {
...
def _statistics(samples: list[dict[str, int]]) -> dict[str, object]:
    ...
    for metric in ("subprocess_wall_ns", "import_ns", "generation_ns", "max_rss_kib"):
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    evidence: dict[str, object] = {
        ...
        "operation": "import safety package and generate one deterministic IEC 61508 evidence set",
        "configuration": {
        ...
        "commands": {
            "harness": (
                "python benchmarks/bench_safety_certification.py --baseline-root <parent-root> "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"safety-evidence benchmark written: {args.output}")
    return 0
