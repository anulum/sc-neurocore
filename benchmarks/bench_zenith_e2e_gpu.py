...
def benchmark_torch_autograd(scale: int, rule: int, device: str) -> Dict[str, Any]:
    print(f"  [Torch] Benchmarking Surrogate Autograd (Rule: {rule}, Scale: {scale}, Device: {device})")
    ...
    t0_fwd = benchmark.Timer(
        ...
        globals={"layer": layer, "pre_spikes": pre_spikes, "post_spikes": post_spikes, "rewards": rewards, "dt": dt}
    )
...
def benchmark_rust_physics(scale: int, rule: int, force_deterministic: bool = True, backend="rust") -> Dict[str, Any]:
    print(f"  [{backend.upper()}] Benchmarking Native Physics (Rule: {rule}, Scale: {scale}, Deterministic: {force_deterministic})")
...
def check_parity(scale: int) -> bool:
    ...
    torch_layer = create_plasticity_layer(count=scale, rule_type=RULE_STDP, backend="torch", weight=0.5)
    rust_layer = create_plasticity_layer(count=scale, rule_type=RULE_STDP, backend="rust", weight=0.5)
...
def main():
    ...
    parser.add_argument("--scale", type=int, nargs='+', default=[1_000_000], help="Scales to test (e.g. 100000 1000000)")
    parser.add_argument("--rules", type=str, default="all", help="Rules to test: 'all' or 'stdp,bcm'")
    parser.add_argument("--backend", type=str, default="all", help="Native backend to test against torch ('rust', 'rust-wgpu', or 'all')")
    parser.add_argument("--output", type=str, default="zenith_benchmarks.json", help="JSON output file.")
    parser.add_argument("--ci", action="store_true", help="Run in CI mode (errors out if failure).")
    parser.add_argument("--quick", action="store_true", help="Quick mode (100k nodes overrides scale).")
    ...
    if not torch.cuda.is_available():
        print("CUDA ERROR: The explicit GPU benchmark `bench_zenith_e2e_gpu.py` requires an active CUDA GPU environment.")
    ...
    for scale in scales:
        ...
        for rule_name, rule_id in zip(rule_names, rules):
            ...
            if args.backend in ["rust", "all"]:
                rust_res_det = benchmark_rust_physics(scale, rule_id, force_deterministic=True, backend="rust")
                results[scale][rule_name]["rust"] = rust_res_det
            if args.backend in ["rust-wgpu", "all"]:
                rust_wgpu_res = benchmark_rust_physics(scale, rule_id, force_deterministic=True, backend="rust-wgpu")
