...
# SC-NeuroCore — Python/Rust benchmark for Prescott 2008 RK4 dynamics

"""Measure Prescott Python reference and Rust PyO3 class throughput."""
...
def _run_python(n_steps: int) -> tuple[float, dict[str, float | int]]:
    ...
    return wall, {"spikes": spikes, "v": neuron.v, "w": neuron.w}
...
def _probe_rust_class() -> type[Any] | None:
    try:
        ...
        return None
    cls = getattr(module, "PrescottNeuron", None)
...
def _run_rust(n_steps: int) -> tuple[float, dict[str, float | int]] | None:
    ...
    return wall, {"spikes": spikes, "v": float(state["v"]), "w": float(state["w"])}
...
def _parity() -> dict[str, float | str]:
    ...
    return {
        "status": "measured",
        "max_abs_delta": max(
            abs(float(py_state[name]) - float(rust_state[name])) for name in ("v", "w")
        ),
...
def main() -> int:
    ...
    out_path = RESULTS_DIR / "bench_prescott.json"
