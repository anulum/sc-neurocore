...
def time_trap_report(compiled: TrapReporter, inputs: np.ndarray) -> tuple[float, int, int, int]:
    ...
    for _ in range(ITERATIONS):
        ...
        underflow_count = int(report.underflow_count)
    elapsed_ns = time.perf_counter_ns() - start_ns
    return elapsed_ns / ITERATIONS, checksum, overflow_count, underflow_count
...
def main() -> int:
    ...
    mixed_underflow_report = underflow_mixed.precision_trap_report(underflow_mixed_inputs)
    bfp_underflow_report = underflow_bfp.precision_trap_report(underflow_bfp_inputs)
        ...
        "mixed_results": [
            {
                "ns_per_call": item[0],
                "checksum": item[1],
                "overflow_count": item[2],
                "underflow_count": item[3],
            }
