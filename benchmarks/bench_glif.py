...
# SC-NeuroCore — Python/Rust benchmark for GLIF RK4 dynamics

"""Measure GLIF Python reference and Rust PyO3 class throughput."""
...
def _state_dict(neuron: GLIFNeuron) -> dict[str, float]:
    return {
        "v": neuron.v,
        "theta": neuron.theta,
...
def _probe_rust_class() -> type[Any] | None:
    try:
        ...
        return None
    cls = getattr(module, "GLIFNeuron", None)
...
def _run_rust(n_steps: int) -> tuple[float, dict[str, float | int]] | None:
    ...
    return wall, {
        ...
        "i_asc1": float(state["i_asc1"]),
        "i_asc2": float(state["i_asc2"]),
    }
...
def main() -> int:
    ...
    out_path = RESULTS_DIR / "bench_glif.json"
