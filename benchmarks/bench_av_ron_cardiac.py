...
# SC-NeuroCore — Python/Rust benchmark for Av-Ron cardiac RK4 dynamics

"""Measure Av-Ron cardiac Python reference and Rust PyO3 throughput."""
...
def _state_dict(neuron: AvRonCardiacNeuron) -> dict[str, float]:
    return {"v": neuron.v, "h": neuron.h, "n": neuron.n, "s": neuron.s}
...
def _probe_rust_class() -> type[Any] | None:
    try:
        ...
        return None
    cls = getattr(module, "AvRonCardiacNeuron", None)
...
def _parity() -> dict[str, float | int | str]:
...
def main() -> int:
    ...
    out_path = RESULTS_DIR / "bench_av_ron_cardiac.json"
