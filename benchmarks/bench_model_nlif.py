...
# SC-NeuroCore — Nonlinear LIF RK4 multi-backend local regression benchmark
...
REPEATS = 5
CURRENT = 20.0
OUTPUT = Path("benchmarks/results/local_python_2026-06-18_nlif_rk4.json")
REPO_ROOT = Path(__file__).resolve().parents[1]
GO_BENCH_RE = re.compile(r"^BenchmarkNonlinearLIFRK4(?:-\d+)?\s+\d+\s+([0-9.]+)\s+ns/op")
GO_SPIKES_RE = re.compile(r"\s([0-9.]+)\s+spikes(?:\s|$)")
SOURCE_HASH_PATHS = {
    "benchmarks/bench_model_nlif.py": REPO_ROOT / "benchmarks/bench_model_nlif.py",
    "src/sc_neurocore/neurons/models/nlif.py": REPO_ROOT
    / "src/sc_neurocore/neurons/models/nlif.py",
    "src/sc_neurocore/accel/go/services/nlif.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/nlif.go",
    "src/sc_neurocore/accel/go/services/nlif_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/nlif_test.go",
    "src/sc_neurocore/accel/julia/neurons/nlif.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/nlif.jl",
    "src/sc_neurocore/accel/mojo/kernels/nlif.mojo": REPO_ROOT
    / "src/sc_neurocore/accel/mojo/kernels/nlif.mojo",
    "src/sc_neurocore/accel/rust/safety/nlif.rs": REPO_ROOT
    / "src/sc_neurocore/accel/rust/safety/nlif.rs",
}
...
def _run_python_backend() -> dict[str, object]:
    ...
    for _ in range(REPEATS):
        neuron: _StepNeuron = NonlinearLIFNeuron()
        spikes = 0
...
def _run_rust_backend() -> dict[str, object]:
    program = textwrap.dedent(
        f"""
        include!(r#"{(REPO_ROOT / "src/sc_neurocore/accel/rust/safety/nlif.rs").as_posix()}"#);
        ...
        fn run_once() -> (f64, i32, f64, f64) {{
            ...
            for _ in 0..STEPS {{
                let result = step(&mut state, CURRENT);
                if result < 0 {{
            ...
            (elapsed_ns / STEPS as f64, spikes, state.v, state.w)
        }}
        ...
        fn main() {{
            ...
            let mut final_ws = Vec::with_capacity(REPEATS);
            for _ in 0..REPEATS {{
                let (ns, spike_count, v, w) = run_once();
                values.push(ns);
            ...
            println!("final_ws={{}}", final_ws.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(","));
        }}
        """
    )
    with tempfile.TemporaryDirectory() as temp_dir:
        source = Path(temp_dir) / "bench_nlif.rs"
        binary = Path(temp_dir) / "bench_nlif"
    ...
    return {
        "backend": "rust",
        "command": "rustc -O <temp nlif safety benchmark> && <temp nlif safety benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/nlif.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = NlifAccel.NonlinearLIFNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        result = NlifAccel.step!(s, CURRENT)
        if result < 0
...
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <nlif RK4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": float(fields["median_ns_per_step"]),
...
def _run_mojo_backend() -> dict[str, object]:
    program = textwrap.dedent(
        f"""
        from nlif import nlif_next_v, nlif_next_w, nlif_step_spike
        ...
        def run_once() raises:
            ...
            for _ in range(STEPS):
                var next_v = nlif_next_v(v, w, CURRENT, -65.0, -40.0, -20.0, -65.0, 0.04, 0.5, 100.0, 1.0, 0.1)
                var next_w = nlif_next_w(v, w, CURRENT, -65.0, -40.0, -20.0, -65.0, 0.04, 0.5, 100.0, 1.0, 0.1)
                var spike = nlif_step_spike(v, w, CURRENT, -65.0, -40.0, -20.0, -65.0, 0.04, 0.5, 100.0, 1.0, 0.1)
                if spike < 0:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        source = Path(temp_dir) / "bench_nlif.mojo"
        source.write_text(program, encoding="utf-8")
        command = [
            "mojo",
            "-I",
    ...
    return {
        "backend": "mojo",
        "command": "mojo -I src/sc_neurocore/accel/mojo/kernels <temp nlif benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": statistics.median(values),
        ...
        "final_ws": [float(value) for value in fields.get("final_w", [])],
    }


def main() -> int:
    """Run the local multi-backend NLIF RK4 benchmark."""
    ...
    payload = {
        ...
        "benchmark": "NonlinearLIFNeuron candidate-first RK4 two-state ODE",
