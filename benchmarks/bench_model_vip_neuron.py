...
# SC-NeuroCore — VIP RK4 multi-backend local regression benchmark
...
REPEATS = 5
CURRENT = 1.0
OUTPUT = Path("benchmarks/results/local_python_2026-06-24_vip_neuron_rk4.json")
REPO_ROOT = Path(__file__).resolve().parents[1]
GO_BENCH_RE = re.compile(r"^BenchmarkVIPRK4-\d+\s+\d+\s+([0-9.]+)\s+ns/op")
GO_SPIKES_RE = re.compile(r"\s([0-9.]+)\s+spikes(?:\s|$)")
SOURCE_HASH_PATHS = {
    "benchmarks/bench_model_vip_neuron.py": REPO_ROOT / "benchmarks/bench_model_vip_neuron.py",
    ...
    / "src/sc_neurocore/accel/go/services/vip_neuron.go",
    "src/sc_neurocore/accel/go/services/vip_neuron_test.go": REPO_ROOT
    / "src/sc_neurocore/accel/go/services/vip_neuron_test.go",
    "src/sc_neurocore/accel/julia/neurons/vip_neuron.jl": REPO_ROOT
    / "src/sc_neurocore/accel/julia/neurons/vip_neuron.jl",
    "src/sc_neurocore/accel/mojo/kernels/vip_neuron.mojo": REPO_ROOT
    / "src/sc_neurocore/accel/mojo/kernels/vip_neuron.mojo",
}
...
def _run_once(backend: str) -> dict[str, object]:
    ...
    return {
        ...
        "spikes": spikes,
        "ending_state": [float(neuron.v), float(neuron.b)],
    }
...
def _run_go_backend() -> dict[str, object]:
    ...
    if not values:
        return {
            "backend": "go",
            "skipped": True,
            "reason": "Go benchmark output did not include BenchmarkVIPRK4 ns/op rows",
...
def _run_julia_backend() -> dict[str, object]:
    script = f"""
using Statistics
include("src/sc_neurocore/accel/julia/neurons/vip_neuron.jl")
const STEPS = {STEPS}
const REPEATS = {REPEATS}
const CURRENT = {CURRENT}
function run_once()
    s = VipNeuronAccel.VIPNeuronState()
    spikes = 0
    start = time_ns()
    for _ in 1:STEPS
        spikes += VipNeuronAccel.step!(s, CURRENT)
    end
    elapsed = time_ns() - start
    return elapsed / STEPS, spikes, s.v, s.b
end
...
println("final_bs=", join([r[4] for r in results], ","))
"""
    ...
    return {
        "backend": "julia",
        "command": "julia --project=. -e <vip_neuron rk4 benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        ...
        "final_bs": [float(value) for value in fields["final_bs"].split(",")],
    }
...
def _run_mojo_backend() -> dict[str, object]:
    ...
    return {
        "backend": "mojo",
        "command": "mojo run --disable-warnings -I src/sc_neurocore/accel/mojo/kernels <temp vip_neuron benchmark>",
        "steps": STEPS,
        "repeats": len(values),
        "current": CURRENT,
        "median_ns_per_step": statistics.median(values),
...
def _require_all_backends(payloads: list[dict[str, object]]) -> None:
    ...
    if skipped:
        raise RuntimeError("VIP benchmark requires every backend: " + "; ".join(skipped))
...
def main() -> None:
    ...
    payload = {
        ...
        "benchmark": "VIPNeuron candidate-first RK4 A-type-K irregular-spiking step",
        "timestamp_utc": datetime.now(UTC).isoformat(timespec="seconds"),
        "command": "PYTHONPATH=src .venv/bin/python benchmarks/bench_model_vip_neuron.py",
