...
# SC-NeuroCore — reproducible DNA mapper import benchmark

"""Compare DNA mapper import and golden-path cost across two source trees."""
...
_SCHEMA_VERSION = "sc-neurocore.dna-mapper-import-benchmark.v1"
_CHILD_PROGRAM = """
import json
import resource
import statistics
...
operation_ns = int(statistics.median(operation_samples))
...
class Variant:
    """One source checkout measured by the DNA mapper probe."""
...
def _source_digest(root: Path) -> str:
    """Hash the public package boundary and flat or modular mapper surface."""
    bridges = root / "src" / "sc_neurocore" / "bridges"
    files = [bridges / "__init__.py", *sorted(bridges.glob("dna_*.py"))]
    if not files:
        raise ValueError(f"no DNA mapper source surface found below {root}")
...
def _git_metadata(variant: Variant) -> dict[str, object]:
    ...
    status = subprocess.run(
        [
            ...
            "src/sc_neurocore/bridges/dna_*.py",
        ],
        capture_output=True,
        text=True,
        check=False,
    ...
    return {
        ...
        "git_head": git_head,
        "dna_mapper_source_sha256": _source_digest(variant.root),
        "dna_mapper_surface_dirty": dirty,
    }
...
def _sample(
    ...
    *,
    ...
    operation_ns = payload.get("operation_ns")
    ...
    if not isinstance(operation_ns, int) or isinstance(operation_ns, bool):
        raise RuntimeError(f"{variant.label} probe emitted non-integer metrics")
    if not isinstance(max_rss_kib, int) or isinstance(max_rss_kib, bool):
        raise RuntimeError(f"{variant.label} probe emitted non-integer metrics")
    if gate_count != 1:
        raise RuntimeError(f"{variant.label} DNA mapper returned an invalid design")
    return {
...
def _statistics(samples: list[dict[str, int]]) -> dict[str, object]:
    ...
    for metric in ("subprocess_wall_ns", "import_ns", "operation_ns", "max_rss_kib"):
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    evidence: dict[str, object] = {
        ...
        "operation": "import DNA mapper and compile one deterministic AND gate",
        "configuration": {
        ...
        "commands": {
            "harness": (
                "python benchmarks/bench_dna_mapper_import.py --baseline-root <parent-root> "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"DNA mapper import benchmark written: {args.output}")
    return 0
