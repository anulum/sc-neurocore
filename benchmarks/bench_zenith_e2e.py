...
def benchmark_rust_physics(scale: int, rule: int, force_deterministic: bool = True) -> Dict[str, Any]:
    print(f"  [Rust] Benchmarking Native Physics (Rule: {rule}, Scale: {scale}, Deterministic: {force_deterministic})")
...
def main():
    ...
    device = "cuda" if (torch.cuda.is_available() and check_cuda_works()) else "cpu"
    results = {}
    ...
    for scale in scales:
        ...
        for rule_name, rule_id in zip(rule_names, rules):
            ...
            # 2b. Rust Physics (Deterministic)
            rust_res_det = benchmark_rust_physics(scale, rule_id, force_deterministic=True)
            
            # 2c. Exascale IO
            io_res = benchmark_exascale_io(scale, rule_id)
            
            results[scale][rule_name] = {
                "torch": torch_res,
                "rust": rust_res_det,
                "io": io_res
            }
