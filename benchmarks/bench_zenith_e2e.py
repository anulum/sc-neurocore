...
# SC-NeuroCore — Zenith end-to-end benchmark
...
def benchmark_rust_physics(
    scale: int, rule: int, force_deterministic: bool = True
) -> Dict[str, Any]:
    print(
        f"  [Rust] Benchmarking Native Physics (Rule: {rule}, Scale: {scale}, Deterministic: {force_deterministic})"
    )
    ...
    layer = create_plasticity_layer(
        count=scale, rule_type=rule, backend="rust", param_a=0.1, param_b=0.05
    )
...
def main():
    ...
    for scale in scales:
        ...
        for rule_name, rule_id in zip(rule_names, rules):
            ...
            results[scale][rule_name] = {"torch": torch_res, "rust": rust_res_det, "io": io_res}
