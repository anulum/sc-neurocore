...
def _probe_backend(backend: str) -> tuple[bool, str]:
    ...
    if backend == "rust":
        return (
            expif._HAS_RUST,
            "" if expif._HAS_RUST else "Rust engine ExpIF symbol unavailable",
        )
    if backend == "julia":
        available = expif._ensure_julia_loaded()
        return available, "" if available else "juliacall or ExpIF Julia module unavailable"
    if backend == "go":
        available = expif._ensure_go_loaded()
        return available, "" if available else "compiled Go libexpif.so unavailable"
    available = expif._ensure_mojo_loaded()
    return available, "" if available else "compiled Mojo libexpif.so unavailable"
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(description="Controlled ExpIF five-backend benchmark")
    ...
    print(f"ExpIF benchmark: {N_STEPS} steps x {N_REPEATS} repeats")
