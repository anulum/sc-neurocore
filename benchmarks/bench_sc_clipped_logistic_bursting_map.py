...
# SC-NeuroCore — SC clipped-logistic bursting-map multi-language benchmark

"""Multi-language benchmark for ``SCClippedLogisticBurstingMapNeuron.simulate``.
...
Usage::
    ...
    python benchmarks/bench_sc_clipped_logistic_bursting_map.py
    python benchmarks/bench_sc_clipped_logistic_bursting_map.py --json benchmarks/results/bench_sc_clipped_logistic_bursting_map.json
...
from sc_neurocore.neurons.models import sc_clipped_logistic_bursting_map
...
SOURCE_HASH_PATHS = (
    "benchmarks/bench_sc_clipped_logistic_bursting_map.py",
    "src/sc_neurocore/neurons/models/sc_clipped_logistic_bursting_map.py",
    "engine/src/neurons/sc_clipped_logistic_bursting_map.rs",
    "src/sc_neurocore/accel/go/neurons/sc_clipped_logistic_bursting_map/sc_clipped_logistic_bursting_map.go",
    "src/sc_neurocore/accel/julia/neurons/sc_clipped_logistic_bursting_map.jl",
    "src/sc_neurocore/accel/mojo/neurons/sc_clipped_logistic_bursting_map.mojo",
)
...
def _probe_rust() -> tuple[bool, str]:
    return (
        sc_clipped_logistic_bursting_map._HAS_RUST,
        "" if sc_clipped_logistic_bursting_map._HAS_RUST else "engine wheel lacks the symbol",
    )


def _probe_julia() -> tuple[bool, str]:
    return (
        sc_clipped_logistic_bursting_map._ensure_julia_loaded(),
        ""
        if sc_clipped_logistic_bursting_map._ensure_julia_loaded()
        else "juliacall/.jl unavailable",
    )


def _probe_go() -> tuple[bool, str]:
    ok = sc_clipped_logistic_bursting_map._ensure_go_loaded()
    return (
        ok,
        ""
        if ok
        else "accel/go/neurons/sc_clipped_logistic_bursting_map/libsc_clipped_logistic_bursting_map.so not built",
    )
...
def _probe_mojo() -> tuple[bool, str]:
    ...
    ok = sc_clipped_logistic_bursting_map._ensure_mojo_loaded()
    return (ok, "" if ok else "accel/mojo/neurons/libsc_clipped_logistic_bursting_map.so not built")


def _run(backend: str) -> tuple[float, float, npt.NDArray[np.float64], int]:
    SCClippedLogisticBurstingMapNeuron().simulate(
        N_STEPS, CURRENT, backend=backend
    )  # warm-up (JIT for Julia)
    times_ms: list[float] = []
    trace: npt.NDArray[np.float64] = np.empty(0, dtype=np.float64)
    spikes = 0
    for _ in range(N_REPEATS):
        t0 = time.perf_counter()
        trace, spikes = SCClippedLogisticBurstingMapNeuron().simulate(
            N_STEPS, CURRENT, backend=backend
        )
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="SC clipped-logistic bursting-map multi-language benchmark."
    )
    ...
    print("# SC clipped-logistic bursting-map N-step benchmark")
    print(f"# Workload: {N_STEPS:,} steps, retained a=3.8 (chaotic), current={CURRENT}")
    ...
    if missing and not args.allow_unavailable_backends:
        print(
            "Missing required SC clipped-logistic backend(s): "
            + ", ".join(missing)
    ...
    report = {
        "benchmark": "sc_clipped_logistic_bursting_map_simulate",
