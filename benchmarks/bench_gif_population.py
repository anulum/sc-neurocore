...
# SC-NeuroCore — Python/Rust benchmark for GIF population exact flow

"""Measure GIF population exact-flow Python reference and Rust PyO3 throughput."""
...
CURRENT = -20.0
SEED = 42
INTEGRATOR = "exact_coupled_subthreshold_escape_rate"
...
def _state_dict(neuron: GIFPopulationNeuron) -> dict[str, float]:
    return {"v": neuron.v, "theta": neuron.theta, "eta": neuron.eta}


def _run_python(n_steps: int) -> tuple[float, dict[str, float | int]]:
    neuron = GIFPopulationNeuron(seed=SEED)
    spikes = 0
...
def _probe_rust_class() -> type[Any] | None:
    try:
        ...
        return None
    cls = getattr(module, "GIFPopulationNeuron", None)
...
def _parity() -> dict[str, float | str]:
    ...
    return {
        ...
        "note": "parity current is hyperpolarizing so seeded stochastic paths remain silent",
    }
...
def main() -> int:
    ...
    out_path = RESULTS_DIR / "bench_gif_population.json"
