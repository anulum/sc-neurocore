...
# SC-NeuroCore — source-bound Python benchmark for the 2,560-cell SC network

"""Measure deterministic SC Compte network execution without behaviour claims."""
...
from sc_neurocore.network import SCCompteWMNetwork

REPOSITORY = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT = REPOSITORY / "benchmarks/results/bench_sc_compte_wm_network.json"
N_STEPS = 1_000
N_REPEATS = 3
SOURCE_PATHS = (
    "benchmarks/bench_sc_compte_wm_network.py",
...
def build_payload(steps: int, repeats: int) -> dict[str, Any]:
    """Measure fresh deterministic runs and return a source-bound payload."""
    if steps <= 0 or repeats <= 0:
        raise ValueError("steps and repeats must be positive")
    SCCompteWMNetwork().run(16 * 0.02, statistics_window_ms=16 * 0.02)
    samples_ns: list[int] = []
    ...
    return {
        ...
        "execution_path": "python-vectorized-rk2-rfft",
