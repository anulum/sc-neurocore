...
# SC-NeuroCore — Julia acceleration for analysis_spike_stats/temporal
...
function burst_detection(binary_train, dt, max_isi_ms, min_spikes)
    ...
    while i < intervals.size
        if intervals[i] < max_isi
            start = i
            while i < intervals.size && intervals[i] < max_isi
                i += 1
            n_spikes = i - start + 1
            if n_spikes >= min_spikes
                bursts = push!(, (float(times[start]), float(times[i]), n_spikes))
        else
            i += 1
    return bursts
end
...
function response_onset(binary_train, baseline_steps, dt, threshold_sigma)
    ...
    above = findall(post > threshold)[0]
    if above.size == 0
        return float("nan")
