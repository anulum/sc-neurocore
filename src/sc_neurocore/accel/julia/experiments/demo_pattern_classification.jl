...
# SC-NeuroCore — Julia acceleration for experiments/demo_pattern_classification

module DemoPatternClassificationAccel
...
function nearest_centroid_classify(sample, centroid_A, centroid_B)
    ...
    dA = norm(sample - centroid_A)
    dB = norm(sample - centroid_B)
...
function demo()
    ...
    # --- TESTING: generate new samples && classify by nearest centroid ---
    ...
    labels_true = vcat([labels_A, labels_B])
    pred_list: list[int] = []
...
end # module DemoPatternClassificationAccel
