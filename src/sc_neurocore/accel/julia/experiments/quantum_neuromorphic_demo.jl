...
# SC-NeuroCore — Julia acceleration for experiments/quantum_neuromorphic_demo

module QuantumNeuromorphicDemoAccel
...
function run_demo()
    ...
    g1 = collect([1, 1, 0, 0, 1], dtype=np.uint8)
    g2 = collect([1, 0, 0, 0, 1], dtype=np.uint8)
    g3 = collect([0, 1, 0, 1, 0], dtype=np.uint8)
    ...
    print(f"    Small-World Edges: {sum(adj)}")
...
end # module QuantumNeuromorphicDemoAccel
