...
# SC-NeuroCore — Julia acceleration for experiments/demo_swarm_control

module DemoSwarmControlAccel
...
function run_demo()
    ...
    for step in 1:100
        ...
        if step % 20 == 0
            ...
            cx = positions[:, 0].mean()
            cy = positions[:, 1].mean()
            spread = sqrt(((positions - positions.mean(axis=0)) ^ 2).sum(axis=-1)).mean()
...
end # module DemoSwarmControlAccel
