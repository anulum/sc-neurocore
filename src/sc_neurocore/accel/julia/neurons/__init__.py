...
_ADAPTIVE_THRESHOLD_IF_LOADED = False
...
def _as_adaptive_threshold_if_input(
    current: npt.ArrayLike,
) -> npt.NDArray[np.float64]:
...
def simulate_adaptive_threshold_if(
    ...
    drive = _as_adaptive_threshold_if_input(current)
    module = _ensure_adaptive_threshold_if_loaded()
    v_out = np.empty(drive.size, dtype=np.float64)
