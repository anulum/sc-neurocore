...
# SC-NeuroCore — Julia for persistent_na_neuron

module PersistentNaNeuronAccel

export step!, simulate, PersistentNaNeuronState
...
function PersistentNaNeuronState()
    PersistentNaNeuronState(-65.0, 0.6, 0.32, 0.0, 35.0, 0.15, 9.0, 0.3, 55.0, -90.0, -65.0, 1.0, 5.0, 0.5, -20.0, 1.0, 0.0)
end

function step!(s::PersistentNaNeuronState, I_ext::Float64=0.0; dt::Float64=0.1)
    try
        inp = s.gain * I_ext
        ...
        for _ in 1:s._sub_steps
            ...
            i_l = s.g_l * (v - s.e_l)
            dv = (-i_na - i_nap - i_k - i_l + inp) / s.c_m
            s.v += sub_dt * dv
