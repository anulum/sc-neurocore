...
function step!(s::RenshawCellState, I_ext::Float64=0.0; dt::Float64=0.1)
    try
        ...
        for _ in 1:n_sub
            ...
            adapt_inf = 1.0 / (1.0 + exp(-(s.v + 30.0) / 5.0))
            s.adapt += (adapt_inf - s.adapt) / s.tau_adapt * s.dt
            i_na = s.g_na * m_inf ^ 3 * s.h * (s.v - s.e_na)
            i_k = s.g_k * s.n ^ 4 * (s.v - s.e_k)
            i_adapt = s.g_adapt * s.adapt * (s.v - s.e_k)
            i_l = s.g_l * (s.v - s.e_l)
            s.v += (-i_na - i_k - i_adapt - i_l + I_ext) / s.c_m * s.dt
        end
