...
export step!, simulate, ChayNeuronState, validate
...
function validate(s::ChayNeuronState)
    ...
    if any(x -> !_nonnegative(x), (s.g_ca, s.g_k, s.g_kca, s.g_l, s.rho, s.alpha_ca, s.k_ca))
        return false
    end
...
function _candidate(s::ChayNeuronState, v::Float64, n::Float64, ca::Float64, h::Float64, I_ext::Float64)
    ...
    i_k = s.g_k * n * (v - s.e_k)
    i_kca = s.g_kca * kca_act * (v - s.e_k)
    i_l = s.g_l * (v - s.e_l)

    v_next = v + (-i_ca - i_k - i_kca - i_l + I_ext) * h
    n_next = n + (n_inf - n) / max(tau_n, 0.01) * h
    ca_next = ca + s.rho * (-s.alpha_ca * i_ca - s.k_ca * ca) * h

    if !isfinite(v_next) || !(V_MIN <= v_next <= V_MAX)
        error("Chay voltage candidate outside safety envelope")
    end
    if !_probability(n_next)
        error("Chay n-gate candidate outside [0, 1]")
    end
    if !_nonnegative(ca_next) || ca_next > CA_MAX
        error("Chay calcium candidate outside safety envelope")
    end
    return v_next, n_next, ca_next
end

function step!(s::ChayNeuronState, I_ext::Float64=0.0; dt::Float64=s.dt)
