...
# SC-NeuroCore — Julia for sk_neuron

module SkNeuronAccel

export step!, simulate, SKNeuronState
...
function SKNeuronState()
    SKNeuronState(-65.0, 0.6, 0.32, 0.0, 35.0, 9.0, 2.0, 0.1, 55.0, -90.0, -65.0, 1.0, 5.0, 150.0, 0.5, -20.0, 1.0, 0.0)
end

function step!(s::SKNeuronState, I_ext::Float64=0.0; dt::Float64=0.1)
    try
        inp = s.gain * I_ext
        ...
        for _ in 1:s._sub_steps
            ...
            i_l = s.g_l * (v - s.e_l)
            dv = (-i_na - i_k - i_sk - i_l + inp) / s.c_m
            s.v += sub_dt * dv
