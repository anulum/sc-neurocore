...
# SC-NeuroCore — dedicated Julia Nagumo–Sato facade

"""Python facade for the maintained Julia Nagumo–Sato kernel."""
...
def _ensure_loaded() -> Any:
    ...
    if not _LOADED:
        path = _KERNEL_DIR / "nagumo_sato_map_neuron.jl"
        if not path.is_file():
            raise FileNotFoundError(f"Nagumo-Sato Julia kernel missing at {path}")
        _jl.include(str(path))
        _LOADED = True
    return _jl.NagumoSatoMapNeuronAccel
...
def simulate_nagumo_sato_map(
    ...
    try:
        y_final, x_final, spike_count = module.simulate_nagumo_sato_map_b(
            y, k, alpha, bias, drive, *traces
        )
...
__all__ = ["simulate_nagumo_sato_map"]
