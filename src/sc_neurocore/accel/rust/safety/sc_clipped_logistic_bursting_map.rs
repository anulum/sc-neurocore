...
// SC-NeuroCore — Rust safety for retained clipped-logistic bursting map
...
impl SCClippedLogisticBurstingMapNeuron {
    ...
    pub fn step(&mut self, i_ext: f64) -> i32 {
        if !validate_sc_clipped_logistic_bursting_map(self) || !i_ext.is_finite() {
            return 0;
        }
        // SC clipped-logistic logistic-driven map (models/sc_clipped_logistic_bursting_map.py step()):
        //   f     = a * x * (1 - x)
    ...
    pub fn reset(&mut self) {
        // Mirror models/sc_clipped_logistic_bursting_map.py `reset`: restore only the state variables x and y,
...
pub fn validate_sc_clipped_logistic_bursting_map(
    state: &SCClippedLogisticBurstingMapNeuron,
) -> bool {
    state.x.is_finite()
        && state.y.is_finite()
        && state.a.is_finite()
...
mod tests {
    ...
    // Independent re-derivation of one SC clipped-logistic map iteration, mirroring
    // models/sc_clipped_logistic_bursting_map.py step() exactly, to cross-check step().
    fn map_reference(n: &SCClippedLogisticBurstingMapNeuron, current: f64) -> (f64, f64) {
    ...
    #[test]
    fn test_sc_clipped_logistic_bursting_map_new() {
        let state = SCClippedLogisticBurstingMapNeuron::new();
        assert!(validate_sc_clipped_logistic_bursting_map(&state));
    }

    #[test]
    fn test_sc_clipped_logistic_bursting_map_step() {
        let mut state = SCClippedLogisticBurstingMapNeuron::new();
        let spike = state.step(10.0);
        assert!(spike == 0 || spike == 1);
    }

    #[test]
    fn test_sc_clipped_logistic_bursting_map_matches_reference_interior_and_clamped() {
        ...
        for (x0, y0, current) in [(0.3_f64, 0.1, 0.2), (0.5, -2.0, 0.0)] {
            let mut state = SCClippedLogisticBurstingMapNeuron {
                x: x0,
                y: y0,
                ..SCClippedLogisticBurstingMapNeuron::new()
            };
    ...
    #[test]
    fn test_sc_clipped_logistic_bursting_map_invalid_current_preserves_state() {
        let mut state = SCClippedLogisticBurstingMapNeuron::new();
        state.x = 0.42;
        state.y = 0.11;
    ...
    #[test]
    fn test_sc_clipped_logistic_bursting_map_overflow_preserves_state() {
        let mut state = SCClippedLogisticBurstingMapNeuron::new();
    ...
    fn matches_python_golden_spike_count() {
        // Parity with models/sc_clipped_logistic_bursting_map.py (default parameters). The SC clipped-logistic map is a
