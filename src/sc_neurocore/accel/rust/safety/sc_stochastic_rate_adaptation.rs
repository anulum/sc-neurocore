...
// SC-NeuroCore — Rust safety for SC stochastic rate adaptation
...
impl SCStochasticRateAdaptationNeuron {
    ...
    pub fn step(&mut self, i_ext: f64) -> i32 {
        if !i_ext.is_finite() || !validate_sc_stochastic_rate_adaptation(self) {
            return 0;
        }
...
#[cfg(test)]
fn rk4_reference(n: &SCStochasticRateAdaptationNeuron, i_ext: f64) -> (f64, f64) {
    let rhs = |a: f64| {
...
mod tests {
    ...
    fn rk4_candidate_matches_reference() {
        let state = SCStochasticRateAdaptationNeuron {
            a: 0.35,
            dt: 0.25,
            _rng: 0.5,
            ..SCStochasticRateAdaptationNeuron::new()
        };
    ...
    fn step_commits_rk4_candidate() {
        let mut state = SCStochasticRateAdaptationNeuron {
            a: 0.25,
            dt: 0.5,
            _rng: 0.5,
            ..SCStochasticRateAdaptationNeuron::new()
    ...
    fn test_benda_herz_new() {
        let state = SCStochasticRateAdaptationNeuron::new();
        assert!(validate_sc_stochastic_rate_adaptation(&state));
    }

    #[test]
    fn test_benda_herz_step() {
        let mut state = SCStochasticRateAdaptationNeuron::new();
