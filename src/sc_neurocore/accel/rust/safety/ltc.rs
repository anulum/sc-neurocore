...
// SC-NeuroCore — Rust safety for ltc
...
impl LiquidTimeConstantNeuron {
    ...
    pub fn step(&mut self, i_ext: f64) -> i32 {
        ...
        // tau = self.tau_base * (1.0 / (1.0 + (-(self.w_tau * current + self.bia
        // tau = max(tau, 0.1)
...
mod tests {
    ...
    #[test]
    fn test_ltc_new() {
        let state = LiquidTimeConstantNeuron::new();
        assert!(validate_ltc(&state));
    }

    #[test]
    fn test_ltc_step() {
        let mut state = LiquidTimeConstantNeuron::new();
