...
// SC-NeuroCore — Rust safety for hay_l5 candidate-first RK4
...
impl HayL5PyramidalNeuron {
    ...
    pub fn step(&mut self, i_ext: f64) -> i32 {
        self.step_dual(i_ext, 0.0)
    }

    pub fn step_dual(&mut self, current_soma: f64, current_tuft: f64) -> i32 {
        ...
        self.v_a = state[7];
        self.ca_a = state[8];
        i32::from(self.v_s >= self.v_threshold && v_prev < self.v_threshold)
    }
...
mod tests {
    ...
    #[test]
    fn test_hay_l5_cross_backend_somatic_anchor() {
        let mut state = HayL5PyramidalNeuron::new();
        ...
        assert_eq!(spikes, 1);
        assert!(state.ca_a >= 0.0);
    }

    #[test]
    fn test_hay_l5_cross_backend_dual_anchor() {
        let mut state = HayL5PyramidalNeuron::new();
        let mut spikes = 0_i32;
        for _ in 0..20_000 {
            spikes += state.step_dual(5.0, 5.0);
        }
        assert_eq!(spikes, 4);
    }

    #[test]
    fn test_hay_l5_invalid_input_preserves_state() {
        let mut state = HayL5PyramidalNeuron::new();
        for _ in 0..10 {
            let _ = state.step(10.0);
        }
        let old = [
            state.v_s, state.h_na, state.n_k, state.v_t, state.m_ca, state.h_ca, state.m_ih,
            state.v_a, state.ca_a,
        ];
    ...
    #[test]
    fn test_hay_l5_calcium_non_negative() {
        let mut state = HayL5PyramidalNeuron::new();
        for _ in 0..20_000 {
            let _ = state.step(10.0);
            assert!(state.ca_a >= 0.0);
