...
// SC-NeuroCore — Standalone Rust safety mirror for SKNeuron

#[derive(Debug, Clone)]
/// Standalone safety mirror of the WB base with the calcium-gated SK current, matching the Python
...
impl SKNeuron {
    ...
    pub fn step(&mut self, current: f64) -> Result<i32, &'static str> {
        ...
        for _ in 0..candidate.sub_steps {
            ...
            candidate.v += sub_dt * (-i_na - i_k - i_sk - i_l + input) / candidate.c_m;
...
pub fn validate_sk_neuron(state: &SKNeuron) -> bool {
    ...
    finite
        ...
        && (10.0..=2000.0).contains(&state.tau_ca)
        && state.dt > 0.0
        && state.dt <= 1.0
...
mod tests {
    ...
    fn nominal_step_matches_reference_anchor() {
        ...
        assert!((state.v - -63.180_064_213_072_19).abs() < 1.0e-12);
        assert!((state.h - 0.648_122_835_749_998_1).abs() < 1.0e-12);
        assert!((state.n - 0.237_186_365_946_861_5).abs() < 1.0e-12);
        assert_eq!(state.ca, 0.0);
    }
