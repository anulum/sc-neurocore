...
// SC-NeuroCore — Rust safety kernel for Hill and Tononi 2005

//! Source cortical-excitatory waking recurrence with optional Ih and IT.

#[derive(Clone, Debug)]
/// Dependency-free Hill-Tononi state and complete scalar-cell configuration.
...
impl HillTononiNeuron {
    ...
    /// Check that every state and parameter is finite and physically admissible.
    pub fn configuration_is_valid(&self) -> bool {
        let values = [
            self.v,
            self.theta,
            self.d_k,
    ...
    /// Advance one step and fail closed through the compatibility event API.
...
/// Validate a Hill-Tononi state without advancing it.
pub fn validate_hill_tononi(state: &HillTononiNeuron) -> bool {
    state.configuration_is_valid()
}
...
mod tests {
    ...
    fn source_defaults_and_python_anchor() {
        ...
        let expected = [
            ...
            0.287_184_735_636_522_2,
            0.145_178_520_059_308_1,
