...
// SC-NeuroCore — Rust safety for retained scaled-reset adaptive IF RK4 dynamics
...
impl SCScaledResetAdaptiveIFNeuron {
    ...
    /// Advance one interval, returning `None` when the candidate is invalid.
    pub fn try_step(&mut self, i_ext: f64) -> Option<i32> {
        if !i_ext.is_finite() || !self.valid_runtime() {
            return None;
        }
        let candidate = self.rk4_candidate(i_ext)?;
        ...
        if event == 1 {
            ...
            next.theta = next.theta.max(self.theta_reset);
            next.i1 += self.r1;
            next.i2 += self.r2;
        }
    ...
    /// Advance one interval and preserve state on invalid input.
...
pub fn validate_sc_scaled_reset_adaptive_if(state: &SCScaledResetAdaptiveIFNeuron) -> bool {
    state.valid_runtime()
}
...
mod tests {
    ...
    #[test]
    fn test_sc_scaled_reset_adaptive_if_new() {
        let state = SCScaledResetAdaptiveIFNeuron::new();
        assert!(state.v.is_finite());
        assert!(validate_sc_scaled_reset_adaptive_if(&state));
    }

    #[test]
    fn test_sc_scaled_reset_adaptive_if_rk4_reference_point() {
        let mut state = SCScaledResetAdaptiveIFNeuron::new();
    ...
    #[test]
    fn test_sc_scaled_reset_adaptive_if_spike_reset_uses_b() {
        let mut state = SCScaledResetAdaptiveIFNeuron::new();
        state.v = 0.99;
        state.b = 0.5;
        state.r1 = 1.25;
    ...
    #[test]
    fn test_sc_scaled_reset_adaptive_if_invalid_input_preserves_state() {
        let mut state = SCScaledResetAdaptiveIFNeuron::new();
        state.v = 0.2;
        state.i1 = 0.3;
    ...
    fn matches_python_golden_spike_count() {
        // Parity with models/sc_scaled_reset_adaptive_if.py (RK4 integrator, default parameters). The
        // retained scaled-reset adaptive IF 2009 right-hand side is entirely linear (leaky membrane, linearly-adapting
        ...
        // covered exactly by test_sc_scaled_reset_adaptive_if_spike_reset_uses_b). Drive gates it cleanly around
        ...
        // Rust-engine backends reproduce the same trajectory via test_sc_scaled_reset_adaptive_if_backends.py.
