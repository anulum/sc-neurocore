...
# SC-NeuroCore — Mojo-backed source-faithful Aihara batch

"""Typed ctypes facade for the Mojo Aihara shared library."""
...
_LIB_PATH = _ACCEL_ROOT / "mojo" / "aihara_map" / "libaihara_map.so"
...
try:
    ...
    _lib = None
_HAS_MOJO_AIHARA_MAP = _lib is not None
...
def simulate_aihara_map(
    ...
    """Run the checked Mojo implementation of Aihara Eqs. 10-12."""
    ...
    if _lib is None:
        raise ImportError(
            "libaihara_map.so not built. Run: mojo build --emit shared-lib "
            f"-o {_LIB_PATH} {_LIB_PATH.parent / 'aihara_map.mojo'}"
        )
...
__all__ = ["_HAS_MOJO_AIHARA_MAP", "simulate_aihara_map"]
