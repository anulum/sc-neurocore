...
# SC-NeuroCore — Mojo-backed adaptive-threshold exact-relaxation batch

"""Typed ctypes facade for the Mojo adaptive-threshold shared library."""
...
_LIB_PATH = _ACCEL_ROOT / "mojo" / "adaptive_threshold_if" / "libadaptive_threshold_if.so"
_MAX_NATIVE_STEPS = (1 << 31) - 1


def _configure_library(library: ctypes.CDLL) -> ctypes.CDLL:
    function = library.adaptive_threshold_if_simulate_c
    function.argtypes = [ctypes.c_int32, *([ctypes.c_double] * 9), *([ctypes.c_void_p] * 7)]
...
_lib, _HAS_MOJO_ADAPTIVE_THRESHOLD_IF = _load_library()
...
def simulate_adaptive_threshold_if(
    ...
    """Run the Mojo implementation of the complete exact-relaxation recurrence."""
    ...
    if _lib is None:
        raise ImportError(
            ...
            "adaptive_threshold_if.mojo --target-cpu x86-64-v3"
        )
...
__all__ = ["_HAS_MOJO_ADAPTIVE_THRESHOLD_IF", "simulate_adaptive_threshold_if"]
