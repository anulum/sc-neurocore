...
# SC-NeuroCore — Mojo-backed ADC-to-spike encoder (ctypes dispatch)

"""Python entry point for the Mojo-compiled ADC-to-spike window encoder.

Build::

    cd src/sc_neurocore/accel/mojo/adc_to_spike
    mojo build --emit shared-lib -o libadc_to_spike.so adc_to_spike.mojo

The ``.so`` is platform-specific and gitignored; the ``.mojo`` source is tracked.
``_HAS_MOJO_ADC_TO_SPIKE`` is ``True`` iff the shared library is present at import
...
try:
    ...
    _lib.adc_to_spike_windows_c.argtypes = [
        ...
        ctypes.c_long,
        ctypes.c_long,
        ctypes.c_long,  # n_windows, adc_width, q_int, q_frac
        ctypes.c_long,
        ctypes.c_long,
        ctypes.c_long,  # decimation, signed_input, threshold_q
        ctypes.c_void_p,  # samples
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,  # window_values, spike_counts, polarities
    ]
    _lib.adc_to_spike_windows_c.restype = ctypes.c_long
    _HAS_MOJO_ADC_TO_SPIKE = True
except OSError:
    _lib = None
    _HAS_MOJO_ADC_TO_SPIKE = False
...
def adc_to_spike_windows(
    ...
    """Mojo-accelerated per-window ADC-to-spike encode.
    ...
    if _lib is None:
        raise ImportError(
            ...
            f"mojo build --emit shared-lib -o {_LIB_PATH.name} adc_to_spike.mojo"
        )
    ...
    rc = _lib.adc_to_spike_windows_c(
        ctypes.c_long(n_windows),
        ctypes.c_long(int(adc_width)),
        ctypes.c_long(int(q_int)),
        ctypes.c_long(int(q_frac)),
        ctypes.c_long(int(decimation)),
        ctypes.c_long(int(signed_input)),
        ctypes.c_long(int(threshold_q)),
    ...
    if rc != 0:
        raise ValueError(f"Mojo adc_to_spike_windows_c returned non-zero: {rc}")
