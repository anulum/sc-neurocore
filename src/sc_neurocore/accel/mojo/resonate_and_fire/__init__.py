...
# SC-NeuroCore — Mojo-backed resonate-and-fire exact-flow batch

"""Typed ctypes facade for the Mojo resonate-and-fire shared library."""
...
_LIB_PATH = _ACCEL_ROOT / "mojo" / "resonate_and_fire" / "libresonate_and_fire.so"
...
_lib, _HAS_MOJO_RESONATE_AND_FIRE = _load_library()
...
def simulate_resonate_and_fire(
    ...
    if _lib is None:
        raise ImportError(
            "libresonate_and_fire.so not built. Run: mojo build --emit shared-lib "
            f"-o {_LIB_PATH} {_LIB_PATH.parent / 'resonate_and_fire.mojo'} "
...
__all__ = ["_HAS_MOJO_RESONATE_AND_FIRE", "simulate_resonate_and_fire"]
