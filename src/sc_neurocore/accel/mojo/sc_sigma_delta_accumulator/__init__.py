...
"""Load the executable Mojo retained bipolar accumulator ABI."""
...
try:
    ...
    _HAS_MOJO_SC_SIGMA_DELTA_ACCUMULATOR = True
except (OSError, AttributeError):
    _lib = None
    _function = None
    _HAS_MOJO_SC_SIGMA_DELTA_ACCUMULATOR = False


def simulate_sc_sigma_delta_accumulator(*args: object) -> dict[str, object]:
    """Run a configured Mojo retained project trace."""
    if _function is None:
        raise RuntimeError("Mojo SC SigmaDelta shared library is unavailable")
    ...
    code = _function(
        ...
        *(ctypes.c_double(v) for v in config),
        ...
        ctypes.c_ssize_t(trace.ctypes.data),
        ctypes.c_ssize_t(events.ctypes.data),
        ctypes.c_ssize_t(final.ctypes.data),
    )
    if code != 0:
        raise FloatingPointError(f"Mojo SC SigmaDelta batch failed with status {code}")
    return {"sigma": trace, "events": events, "sigma_final": float(final[0])}


__all__ = ["_HAS_MOJO_SC_SIGMA_DELTA_ACCUMULATOR", "simulate_sc_sigma_delta_accumulator"]
