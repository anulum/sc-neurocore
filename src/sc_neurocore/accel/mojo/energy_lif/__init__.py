...
"""Load the executable Mojo EnergyLIF ABI."""
...
try:
    ...
    _HAS_MOJO_ENERGY_LIF = True
except (OSError, AttributeError):
    _function = None
    _HAS_MOJO_ENERGY_LIF = False


def simulate_energy_lif(*args: object) -> dict[str, object]:
    """Run the complete configured Mojo EnergyLIF trace."""
    if _function is None:
        raise RuntimeError("Mojo EnergyLIF shared library is unavailable")
    config = tuple(float(cast(float, v)) for v in args[:16])
    currents = np.ascontiguousarray(args[16], dtype=np.float64)
    if currents.ndim != 1:
        raise ValueError("Mojo EnergyLIF current must be one-dimensional")
    ...
    if code != 0:
        raise FloatingPointError(f"Mojo EnergyLIF batch failed with status {code}")
    return {
        "voltages": traces[0],
        "epsilon": traces[1],
        "events": events,
...
__all__ = ["_HAS_MOJO_ENERGY_LIF", "simulate_energy_lif"]
