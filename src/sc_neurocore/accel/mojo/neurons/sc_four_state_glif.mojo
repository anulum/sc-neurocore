...
# SC-NeuroCore — Mojo retained four-state GLIF recurrence
#
# Build:
#   mojo build --emit shared-lib -o libsc_four_state_glif.so sc_four_state_glif.mojo
#
...
@export
def sc_four_state_glif_simulate_c(
    v0: Float64,
    theta0: Float64,
    theta_inf: Float64,
    i_asc1_0: Float64,
    ...
    if (
        ...
        or not isfinite(v_rest)
        or not isfinite(v_reset)
        or not isfinite(tau_m)
        or not isfinite(tau_theta)
        or not isfinite(tau_asc1)
        or not isfinite(tau_asc2)
        or not isfinite(a_theta)
        or not isfinite(delta_theta)
        or not isfinite(r_asc1)
        or not isfinite(r_asc2)
    ...
    for t in range(n_steps):
        ...
        if not isfinite(v) or not isfinite(theta) or not isfinite(a1) or not isfinite(a2):
            return -1
