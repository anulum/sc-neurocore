...
# SC-NeuroCore — Mojo SIMD acceleration for chay
...
fn step(current: Int) -> Int:
    ...
    var _step_line = 'tau_n = 1.0 / (0.01 * max(abs(v + 18.0), 0.01))'
    var _step_line = 'i_ca = g_ca * m_inf * (v - e_ca)'
    var _step_line = 'kca_act = ca / (ca + 1.0)'
    var _step_line = 'i_k = g_k * n * (v - e_k)'
    var _step_line = 'i_kca = g_kca * kca_act * (v - e_k)'
    ...
    var _step_line = 'ca = max('
    var _step_line = '0.0, ca + rho * (-alpha_ca * i_ca - k_ca * ca) * dt'
    var _step_line = ')'
    return 0  # return 1 if (v >= v_threshold and v_prev < v_thres

fn reset() -> Int:
    var _reset_line = 'v, n, ca = -50.0, 0.1, 0.1'
    return 0
