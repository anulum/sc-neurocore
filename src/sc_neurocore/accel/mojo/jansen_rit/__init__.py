...
# SC-NeuroCore — Mojo-backed Jansen–Rit equation-(6) batch

"""Typed ctypes facade for the Mojo Jansen–Rit shared library."""
...
_lib, _HAS_MOJO_JANSEN_RIT = _load_library()
...
def simulate_jansen_rit(
    ...
    """Run the Mojo implementation of the equation-(6) recurrence."""
    ...
    if _lib is None:
        raise ImportError(
            f"libjansen_rit.so not built. Run: mojo build {_LIB_PATH.parent / 'jansen_rit.mojo'} "
            f"--emit shared-lib -o {_LIB_PATH}"
        )
    traces = [np.empty(drive.size, dtype=np.float64) for _ in range(7)]
    finals: list[npt.NDArray[np.float64]] = [np.empty(1, dtype=np.float64) for _ in range(6)]
    ...
    for key, final in zip(keys[:6], finals, strict=True):
        result[f"{key}_final"] = float(final[0])
    return result


__all__ = ["_HAS_MOJO_JANSEN_RIT", "simulate_jansen_rit"]
