...
"""Load the native Go SC normalized EnergyLIF batch."""
...
try:
    ...
    _HAS_GO_SC_NORMALIZED_ENERGY_LIF = True
except (OSError, AttributeError):
    _function = None
    _HAS_GO_SC_NORMALIZED_ENERGY_LIF = False


def simulate_sc_normalized_energy_lif(*args: object) -> dict[str, object]:
    """Run the complete configured Go SC normalized EnergyLIF trace."""
    if _function is None:
        raise RuntimeError("Go SC normalized EnergyLIF shared library is unavailable")
    config = tuple(float(cast(float, v)) for v in args[:11])
    currents = np.ascontiguousarray(args[11], dtype=np.float64)
    if currents.ndim != 1:
        raise ValueError("Go SC normalized EnergyLIF current must be one-dimensional")
    ...
    if code != 0:
        raise FloatingPointError(f"Go SC normalized EnergyLIF batch failed with status {code}")
    return {
        "voltages": traces[0],
        "epsilon": traces[1],
        "events": events,
...
__all__ = ["_HAS_GO_SC_NORMALIZED_ENERGY_LIF", "simulate_sc_normalized_energy_lif"]
