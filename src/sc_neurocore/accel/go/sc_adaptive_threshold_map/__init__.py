...
# SC-NeuroCore — Go-backed project adaptive-threshold-map batch

"""Typed ctypes facade for the Go SC adaptive-map shared library."""
...
_LIB_PATH = _ACCEL_ROOT / "go" / "sc_adaptive_threshold_map" / "libsc_adaptive_threshold_map.so"
...
try:
    ...
    _lib = None
_HAS_GO_SC_ADAPTIVE_THRESHOLD_MAP = _lib is not None
...
def simulate_sc_adaptive_threshold_map(
    ...
    """Run the checked Go implementation of the project recurrence."""
    ...
    if _lib is None:
        raise ImportError(
            f"libsc_adaptive_threshold_map.so not built. Run: cd {_LIB_PATH.parent} && "
...
__all__ = [
    "_HAS_GO_SC_ADAPTIVE_THRESHOLD_MAP",
