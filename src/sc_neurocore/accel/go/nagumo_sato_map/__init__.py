...
# SC-NeuroCore — Go-backed Nagumo–Sato batch

"""Typed ctypes facade for the Go Nagumo–Sato shared library."""
...
_LIB_PATH = _ACCEL_ROOT / "go" / "nagumo_sato_map" / "libnagumo_sato_map.so"
...
try:
    ...
    _lib = None
_HAS_GO_NAGUMO_SATO_MAP = _lib is not None
...
def simulate_nagumo_sato_map(
    ...
    """Run the checked Go implementation of the source recurrence."""
    ...
    if _lib is None:
        raise ImportError(
            f"libnagumo_sato_map.so not built. Run: cd {_LIB_PATH.parent} && "
...
__all__ = ["_HAS_GO_NAGUMO_SATO_MAP", "simulate_nagumo_sato_map"]
