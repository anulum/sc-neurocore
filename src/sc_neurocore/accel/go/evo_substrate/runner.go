...
func applyPoint(cfg *MutationConfig, g *Genome, rng *rand.Rand) {
	v := g.toVector()
	for i := 0; i < genomeDim; i++ {
		if rng.Float64() < cfg.PointRate {
			noise := rng.NormFloat64() * cfg.PointSigma
...
func applyStructural(cfg *MutationConfig, g *Genome, rng *rand.Rand) {
	deltas := []int32{-2, -1, 1, 2}
	delta := deltas[rng.Intn(4)]
	g.Topology.NumNeurons = clampInt32(g.Topology.NumNeurons+delta, cfg.MinNeurons, cfg.MaxNeurons)
	g.Topology.Connectivity = clampF64(g.Topology.Connectivity+rng.NormFloat64()*0.05, 0.01, 1.0)
}
...
func newMutationEngine(cfg MutationConfig, seed int64) *MutationEngine {
	return &MutationEngine{Config: cfg, RNG: rand.New(rand.NewSource(seed))}
}
...
func newCrossoverEngine(seed int64) *CrossoverEngine {
	return &CrossoverEngine{RNG: rand.New(rand.NewSource(seed))}
}
...
func (d *ExtinctionDetector) apply(pop []Organism, rng *rand.Rand) int {
...
func (t *TournamentSelector) selectOne(pop []Organism, rng *rand.Rand) *Organism {
	if len(pop) == 0 {
		return nil
	}
	k := t.TournamentSize
...
func evolveRun(cfg *EvolveConfig) EvolveResult {
	master := rand.New(rand.NewSource(int64(cfg.Seed)))
	mutator := newMutationEngine(cfg.Mutation, master.Int63())
	xover := newCrossoverEngine(master.Int63())
	...
	for gen := int32(1); gen <= int32(cfg.NGenerations); gen++ {
		...
		sort.Slice(aliveIndices, func(i, j int) bool {
		...
		for len(population) < cfg.PopSize && len(survivors) > 0 {
			...
			if partner != nil && mutator.RNG.Float64() < cfg.CrossoverProb {
