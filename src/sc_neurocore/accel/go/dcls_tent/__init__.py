...
# SC-NeuroCore — Go-backed DCLS-max Q8.8 tent kernel (ctypes dispatch)

"""Python entry point for the Go-compiled DCLS-max tent kernel.

The Go source ``dcls_tent.go`` must be pre-built into ``libdcls_tent.so`` via::

    cd src/sc_neurocore/accel/go/dcls_tent
    PATH=/usr/local/go/bin:$PATH GOTOOLCHAIN=local \\
        go build -buildmode=c-shared -o libdcls_tent.so dcls_tent.go

The ``.so`` is platform-specific and gitignored; the ``.go`` source and the
generated ``.h`` header are tracked. ``_HAS_GO_DCLS`` is ``True`` iff the shared
library exists next to this module at import time.
"""
...
try:
    ...
    _lib.dcls_max_forward_batch_q88_c.argtypes = [
        ctypes.c_int,
        ctypes.c_int,  # n_channels, n_taps
        ctypes.c_void_p,
    ...
    _lib.dcls_max_forward_batch_q88_c.restype = ctypes.c_int
    _HAS_GO_DCLS = True
except OSError:
    _lib = None
    _HAS_GO_DCLS = False
...
def dcls_max_forward_batch(
    ...
    """Go-accelerated batched DCLS-max tent kernel.
    ...
    if _lib is None:
        raise ImportError(
            ...
            f"go build -buildmode=c-shared -o {_LIB_PATH.name} dcls_tent.go"
        )
    ...
    if rc != 0:
        raise ValueError(f"dcls_max_forward_batch_q88_c returned non-zero: {rc}")
