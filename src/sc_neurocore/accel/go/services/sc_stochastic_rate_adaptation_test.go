...
// SC-NeuroCore — Go SC stochastic rate-adaptation tests
...
func scStochasticRateAdaptationReference(s SCStochasticRateAdaptationNeuronState, current float64) (float64, float64) {
...
func TestSCStochasticRateAdaptationRK4CandidateMatchesReference(t *testing.T) {
	s := NewSCStochasticRateAdaptationNeuron()
	s.A = 0.35
	s.Dt = 0.25
	s.Rng = 0.5
	expectedA, expectedP := scStochasticRateAdaptationReference(*s, 12.5)
...
func TestSCStochasticRateAdaptationStepCommitsRK4Candidate(t *testing.T) {
	s := NewSCStochasticRateAdaptationNeuron()
	s.A = 0.25
	s.Dt = 0.5
	s.Rng = 0.5
	expectedA, _ := scStochasticRateAdaptationReference(*s, 15.0)
...
func TestSCStochasticRateAdaptationInvalidRuntimeInputPreservesState(t *testing.T) {
	s := NewSCStochasticRateAdaptationNeuron()
	s.A = 0.5
...
func TestSCStochasticRateAdaptationInvalidCandidatePreservesState(t *testing.T) {
	s := NewSCStochasticRateAdaptationNeuron()
	s.A = 0.5
	s.FMax = 1.0e-306
	s.DeltaA = 1.0e308
...
func BenchmarkSCStochasticRateAdaptationStep(b *testing.B) {
	s := NewSCStochasticRateAdaptationNeuron()
	s.Rng = 0.5
	for i := 0; i < b.N; i++ {
		s.Step(12.5)
