...
"""Load the native Go MAT(1) batch without fallback substitution."""
...
try:
    ...
    _HAS_GO_NON_RESETTING_LIF = True
except (OSError, AttributeError):
    _function = None
    _HAS_GO_NON_RESETTING_LIF = False


def simulate_non_resetting_lif(*args: object) -> dict[str, object]:
    """Run the complete configured Go MAT(1) trace."""
    if _function is None:
        raise RuntimeError("Go MAT(1) shared library is unavailable")
    config = tuple(float(cast(float | int, value)) for value in args[:10])
    currents = np.ascontiguousarray(args[10], dtype=np.float64)
    if currents.ndim != 1:
        raise ValueError("Go MAT(1) current must be one-dimensional")
    ...
    if code != 0:
        raise FloatingPointError(f"Go MAT(1) batch failed with status {code}")
    return {
        "voltages": traces[0],
        "theta": traces[1],
        "refractory": traces[2],
...
__all__ = ["_HAS_GO_NON_RESETTING_LIF", "simulate_non_resetting_lif"]
