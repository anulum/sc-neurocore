...
# SC-NeuroCore — Go-backed RK4 neuron integrators (ctypes dispatch)

"""Python entry point for the Go-compiled RK4 neuron simulators.
...
The shared object is platform-specific and gitignored; the Go source is
tracked. The returned schema mirrors the Rust and Julia RK4 dispatchers.
"""
...
try:
    ...
    _HAS_GO_RK4_NEURONS = True
except OSError:
    _lib = None
    _HAS_GO_RK4_NEURONS = False
...
def _missing_library_error() -> ImportError:
    return ImportError(
        ...
        f"go build -buildmode=c-shared -o {_LIB_PATH.name} rk4_neurons.go"
    )
...
def simulate_rk4_neuron(
    ...
    """Go-backed RK4 batch simulator for priority neuron models."""
    if _lib is None:
