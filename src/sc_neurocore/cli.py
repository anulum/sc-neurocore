...
def main() -> int:
    ...
    parser.add_argument("--target", default="ice40", choices=["ice40", "ecp5", "artix7", "zynq"],
                        help="FPGA target for deploy (default: ice40)")
    parser.add_argument("--output", "-o", default="build", help="Output directory for deploy")
    parser.add_argument("--dt", type=float, default=0.001, help="Simulation timestep for NIR import")
    ...
    if args.command == "deploy":
        if not args.model:
            print("Error: deploy requires a model file. Usage: sc-neurocore deploy model.nir --target artix7")
            return 1
...
def _cmd_deploy(model_path: str, target: str, output_dir: str, dt: float, bitstream_length: int) -> int:
    """Deploy a model to FPGA: NIR/PyTorch → quantize → Verilog → project."""
    import os

    os.makedirs(output_dir, exist_ok=True)
    print(f"SC-NeuroCore Deploy")
