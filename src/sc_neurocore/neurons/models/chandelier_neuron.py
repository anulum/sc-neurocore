...
class ChandelierNeuron:
    ...
    def step(self, current: float = 0.0) -> int:
        ...
        for _ in range(n_sub):
            ...
            d_inf = 1.0 / (1.0 + math.exp(-(self.v + 50.0) / 10.0))
            self.d += (d_inf - self.d) / 150.0 * self.dt
            ...
            i_kv1 = self.g_kv1 * self.d**4 * (self.v - self.e_k)
            i_kv3 = self.g_kv3 * self.p * (self.v - self.e_k)
