...
class SCWBNMDAMagnesiumBlockNeuron:
    ...
    def _validate_configuration(self) -> None:
        ...
        if not all(math.isfinite(value) for value in values):
            raise ValueError("SC NMDA state and parameters must be finite")
    ...
    def step(self, current: float = 0.0) -> int:
        ...
        fired = 0
        v_candidate, h_candidate, n_candidate = self.v, self.h, self.n
        drive = inp / (inp + 5.0) if inp > 0.0 else 0.0
        tau = self.tau_rise if drive > self.s_nmda else self.tau_decay
        s_candidate = max(0.0, min(1.0, self.s_nmda + self.dt * (drive - self.s_nmda) / tau))
        for _ in range(self._sub_steps):
            ...
            v_candidate += sub_dt * (-i_na - i_k - i_nmda - i_l + inp) / self.c_m
            if not all(math.isfinite(value) for value in (v_candidate, h_candidate, n_candidate)):
                raise ValueError("SC NMDA candidate state became non-finite")
