...
def _load_rust_simulate() -> _RustSimulate:
    ...
    return engine.py_mckean_simulate  # type: ignore[no-any-return]
...
def _ensure_julia_loaded() -> bool:
    ...
    if importlib_util.find_spec("juliacall") is None:
        return False
    jl_path = _os.path.abspath(_os.path.join(_ACCEL_ROOT, "julia", "neurons", "mckean.jl"))
    if not _os.path.isfile(jl_path):
        return False
    juliacall = _importlib.import_module("juliacall")
    jl = juliacall.Main
    jl.include(jl_path)
    _julia_module = jl.McKeanAccel
    _HAS_JULIA = True
    return True
...
def _ensure_go_loaded() -> bool:
    ...
    so_path = _os.path.abspath(
        _os.path.join(_ACCEL_ROOT, "go", "neurons", "mckean", "libmckean.so")
    )
    ...
    try:
        ...
        return False
    fn = getattr(lib, "mckean_simulate_c", None)
    if fn is None:
        return False
...
def _ensure_mojo_loaded() -> bool:
    ...
    so_path = _os.path.abspath(_os.path.join(_ACCEL_ROOT, "mojo", "neurons", "libmckean.so"))
    if not _os.path.isfile(so_path):
        return False
    try:
        lib = ctypes.CDLL(so_path)
...
class McKeanNeuron:
    ...
    def simulate(
        ...
        if backend == "rust" and not _HAS_RUST:
            raise RuntimeError("Rust McKean backend requested but the engine wheel lacks it.")
        if backend == "julia" and not _ensure_julia_loaded():
            raise RuntimeError("Julia McKean backend requested but juliacall/.jl is unavailable.")
        if backend == "go" and not _ensure_go_loaded():
            raise RuntimeError(
                "Go McKean backend requested but libmckean.so is not built; run "
                "`cd src/sc_neurocore/accel/go/neurons/mckean && go build "
                "-buildmode=c-shared -o libmckean.so mckean.go`."
            )
        if backend == "mojo" and not _ensure_mojo_loaded():
            raise RuntimeError(
                "Mojo McKean backend requested but libmckean.so is not built; run "
                "`cd src/sc_neurocore/accel/mojo/neurons && mojo build --emit shared-lib "
                "-o libmckean.so mckean.mojo`."
            )
    ...
    def _simulate_go(
        ...
        spikes = _go_lib.mckean_simulate_c(
    ...
    def _simulate_mojo(
        ...
        trace = np.zeros(n_steps + 2, dtype=np.float64, order="C")
        spikes = _mojo_lib.mckean_simulate_c(
            float(self.v),
            float(self.w),
            float(self.a),
            float(self.epsilon),
