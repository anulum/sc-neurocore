...
# SC-NeuroCore — retained four-state GLIF project recurrence

"""Historical four-state RK4 recurrence retained under an explicit SC identity.

This project model is not the five-state GLIF5 system of Teeter et al. It is
kept count-neutral so existing users retain the original dynamics without a
false whole-model publication attribution.
"""
...
# accelerates `simulate`. The SC four-state GLIF right-hand side is purely linear —
...
class _RustSimulate(Protocol):
    """Callable batch surface exported by the production Rust engine."""

    def __call__(self, *args: float | int) -> tuple[Any, int, float, float, float, float]: ...


class _JuliaResult(Protocol):
    """Named result returned by the Julia retained recurrence."""
...
def _load_rust_simulate() -> _RustSimulate:
    ...
    return cast(_RustSimulate, engine.py_sc_four_state_glif_simulate)
...
def _ensure_julia_loaded() -> bool:
    ...
    jl_path = _os.path.abspath(
        _os.path.join(_ACCEL_ROOT, "julia", "neurons", "sc_four_state_glif.jl")
    )
    ...
    jl = juliacall.Main
    jl.include(jl_path)
    _julia_module = cast(_JuliaAccel, jl.SCFourStateGLIFAccel)
    _HAS_JULIA = True
    return True
...
def _ensure_go_loaded() -> bool:
    ...
    so_path = _os.path.abspath(
        _os.path.join(
            _ACCEL_ROOT, "go", "neurons", "sc_four_state_glif", "libsc_four_state_glif.so"
        )
    )
    ...
    try:
        ...
        return False
    fn = getattr(lib, "sc_four_state_glif_simulate_c", None)
    if fn is None:
        return False
...
def _ensure_mojo_loaded() -> bool:
    ...
    if _mojo_lib is not None:
        return True
    so_path = _os.path.abspath(
        _os.path.join(_ACCEL_ROOT, "mojo", "neurons", "libsc_four_state_glif.so")
    )
    if not _os.path.isfile(so_path):
        return False
    try:
...
class SCFourStateGLIFNeuron:
    ...
    def simulate(
        ...
        step. The SC four-state GLIF right-hand side is purely linear, so the Rust, Julia
        ...
        if backend == "rust" and not _HAS_RUST:
            raise RuntimeError(
                "Rust SC four-state GLIF backend requested but the engine wheel lacks it."
            )
        if backend == "julia" and not _ensure_julia_loaded():
            raise RuntimeError(
                "Julia SC four-state GLIF backend requested but juliacall/.jl is unavailable."
            )
        if backend == "go" and not _ensure_go_loaded():
            raise RuntimeError(
                "Go SC four-state GLIF backend requested but its library is not built; run "
                "`cd src/sc_neurocore/accel/go/neurons/sc_four_state_glif && go build "
                "-buildmode=c-shared -o libsc_four_state_glif.so sc_four_state_glif.go`."
            )
        if backend == "mojo" and not _ensure_mojo_loaded():
            raise RuntimeError(
                "Mojo SC four-state GLIF backend requested but its library is not built; run "
                "`cd src/sc_neurocore/accel/mojo/neurons && mojo build --emit shared-lib "
                "-o libsc_four_state_glif.so sc_four_state_glif.mojo`."
            )
    ...
    def _simulate_rust(
        ...
        if _rust_simulate is None:
            raise RuntimeError("Rust SC four-state GLIF backend is unavailable")
    ...
    def _simulate_julia(
        ...
        if _julia_module is None:
            raise RuntimeError("Julia SC four-state GLIF backend is unavailable")
    ...
    def _simulate_go(
        ...
        if _go_lib is None:
            raise RuntimeError("Go SC four-state GLIF backend is unavailable")

        trace = np.zeros(n_steps + 4, dtype=np.float64, order="C")
        spikes = _go_lib.sc_four_state_glif_simulate_c(
        ...
        if spikes < 0:
            raise FloatingPointError("Go SC four-state GLIF backend rejected an invalid state")
    ...
    def _simulate_mojo(
        ...
        if _mojo_lib is None:
            raise RuntimeError("Mojo SC four-state GLIF backend is unavailable")
        trace = np.zeros(n_steps + 4, dtype=np.float64, order="C")
        spikes = _mojo_lib.sc_four_state_glif_simulate_c(
            *(float(x) for x in self._scalar_args(n_steps, current)),
            int(n_steps),
            float(current),
        ...
        if spikes < 0:
            raise FloatingPointError("Mojo SC four-state GLIF backend rejected an invalid state")
        state = self._final_state(trace, n_steps)
        return np.ascontiguousarray(trace[:n_steps]), int(spikes), state
