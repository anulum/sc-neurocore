...
_NON_NEGATIVE_PARAMS = ("g_na", "g_k", "g_m", "g_t", "g_h", "g_l")
_N_SUBSTEPS = 4
...
class SSTNeuron:
    ...
    def __post_init__(self) -> None:
        if self.integrator not in {"rk4", "baseline_euler"}:
            raise ValueError(f"Unsupported integrator for SSTNeuron: {self.integrator}")
    ...
    def _derivatives(
        ...
        ----------
        v, m, h, n, p, s, r:
            Membrane potential and the six gating variables.
        current:
        ...
        -------
        tuple of float
            The time derivatives in ``(V, m, h, n, p, s, r)`` order.
        """
        if not all(math.isfinite(value) for value in (v, m, h, n, p, s, r, current)):
            raise FloatingPointError("SST derivative input became non-finite")
        dv_vt = v - _VT
        ...
        tau_r = 100.0 + 500.0 / (math.exp(-(v + 70.0) / 20.0) + math.exp((v + 70.0) / 20.0))
        dr = (r_inf - r) / tau_r
        ...
        dv = (-i_na - i_k - i_m - i_t - i_h - i_l + current) / self.c_m
        if not all(math.isfinite(value) for value in (dv, dm, dh, dn, dp, ds, dr)):
            raise FloatingPointError("SST derivative became non-finite")
    ...
    def _rk4_substep(
        ...
        ----------
        state:
            The ``(V, m, h, n, p, s, r)`` state at the start of the sub-step.
        current:
        ...
        -------
        tuple of float
            The advanced ``(V, m, h, n, p, s, r)`` candidate after one ``dt`` sub-step.
        """
        dt = self.dt
    ...
    def _validate_candidate(
        ...
        ----------
        candidate:
            The proposed ``(V, m, h, n, p, s, r)`` state from a sub-step.
        ...
        for name, value in zip(_STATE_NAMES, candidate):
            if not math.isfinite(value):
                raise FloatingPointError(f"SST {name} candidate became non-finite")
        return candidate
    ...
    def step(self, current: float = 0.0) -> int:
        ...
        self.v, self.m, self.h, self.n, self.p, self.s, self.r = state
