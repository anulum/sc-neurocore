...
def differentiate(expr: str, wrt: str) -> str:
    ...
    derivative = sympy.diff(sympy_expr, sympy.Symbol(wrt))
...
class _Converter:
    ...
    def convert(self, node: ast.expr) -> sympy.Expr:
        ...
        if isinstance(node, ast.BinOp):
            ...
            if builder is None:
                return self._opaque(node)
            return builder(self.convert(node.left), self.convert(node.right))  # type: ignore[operator]
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            func = _SMOOTH_FUNCS.get(node.func.id)
            if func is not None:
                return func(*[self.convert(arg) for arg in node.args])  # type: ignore[operator]
        return self._opaque(node)
    ...
    def render(self, expr: sympy.Expr) -> str:
        ...
        return str(_GrammarPrinter(self._sources).doprint(expr))
