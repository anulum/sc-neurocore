...
class L8_HolonomicParameters:
    ...
    pulsar_omegas: jnp.ndarray = field(default_factory=lambda: jnp.array([1.6, 2.3, 0.8, 4.1, 1.1, 0.5, 3.2, 2.7, 1.9, 0.4, 5.5, 0.2]))
...
class L8_CosmicAdapter(BaseStochasticAdapter):
    ...
    @staticmethod
    @jax.jit
    def _cosmic_kernel(phases: jnp.ndarray, pulsar_omegas: jnp.ndarray, 
                      k_cosmic: float, dt: float) -> jnp.ndarray:
        """
    ...
    def decode(self, bitstreams: jnp.ndarray) -> Dict[str, float]:
        ...
        """
        return {
            "cosmic_alignment_r8": float(jnp.abs(jnp.mean(jnp.exp(1j * self.system_phases))))
        }
