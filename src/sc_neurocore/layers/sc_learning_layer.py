...
class SCLearningLayer:
    ...
    def __post_init__(self) -> None:
        ...
        self.input_encoders = [
            BitstreamEncoder(x_min=0, x_max=1, length=self.length, seed=self.base_seed + i if self.base_seed else None)
        ...
        for i in range(self.n_neurons):
            ...
            for j in range(self.n_inputs):
                ...
                neuron_syns.append(StochasticSTDPSynapse(
                    w_min=self.w_min, w_max=self.w_max, w=initial_w,
                    learning_rate=self.learning_rate, length=self.length, seed=syn_seed
                ))
    ...
    def run_epoch(self, input_values: Sequence[float]) -> np.ndarray:
        ...
        """
        # 1. Encode inputs
        input_bitstreams = [self.input_encoders[i].encode(input_values[i]) for i in range(self.n_inputs)]
