...
def _hierarchy_from_graph(
    neuron_graph: Any,
    streams: tuple[SCNIRStream, ...],
) -> tuple[SCNIRHierarchyInstance, ...]:
    """Materialise SC-NIR hierarchy metadata from inlined NeuronGraph provenance."""

    hierarchy = tuple(getattr(neuron_graph, "hierarchy", ()))
    if not hierarchy:
        return ()
    ...
    for instance in hierarchy:
        ...
        if not stream_ids:
            raise ValueError(
                f"Hierarchy instance {instance.instance_id!r} did not produce any SC-NIR streams"
            )
        ports: list[SCNIRHierarchyPort] = []
        signal_counts: dict[SCNIRSignalKind, int] = {}
        for stream_id in stream_ids:
            stream = stream_by_id[stream_id]
            signal_kind = stream.signal_kind
            port_index = signal_counts.get(signal_kind, 0)
            signal_counts[signal_kind] = port_index + 1
            ports.append(
                SCNIRHierarchyPort(
                    port_name=f"{_hierarchy_port_prefix(signal_kind)}_{port_index}",
                    direction="output",
...
def _hierarchy_stream_ids(neuron_graph: Any, instance: Any) -> tuple[str, ...]:
    """Return deterministic stream ids generated by one inlined hierarchy instance."""

    prefix = str(instance.node_name_prefix)
    stream_ids: list[str] = []
    seen: set[str] = set()
    ...
    for pop in neuron_graph.populations:
        ...
        if not name.startswith(prefix):
            continue
        signal_kind = _population_signal_kind(str(pop.neuron_type))
        stream_id = _population_stream_id(name, signal_kind=signal_kind)
    ...
    for conn in neuron_graph.connections:
        ...
        if not (src.startswith(prefix) or dst.startswith(prefix)):
            continue
        stream_id = _connection_stream_id(src, dst)
        if stream_id not in seen:
            stream_ids.append(stream_id)
            seen.add(stream_id)
...
def _hierarchy_port_prefix(signal_kind: SCNIRSignalKind) -> str:
    if signal_kind == "analogue_state":
        return "state"
    return signal_kind
