...
def waveform_width(waveform: np.ndarray, dt: float = 1.0 / 30000) -> float:
...
def waveform_amplitude(waveform: np.ndarray) -> float:
    """Peak-to-trough amplitude. Bartho et al. 2004."""
    return float(np.max(waveform) - np.min(waveform))


def waveform_repolarization_slope(waveform: np.ndarray, dt: float = 1.0 / 30000) -> float:
...
def waveform_recovery_slope(waveform: np.ndarray, dt: float = 1.0 / 30000) -> float:
...
def waveform_halfwidth(waveform: np.ndarray, dt: float = 1.0 / 30000) -> float:
...
def waveform_pt_ratio(waveform: np.ndarray) -> float:
