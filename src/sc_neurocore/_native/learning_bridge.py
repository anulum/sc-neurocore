...
def _load_native_library():
    ...
    if env_path:
        ...
        lib_name = "autonomous_learning.dll" if sys_name == "Windows" else \
                   "libautonomous_learning.dylib" if sys_name == "Darwin" else \
                   "libautonomous_learning.so"
        lib_path = _pl.Path(__file__).parent.resolve() / lib_name
        
    if not lib_path.exists():
        raise RuntimeError(f"Native library missing at {lib_path}. Ensure autonomous_learning "
                           "was compiled via rustc or define SC_NEUROCORE_LIB_PATH environment variable.")
    ...
    try:
        ...
        _lib.step_learner.argtypes = [_ct.c_void_p, _ct.c_bool, _ct.c_bool, _ct.c_float, _ct.c_float]
        _lib.step_learner.restype = None
        _lib.destroy_learner.argtypes = [_ct.c_void_p]
        _lib.destroy_learner.restype = None

        _lib.step_rule_batched.argtypes = [_ct.c_void_p, _ct.POINTER(_ct.c_bool), _ct.POINTER(_ct.c_bool), _ct.POINTER(_ct.c_float), _ct.c_size_t, _ct.c_float]
        _lib.step_rule_batched.restype = None
        _lib.step_learner_batched.argtypes = [_ct.c_void_p, _ct.POINTER(_ct.c_bool), _ct.POINTER(_ct.c_bool), _ct.POINTER(_ct.c_float), _ct.c_size_t, _ct.c_float]
        _lib.step_learner_batched.restype = None

        _lib.create_rule_layer.argtypes = [_ct.c_size_t, _ct.c_uint32, _ct.c_float, _ct.c_float, _ct.c_float]
        _lib.create_rule_layer.restype = _ct.c_void_p
        _lib.step_rule_layer.argtypes = [_ct.c_void_p, _ct.POINTER(_ct.c_bool), _ct.POINTER(_ct.c_bool), _ct.POINTER(_ct.c_float), _ct.c_float]
...
class RustPlasticityRule:
    ...
    def step_batched(self, pre_spikes, post_spikes, rewards, dt: float = 0.001) -> None:
        ...
        if len(pre_spikes) != len(post_spikes) or len(pre_spikes) != len(rewards):
            raise ValueError(f"Batched array lengths mismatch: {len(pre_spikes)}, {len(post_spikes)}, {len(rewards)}")
        ...
        _lib.step_rule_batched(self._ptr, pre_ptr, post_ptr, rew_ptr, _ct.c_size_t(count), _ct.c_float(dt))
...
class RustEligentLearner:
    ...
    def step_batched(self, fired_slice, pre_spikes, rewards, dt: float = 0.001) -> None:
        ...
        pre_ptr = pre_spikes.ctypes.data_as(_ct.POINTER(_ct.c_bool))
        rew_ptr = rewards.ctypes.data_as(_ct.POINTER(_ct.c_float))
        _lib.step_learner_batched(self._ptr, fired_ptr, pre_ptr, rew_ptr, _ct.c_size_t(count), _ct.c_float(dt))
...
class RustRuleLayer:
    ...
    def step_analog(self, pre_probs, post_probs, rewards, dt: float = 0.001, seed: int = None) -> None:
        ...
        _lib.step_rule_layer_analog(self._ptr, pre_ptr, post_ptr, rew_ptr, _ct.c_uint64(seed), _ct.c_float(dt))
    ...
    def save(self, path: str) -> bool:
        ...
        if not success:
            raise IOError(f"Failed to save biological traits to {safe_path}")
        return True
    ...
    def load(self, path: str) -> bool:
        ...
        if not success:
            raise IOError(f"Failed to load biological traits or format mismatched from {safe_path}")
        return True
...
try:
    ...
    class _BiologicalAutogradFactory(torch.autograd.Function):
        ...
        @staticmethod
        def forward(ctx, pre_spikes_f, post_spikes_f, rewards_f, weights, pre_trace, post_trace, eligibility, theta_m, act_avg, rule_params, rule_type, dt):
            ...
            if rule_type == RULE_STDP:
                ...
                n_weights = torch.where(mask, n_weights * (target_sum / n_act_avg), torch.zeros_like(n_weights))
                n_act_avg = torch.full_like(n_act_avg, target_sum)
            elif rule_type == RULE_BCM:
                x = pre_spikes_f
                y = post_spikes_f
                n_weights = torch.clamp(n_weights + p_a_plus * y * (y - n_theta_m) * x * dt, 0.0, 1.0)
            ...
            if ctx is not None:
                ctx.save_for_backward(pre_spikes_f, post_spikes_f, rewards_f, n_weights, n_pre_trace, n_post_trace, n_eligibility, n_theta_m, n_act_avg, rule_params)
            
            return n_weights, n_pre_trace, n_post_trace, n_eligibility, n_theta_m, n_act_avg

        @staticmethod
        def backward(ctx, gw, gp, gp2, ge, gt, ga):
            pre_spikes_f, post_spikes_f, rewards_f, n_weights, n_pre_trace, n_post_trace, n_eligibility, n_theta_m, n_act_avg, rule_params = ctx.saved_tensors
            p_a_plus, p_a_minus, p_b_tau1, p_b_tau2, p_tau_e = rule_params
    ...
    class TorchRuleLayer(nn.Module):
        ...
        def __init__(
            ...
            **kwargs
            ...
            self.register_buffer("theta_m", torch.full((count,), 1.0, dtype=torch.float32) if rule_type == RULE_ELIGENT else torch.full((count,), 0.5, dtype=torch.float32))
            self.register_buffer("act_avg", torch.full((count,), weight, dtype=torch.float32) if rule_type == RULE_ELIGENT else torch.zeros(count, dtype=torch.float32))
        ...
        def forward(self, pre_spikes, post_spikes, rewards=None, dt=1.0):
            ...
            if rewards is None:
                if self.rule_type in (RULE_ELIGENT, RULE_REWARD_STDP):
                    import warnings
                    warnings.warn(f"TorchRuleLayer rule_type {self.rule_type} expects 'rewards' gradient but received None. Proceeding with zeros.", UserWarning)
            ...
            if self.autograd:
                new_w, new_pre, new_post, new_e, new_tm, new_avg = _BiologicalAutogradFactory.apply(
                    pre_f, post_f, rew_f, self.weights, self.pre_trace, self.post_trace, 
                    self.eligibility, self.theta_m, self.act_avg, self.rule_params, self.rule_type, dt
                )
                ...
                with torch.no_grad():
                    n_w, n_pr, n_po, n_e, n_t, n_a = _BiologicalAutogradFactory.forward(
                        None, pre_f, post_f, rew_f, self.weights, self.pre_trace, self.post_trace,
                        self.eligibility, self.theta_m, self.act_avg, self.rule_params, self.rule_type, dt
                    )
