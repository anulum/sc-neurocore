...
Public names resolve on first access (PEP 562), so importing one platform
module, as a job worker, its lifetime guard or a storage service does, does not
load every Studio subsystem into an interpreter that must meet a start-up
deadline. A resolved name is cached and is the defining module's object.
"""
...
_EXPORT_MODULES: dict[str, tuple[str, ...]] = {
    ...
    "training_weight_loader": ("load_training_weight_state_dict",),
    "analysis_limits": (
        "DEFAULT_STUDIO_MAX_SYNC_ANALYSIS_SIMULATIONS",
        "DEFAULT_STUDIO_MAX_SYNC_ANALYSIS_STEPS_PER_SIMULATION",
        "DEFAULT_STUDIO_MAX_SYNC_ANALYSIS_TOTAL_STEPS",
...
def __getattr__(name: str) -> object:
    """Import the submodule that defines a public name and cache the name."""
    module = _EXPORT_SOURCES.get(name)
    if module is None:
...
def __dir__() -> list[str]:
    """List loaded attributes together with every lazily exported name."""
