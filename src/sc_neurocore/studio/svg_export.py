...
# SC-NeuroCore — SVG vector export for simulation traces
...
def traces_to_svg(
    ...
    for vi, name in enumerate(var_names):
        values = states[name]
        pts = " ".join(
            f"{to_x(time[i]):.1f},{to_y(values[i]):.1f}" for i in range(0, len(time), stride)
        )
        colour = COLORS[vi % len(COLORS)]
        lines.append(f'<polyline points="{pts}" fill="none" stroke="{colour}" stroke-width="1.5"/>')

    # Spike markers (up to 200)
    if spikes:
        for idx in spikes[:200]:
            t = time[idx] if idx < len(time) else idx * dt
            x = to_x(t)
            lines.append(
