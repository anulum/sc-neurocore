...
def _validate_lp_hp(lp_width: int, lp_frac: int, hp_width: int, hp_frac: int) -> None:
    ...
    if lp_width >= hp_width:
        raise ValueError(
            f"LP data_width ({lp_width}) must be strictly less than "
            f"HP data_width ({hp_width})"
        )
    if lp_frac < 1:
...
def compile_adaptive_precision(
    ...
    *,
    ...
    lines.append(
        f"// LP: {lp_q_label} ({lp_width}-bit), HP: {hp_q_label} ({hp_width}-bit)"
    )
    lines.append(
    ...
    lines.append(
        f"// use_hp asserts when |{primary_var}| exceeds "
        f"{threshold_up_pct*100:.0f}% of LP range"
    )
    lines.append(
    ...
    lines.append(
        f"localparam signed [{lp_width - 1}:0] "
        f"THRESH_UP = {lp_width}'sd{thresh_up};"
    )
    lines.append(
        f"localparam signed [{lp_width - 1}:0] "
        f"THRESH_DOWN = {lp_width}'sd{thresh_down};"
    )
    lines.append("")
    ...
    for var in state_vars:
        safe_var = sanitize_ident(var, context="state variable")
        init_val = int(
            round(neuron.initial_state.get(var, 0.0) * (1 << hp_frac))
        )
