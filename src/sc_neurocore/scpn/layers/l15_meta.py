...
class L15_MetaLayer:
    ...
    def step(
        ...
        actual, resonance_penalty = self._validate_step_inputs(dt, l14_input)
        self.time += dt

        error = abs(self.params.target_coherence - actual)
        self.ethical_dissonance = float(
            np.clip(error + self.params.integration_coupling * resonance_penalty, 0.0, 1.0)
        )
    ...
    def _validate_step_inputs(
        dt: float, l14_input: Optional[Dict[str, Any]]
    ) -> tuple[float, float]:
        ...
        return actual, resonance_penalty
