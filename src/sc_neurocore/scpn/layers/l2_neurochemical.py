...
class L2_NeurochemicalLayer:
    ...
    def __init__(self, params: L2_StochasticParameters = None):
        self.params = params or L2_StochasticParameters()

        # Receptor states: 0 = unbound, 1 = bound
        self.receptor_states = np.zeros(
            (self.params.n_neurotransmitter_types, self.params.n_receptors),
            dtype=np.float32
        )
    ...
    def step(self, dt: float,
             nt_release: Optional[np.ndarray] = None,
             l1_input: Optional[np.ndarray] = None) -> Dict[str, np.ndarray]:
        """
        ...
        if nt_release is not None:
            self.nt_concentrations = np.clip(
                self.nt_concentrations + nt_release * dt - self.params.reuptake_rate * dt,
                0.0, 1.0
            )
        ...
        for nt_idx in range(self.params.n_neurotransmitter_types):
            ...
            unbind_mask = (np.random.random(self.params.n_receptors) <
                          self.params.unbinding_rate * dt)

            # Update states
            self.receptor_states[nt_idx] = np.where(
                bind_mask & (self.receptor_states[nt_idx] < 0.5),
                1.0,
        ...
        self.second_messenger_levels = (
            0.9 * self.second_messenger_levels +
            0.1 * receptor_activity
        )
        ...
        rands = np.random.random((self.params.n_neurotransmitter_types,
                                  self.params.bitstream_length))
    ...
    def release_neurotransmitter(self, nt_type: int, amount: float):
