...
def track_energy(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if layer_obj and hasattr(layer_obj, 'n_neurons') and hasattr(layer_obj, 'n_inputs') and hasattr(layer_obj, 'length'):
            # Dense Layer Ops:
            ops = layer_obj.n_inputs * layer_obj.n_neurons * layer_obj.length
            profiler.total_ops_and += ops
            
            # Memory Read
            mem = (layer_obj.n_neurons * layer_obj.n_inputs * layer_obj.length) + (layer_obj.n_inputs * layer_obj.length)
