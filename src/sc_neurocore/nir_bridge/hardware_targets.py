...
_PROFILES: dict[str, NeuromorphicHardwareProfile] = {
    ...
    "akida": NeuromorphicHardwareProfile(
        target_id="akida",
        display_name="Akida",
        backend_status="capability_manifest",
        supported_nir_nodes=(
        ...
        sc_constraints=SCMappingConstraints(
            bitstream_lengths=(32, 64, 128, 256),
            stream_transport="event_rate_probability",
            precision_modes=("unipolar", "quantised_event_rate"),
            stochastic_sources=("deterministic_lfsr_seed",),
            back_annotation_channels=(
                "event_drop_rate",
                "activation_saturation_rate",
                "quantisation_error_rate",
                "routing_latency_cycles",
            ),
        ),
        notes=(
            "Manifest only; no Akida SDK invocation is performed.",
            "Delay and recurrent subgraphs require an explicit pre-lowering transform.",
        ),
    ),
