...
~~~~~~~~~~~~~~~~~~~~~
The compiler currently emits an explicit per-neuron direct interconnect for
all supported networks.  This is resource-heavier than AER, but preserves the
NIR weighted affine semantics.  AER generation is kept out of the default path
until weighted address-event fan-out and destination accumulation are emitted
and verified.
...
def _build_top_direct(
    ...
    * all fan-in terms and biases accumulate in a widened signed accumulator
    ...
    lines = [
        ...
        "        begin",
        "            if (x > {{(ACC_WIDTH - DATA_WIDTH){Q_MAX[DATA_WIDTH - 1]}}, Q_MAX})",
        "                sat_acc = Q_MAX;",
        "            else if (x < {{(ACC_WIDTH - DATA_WIDTH){Q_MIN[DATA_WIDTH - 1]}}, Q_MIN})",
        "                sat_acc = Q_MIN;",
        "            else",
...
def _build_top_aer(
    ...
    *,
    ...
    """Reject weighted event-bus RTL until exact routing is implemented.
    ...
    -------
    str
        Never returns; weighted event routing is intentionally fail-closed.
    """
    raise NotImplementedError(
        "Weighted event-bus network RTL is not implemented.  Use the exact "
        "direct interconnect path or add audited weighted event fan-out and "
        f"destination accumulation for {module_name!r} ({qgraph.total_neurons} "
        f"neurons, {data_width}-bit Q-format)."
    )
...
def compile_network_to_fpga(
    ...
    *,
    ...
    # Step 4: Top-level interconnect.  Direct per-neuron wiring is emitted for
    # correctness until weighted AER fan-out has an audited implementation.
    total_neurons = graph.total_neurons
    interconnect = "direct"
    if total_neurons > _AER_THRESHOLD:
        warnings.append(
            "AER interconnect is not emitted until weighted event routing is "
