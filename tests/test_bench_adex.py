...
# SC-NeuroCore — AdEx five-backend benchmark contract tests

"""Production-path tests for the controlled AdEx benchmark harness."""
...
from benchmarks import bench_adex as benchmark
...
def test_real_benchmark_writes_complete_parity_evidence(
    ...
    output = tmp_path / "adex.json"

    assert benchmark.main(["--json", str(output), "--allow-unpinned"]) == 0
    payload = json.loads(output.read_text(encoding="utf-8"))
    assert payload["schema_version"] == "sc-neurocore.polyglot-benchmark.v2"
    assert payload["kernel"] == "adex_baseline_euler_simulate"
    ...
    assert all(len(row["complete_packet_sha256"]) == 64 for row in payload["backends"].values())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.array([6.0e-12]), np.array([0], dtype=np.uint8), 4),
    ],
)
def test_parity_contract_failures_have_distinct_exit_codes(
    ...
    def measured(
        ...
        return (
            ...
            np.array([0.0]),
            compiled_events,
            int(compiled_events.sum()),
            (float(compiled_trace[-1]), 0.0),
        )
...
def test_backend_probes_report_each_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(adex, "_HAS_RUST", True)
