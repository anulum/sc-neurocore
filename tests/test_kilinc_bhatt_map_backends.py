...
# SC-NeuroCore — Executed Kilinc-Bhatt backend and custody parity

"""Execute every declared backend and bind public claims to live sources."""
...
_DRIVE = tuple(0.6 + 0.25 * math.sin(index * 0.17) for index in range(64))


class _RustKilincBhatt(Protocol):
...
def _python_trace() -> np.ndarray:
    ...
    for current in _DRIVE:
        ...
        rows.append((neuron.x, neuron.theta, event))
...
def test_production_rust_binding_matches_complete_python_state() -> None:
    ...
    rust = cast(_RustKilincBhatt, engine.KilincBhattMapNeuron())
    actual = []
    for current in _DRIVE:
        event = rust.step(current)
        state = rust.get_state()
        actual.append((state["x"], state["theta"], event))
    np.testing.assert_allclose(actual, _python_trace(), rtol=0.0, atol=2.0e-15)
...
def test_standalone_safety_rust_matches_complete_python_state(tmp_path: Path) -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/rust/safety/kilinc_bhatt_map_neuron.rs"
    literals = ", ".join(f"{current:.17e}_f64" for current in _DRIVE)
    program = tmp_path / "kilinc_bhatt_trace.rs"
    binary = tmp_path / "kilinc_bhatt_trace"
    program.write_text(
        f'''#[path = r#"{source}"#]
mod kilinc_bhatt;

use kilinc_bhatt::KilincBhattMapNeuron;

fn main() {{
    let mut state = KilincBhattMapNeuron::new();
    for current in [{literals}] {{
        let event = state.step(current).expect("finite configured drive");
        println!("{{:.17e}} {{:.17e}} {{}}", state.x, state.theta, event);
    }}
}}
''',
        encoding="utf-8",
    ...
    np.testing.assert_allclose(
        _parse_trace(completed.stdout), _python_trace(), rtol=0.0, atol=2e-15
    )
...
def test_go_backend_matches_complete_python_state(tmp_path: Path) -> None:
    ...
    program = tmp_path / "kilinc_bhatt_trace.go"
    program.write_text(
        f"""package main
...
func main() {{
    state := services.NewKilincBhattMapNeuron()
    for _, current := range []float64{{{literals}}} {{
        event, err := state.TryStep(current)
        if err != nil {{ panic(err) }}
        fmt.Printf("%.17e %.17e %d\\n", state.X, state.Theta, event)
    }}
}}
""",
        encoding="utf-8",
...
def test_julia_backend_matches_complete_python_state() -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/julia/neurons/kilinc_bhatt_map_neuron.jl"
    literals = ", ".join(f"{current:.17e}" for current in _DRIVE)
    program = f'''
include(raw"{source}")
using .KilincBhattMapNeuronAccel
state = KilincBhattMapNeuronState()
for current in [{literals}]
    event = step!(state, current)
    println(state.x, " ", state.theta, " ", event)
end
'''
    completed = subprocess.run(
...
def test_descriptor_and_public_page_report_only_proven_backends() -> None:
    descriptor = tomllib.loads(
        (_ROOT / "src/sc_neurocore/neurons/model_descriptors/KilincBhattMapNeuron.toml").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert "*Preprint.*" not in page
    assert "**Reference:** Kilinc" not in page
    assert "engine/src/neurons/kilinc_bhatt_map.rs" in page
    assert "Mojo" in page and "not implemented" in page
    assert "8.19 ms" not in page
