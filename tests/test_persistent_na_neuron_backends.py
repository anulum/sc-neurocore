...
# SC-NeuroCore — Executed PersistentNa backend and custody parity

"""Execute every declared PersistentNa backend and bind public claims."""
...
class _RustPersistentNa(Protocol):
...
def _python_trace() -> np.ndarray:
    ...
    for current in _DRIVE:
        ...
        rows.append((neuron.v, neuron.h, neuron.n, neuron.p, event))
...
def test_production_rust_binding_matches_complete_python_state() -> None:
    ...
    rust = cast(_RustPersistentNa, engine.PersistentNaNeuron())
    actual = []
    for current in _DRIVE:
        event = rust.step(current)
        state = rust.get_state()
        actual.append((state["v"], state["h"], state["n"], state["p"], event))
...
def test_standalone_safety_rust_matches_complete_python_state(tmp_path: Path) -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/rust/safety/persistent_na_neuron.rs"
    literals = ", ".join(f"{current:.17e}_f64" for current in _DRIVE)
    program = tmp_path / "persistent_na_trace.rs"
    binary = tmp_path / "persistent_na_trace"
    program.write_text(
        f'''#[path = r#"{source}"#]
mod persistent_na;

use persistent_na::PersistentNaNeuron;

fn main() {{
    let mut state = PersistentNaNeuron::new();
    for current in [{literals}] {{
        let event = state.step(current).expect("finite configured drive");
        println!("{{:.17e}} {{:.17e}} {{:.17e}} {{:.17e}} {{}}", state.v, state.h, state.n, state.p, event);
    }}
}}
''',
        encoding="utf-8",
...
def test_go_backend_matches_complete_python_state(tmp_path: Path) -> None:
    ...
    program = tmp_path / "persistent_na_trace.go"
    program.write_text(
        f"""package main
...
def test_julia_backend_matches_complete_python_state() -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/julia/neurons/persistent_na_neuron.jl"
    literals = ", ".join(f"{current:.17e}" for current in _DRIVE)
    program = f'''
include(raw"{source}")
using .PersistentNaNeuronAccel
state = PersistentNaNeuronState()
for current in [{literals}]
    event = step!(state, current)
    println(state.v, " ", state.h, " ", state.n, " ", state.p, " ", event)
end
before = (state.v, state.h, state.n, state.p)
try
    step!(state, NaN)
...
(state.v, state.h, state.n, state.p) == before || error("invalid drive mutated state")
'''
    completed = subprocess.run(
...
def test_descriptor_and_public_page_report_only_proven_backends() -> None:
    descriptor = tomllib.loads(
        (_ROOT / "src/sc_neurocore/neurons/model_descriptors/PersistentNaNeuron.toml").read_text(
            encoding="utf-8"
        )
    )
    ...
    page = (_ROOT / "docs/api/models/persistent_na_neuron.md").read_text(encoding="utf-8")
    assert "**Reference:** Crill" not in page
    assert "publication-exact" in page
    assert "engine/src/neurons/channels/persistent_na.rs" in page
