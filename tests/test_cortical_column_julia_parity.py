...
# SC-NeuroCore — Tests for Cortical Column (Julia Parity)

import numpy as np
import pytest

from sc_neurocore.network.cortical_column import (
    _HAS_JULIA_MULTI_SPMV,
    POPULATIONS,
    CorticalColumn,
)
...
@pytest.mark.skipif(
    not _HAS_JULIA_MULTI_SPMV,
    reason="Julia kernel not loaded",
)
class TestPythonJuliaParity:
    """The bitwise correctness of the Julia batched SpMV against SciPy single-threaded."""
    ...
    def test_spikes_identical(self, scale):
        ...
        for step_idx in range(50):
            ...
            spikes_julia = col_julia.step(0.1)
