...
# SC-NeuroCore — retained clipped-logistic map engine-binding contracts

"""Installed-extension contracts for the retained clipped-logistic map binding."""
...
def _direct(n_steps: int) -> tuple[NDArray[np.float64], int, float, float]:
    result: tuple[Any, int, float, float] = extension.py_sc_clipped_logistic_bursting_map_simulate(
...
def test_exported_name_signature_and_top_level_identity_are_stable() -> None:
    function = extension.py_sc_clipped_logistic_bursting_map_simulate

    assert function.__name__ == "py_sc_clipped_logistic_bursting_map_simulate"
    ...
    assert engine.py_sc_clipped_logistic_bursting_map_simulate is function
    assert "py_sc_clipped_logistic_bursting_map_simulate" in engine.__all__
...
def test_step_count_conversion_errors_are_stable(
    ...
    with pytest.raises(error) as captured:
        extension.py_sc_clipped_logistic_bursting_map_simulate(
            0.1, 0.0, 3.8, 0.01, 0.5, 0.9, n_steps, 0.05
        )
...
def test_production_rust_backend_uses_the_installed_extension() -> None:
    assert sc_clipped_logistic_bursting_map._HAS_RUST is True
    assert (
        sc_clipped_logistic_bursting_map._rust_simulate
        is engine.py_sc_clipped_logistic_bursting_map_simulate
    )

    rust_neuron = SCClippedLogisticBurstingMapNeuron()
    python_neuron = SCClippedLogisticBurstingMapNeuron()
