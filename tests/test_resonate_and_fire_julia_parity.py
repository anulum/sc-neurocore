...
# SC-NeuroCore — Python/Julia resonate-and-fire parity
...
def test_julia_complete_mapping_matches_python(steps: int) -> None:
    ...
    actual = backends.simulate_resonate_and_fire(
        *_PARAMETERS,
        _drive(steps),
        backend="julia",
    )
...
def _direct_call(
    ...
    x_out: npt.NDArray[np.generic],
    y_out: npt.NDArray[np.generic],
    spikes_out: npt.NDArray[np.generic],
    parameters: tuple[float, ...] = _PARAMETERS,
) -> object:
    return _module().simulate_resonate_and_fire_b(
        *parameters,
        drive,
        x_out,
        y_out,
...
def test_julia_rejects_nonfloat_readonly_and_strided_outputs_atomically() -> None:
    ...
    peer_y = np.full(4, -999.0, dtype=np.float64)
    peer_spikes = np.full(4, -999.0, dtype=np.float64)
    _assert_typed_julia_error(
        lambda: _direct_call(drive, integer, peer_y, peer_spikes),
        "is_configuration_error",
    )
    np.testing.assert_array_equal(integer, np.full(4, -999, dtype=np.int64))
    np.testing.assert_array_equal(peer_y, np.full(4, -999.0))
    ...
    _assert_typed_julia_error(
        lambda: _direct_call(drive, read_only, peer_y, peer_spikes),
        "is_configuration_error",
    )
    np.testing.assert_array_equal(read_only, np.full(4, -999.0))

    x_base = np.full(8, -999.0, dtype=np.float64)
    y_base = np.full(8, -999.0, dtype=np.float64)
    ...
    np.testing.assert_array_equal(x_base, np.full(8, -999.0))
    np.testing.assert_array_equal(y_base, np.full(8, -999.0))
...
def test_julia_late_candidate_failure_leaves_all_outputs_unchanged() -> None:
    ...
    x_out = np.full(4, -999.0, dtype=np.float64)
    y_out = np.full(4, -999.0, dtype=np.float64)
    spikes_out = np.full(4, -999.0, dtype=np.float64)
    parameters = (0.25, -0.5, 1_000.0, 1.0, 1.0e300, 1.0)
    _assert_typed_julia_error(
        lambda: _direct_call(drive, x_out, y_out, spikes_out, parameters),
        "is_candidate_error",
    )
    np.testing.assert_array_equal(x_out, np.full(4, -999.0))
    np.testing.assert_array_equal(y_out, np.full(4, -999.0))
