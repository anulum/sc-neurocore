...
# SC-NeuroCore — Terman-Wang benchmark evidence gate

"""Committed source-bound five-runtime Terman-Wang benchmark contracts."""
...
RESULT = ROOT / "benchmarks/results/bench_terman_wang_simulate.json"
...
def test_benchmark_is_source_bound_and_five_runtime() -> None:
    ...
    assert payload["benchmark"] == "terman_wang_simulate_rk4"
    assert payload["model"] == "TermanWangOscillator"
    assert payload["evidence_class"] == "local_regression_non_isolated"
    assert payload["workload"] == {"n_steps": 2_000_000, "current": 0.5, "repeats": 5}
    ...
    assert payload["backends"]["julia"]["parity_max_abs_diff"] <= 1.0e-9
    assert payload["backends"]["go"]["parity_max_abs_diff"] <= 1.0e-9
    assert payload["backends"]["mojo"]["parity_max_abs_diff"] <= 1.0e-8
