...
# SC-NeuroCore — MPR benchmark evidence gates
...
def test_environment_separates_loaded_runtime_and_path_cli_provenance() -> None:
    """Do not label unrelated PATH tools as the measured runtime or builder."""
    ...
    assert go_binary["package"] == ("github.com/anulum/sc-neurocore/accel/ermentrout_kopell_pop")
...
def test_committed_evidence_matches_sources_and_bounded_parity() -> None:
    ...
    assert environment["go_binary"]["package"] == (
        "github.com/anulum/sc-neurocore/accel/ermentrout_kopell_pop"
    )
    assert environment["go_binary"]["cgo_enabled"] == "1"
    ...
    # current tree produces three self-consistent loadable native artefacts.
