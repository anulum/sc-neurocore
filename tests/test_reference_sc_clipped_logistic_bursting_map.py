...
# SC-NeuroCore — Independent retained clipped-logistic map reference trace

"""Independent feature derivation for the committed retained clipped-logistic map trace."""
...
def test_features_match_independent_map_iteration() -> None:
    ...
    spec = load_reference_trace_spec("sc_clipped_logistic_bursting_map_project")
    expected = _independent_features(
        current=spec.protocol.inputs["I"],
        steps=spec.protocol.steps,
    )

    assert spec.schema_name == "sc_clipped_logistic_bursting_map"
    assert spec.provenance.kind == "project_recurrence_reference"
    assert spec.provenance.citation == "project:sc-neurocore-clipped-logistic-bursting-map"
