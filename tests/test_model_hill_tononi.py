...
"""Full pipeline test for HillTononiNeuron. FULL PIPELINE WIRED."""
...
class TestDynamics:
    ...
    def test_rate_increases(self):
        n_low = HillTononiNeuron()
        n_high = HillTononiNeuron()
        s_low = len(_run(n_low, current=0.0, steps=5000))
        s_high = len(_run(n_high, current=5.0, steps=5000))
        assert s_high >= s_low
...
class TestPerformance:
    ...
    def test_network_throughput(self):
        ...
        drive = PoissonInput(n=20, rate_hz=500.0, weight=0.0, dt=0.001, seed=42)
...
class TestPipeline:
    def test_population(self):
        assert Population(HillTononiNeuron, n=10, label="test").n == 10

    def test_network_spikes(self):
        pop = Population(HillTononiNeuron, n=10, label="test")
        drive = PoissonInput(n=10, rate_hz=500.0, weight=0.0, dt=0.001, seed=42)
        mon = SpikeMonitor(pop)
        net = Network(pop, drive, mon)
