...
# SC-NeuroCore — Executable Lapicque polyglot backend parity

"""End-to-end parity and rejection contracts for every Lapicque native lane."""
...
_TRACE_ATOL = 2.0e-15
_GOLDENS = ((0.0, 0), (0.5, 0), (2.0, 71), (5.0, 200), (20.0, 500))
...
def _configured() -> LapicqueNeuron:
...
def test_compiled_backends_match_python_golden(
    ...
    assert state == pytest.approx(reference_state, abs=_TRACE_ATOL)
...
def test_empty_run_preserves_state(backend: str) -> None:
    ...
    neuron = LapicqueNeuron() if backend == "rust" else _configured()
    before = neuron.v
...
def _c_arguments(neuron: LapicqueNeuron) -> tuple[float, ...]:
...
def test_requested_backend_reports_unavailable(
    ...
    with pytest.raises(RuntimeError, match=backend.title()):
        LapicqueNeuron().simulate(1, 0.0, backend=backend)
...
def test_requested_rust_backend_reports_unavailable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backends, "_HAS_RUST", False)
    monkeypatch.setattr(backends, "_EngineLapicqueCls", None)
    with pytest.raises(RuntimeError, match="Rust Lapicque backend"):
        LapicqueNeuron().simulate(1, 0.0, backend="rust")
