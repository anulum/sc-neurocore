...
# SC-NeuroCore — Shared Wilson-HR behavioural test support

"""Behavioural contract for the Wilson 1999 polynomial cortical neuron.

The module-specific tests validate the coupled Wilson-HR ODE, source
capacitance, candidate-first RK4 integration, finite-state error boundaries,
continuous-flow semantics, and the named public workflow contract.
"""
...
def _rhs(neuron: WilsonHRNeuron, v: float, r: float, current: float) -> tuple[float, float]:
    ...
    syn = -26.0 * r * (v + 0.92)
    return (poly + syn + current) / neuron.capacitance, (-r + 1.35 * v + 1.03) / neuron.tau_r
