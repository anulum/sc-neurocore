...
# SC-NeuroCore — committed FitzHugh-Nagumo RTL evidence
...
RTL = ROOT / "hdl/formal/catalogue/sc_fitzhugh_nagumo.v"
MODULE = "sc_fitzhugh_nagumo"


def _compiler_rtl() -> str:
    rtl = UniversalNeuron.from_schema("fitzhugh_nagumo").to_verilog(
        module_name=MODULE,
        data_width=32,
        fraction=16,
    )
...
def test_committed_rtl_matches_the_bounded_source_event_count() -> None:
    steps = 3000
    source = FitzHughNagumoNeuron()
    source_spikes = sum(source.step(0.5) for _ in range(steps))
    universal = UniversalNeuron.from_schema("fitzhugh_nagumo")
