...
# SC-NeuroCore — Go RK4 neuron integrator parity tests
...
from sc_neurocore.accel.go.rk4_neurons import (
    _HAS_GO_RK4_NEURONS,
    simulate_rk4_neuron,
)

if not _HAS_GO_RK4_NEURONS:
    pytest.skip(
        "librk4_neurons.so not built (run go build in accel/go/rk4_neurons)",
...
def test_go_rk4_neuron_simulator_matches_python(
    model: str,
    dt: float,
    currents: np.ndarray,
...
def test_go_rk4_neuron_simulator_rejects_unknown_model() -> None:
...
def test_go_rk4_neuron_simulator_rejects_invalid_trace_or_dt(
...
def test_go_rk4_neuron_simulator_matches_rust_when_available() -> None:
    ...
    go_out = simulate_rk4_neuron("izhikevich", currents, 1.0)

    np.testing.assert_array_equal(
        np.asarray(go_out["spikes"], dtype=np.uint64),
        np.asarray(rust_out["spikes"], dtype=np.uint64),
    )
    np.testing.assert_allclose(
        np.asarray(go_out["v"]), np.asarray(rust_out["v"]), rtol=1e-10, atol=1e-10
    )
    np.testing.assert_allclose(
        np.asarray(go_out["u"]), np.asarray(rust_out["u"]), rtol=1e-10, atol=1e-10
    )
