...
# SC-NeuroCore — Quadratic IF optional native-library loading contracts

"""Fail-closed loading tests for the optional Quadratic IF runtimes."""
...
def test_missing_rust_engine_is_detected_at_import(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with monkeypatch.context() as patch:
        ...
        assert reloaded._EngineQuadraticIFCls is None
...
def test_direct_runners_reject_unloaded_native_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(RuntimeError, match="Rust Quadratic IF"):
        backends.simulate_rust(1, 0.0)
    with pytest.raises(RuntimeError, match="Julia Quadratic IF"):
        backends.simulate_julia(-1.0, -1.0, 1.0, 0.01, 1, 0.0)
    with pytest.raises(RuntimeError, match="Go Quadratic IF"):
        backends.simulate_go(-1.0, -1.0, 1.0, 0.01, 1, 0.0)
    with pytest.raises(RuntimeError, match="Mojo Quadratic IF"):
        backends.simulate_mojo(-1.0, -1.0, 1.0, 0.01, 1, 0.0)
