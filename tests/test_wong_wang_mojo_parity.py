...
# SC-NeuroCore — Python/Mojo Wong-Wang Euler/OU parity

"""Compare the Mojo shared-library batch with the deterministic Python golden."""
...
from sc_neurocore.accel.mojo.wong_wang import _HAS_MOJO_WONG_WANG, simulate_wong_wang
...
def test_mojo_complete_mapping_matches_python(steps: int) -> None:
    ...
    for key in ("s1", "s2", "noise1", "noise2", "r1", "r2"):
        np.testing.assert_allclose(actual[key], expected[key], rtol=0.0, atol=1.0e-9)
    for key in ("s1_final", "s2_final", "noise1_final", "noise2_final"):
        assert float(actual[key]) == pytest.approx(float(expected[key]), abs=1.0e-9)


def test_mojo_rejects_non_finite_samples() -> None:
