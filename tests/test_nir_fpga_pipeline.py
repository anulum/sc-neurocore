...
class TestE2EAutoInterconnect:
    """Verify exact direct wiring is used until weighted AER is audited."""
    ...
    def test_large_network_uses_exact_direct_with_warning(self):
        """Large networks must not use an unverified weighted AER shortcut."""
        ...
        assert result.interconnect == "direct"
        assert "weighted event routing" in " ".join(result.warnings)
        assert "aer_addr" not in result.top_module
        assert "aer_valid" not in result.top_module
