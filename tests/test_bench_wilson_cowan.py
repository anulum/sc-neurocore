...
# SC-NeuroCore — Wilson-Cowan benchmark evidence gates
...
def test_every_public_backend_matches_configured_python_trace() -> None:
    ...
    for backend in benchmark.BACKENDS[1:]:
        ...
        actual, final_state = benchmark._run_backend(backend, 64)
        np.testing.assert_allclose(actual[0], reference[0], rtol=0.0, atol=tolerance)
        np.testing.assert_allclose(actual[1], reference[1], rtol=0.0, atol=tolerance)
        assert final_state[0] == pytest.approx(reference_final[0], abs=tolerance)
        assert final_state[1] == pytest.approx(reference_final[1], abs=tolerance)
