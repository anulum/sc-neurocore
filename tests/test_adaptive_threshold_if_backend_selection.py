...
# SC-NeuroCore — Adaptive-threshold accelerator backend selection tests

"""Validate measured-lane selection, discovery, reload, and native dispatch."""
...
def test_unknown_and_explicitly_unavailable_backends_are_distinct(
    ...
    with pytest.raises(ValueError, match="unknown adaptive-threshold backend"):
        backends.simulate_adaptive_threshold_if(*_PARAMETERS, [1.5], backend="fortran")
    monkeypatch.setattr(backends, "backend_available", lambda _backend: False)
    with pytest.raises(RuntimeError, match="Rust adaptive-threshold backend is unavailable"):
        backends.simulate_adaptive_threshold_if(*_PARAMETERS, [1.5], backend="rust")
