...
def _q1616_rtl_trace(current: float, n_steps: int) -> list[tuple[int, float]]:
    ...
    neuron = UniversalNeuron.from_schema("quadratic_if")
...
def test_schema_formats_preserve_exact_flow_events_and_state_envelope() -> None:
    """TOML and JSON track the exact-flow hand model over varied drive."""
    hand = QuadraticIFNeuron()
    toml_schema = UniversalNeuron.from_schema(_SCHEMA_DIR / "quadratic_if.toml")
    json_schema = UniversalNeuron.from_schema(_SCHEMA_DIR / "quadratic_if.json")
...
def test_schema_threshold_equality_uses_configured_peak(suffix: str) -> None:
    ...
    schema = UniversalNeuron.from_schema(
        _SCHEMA_DIR / f"quadratic_if.{suffix}",
