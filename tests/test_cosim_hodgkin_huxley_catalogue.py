...
# SC-NeuroCore — committed Hodgkin-Huxley RTL evidence
...
from tests.cosim_reference_hodgkin_huxley import _hodgkin_huxley_hand_spike_count

ROOT = Path(__file__).resolve().parents[1]
RTL = ROOT / "hdl/formal/catalogue/sc_hodgkin_huxley.v"
MODULE = "sc_hodgkin_huxley"


def _compiler_rtl() -> str:
    rtl = UniversalNeuron.from_schema("hodgkin_huxley").to_verilog(
        module_name=MODULE,
        data_width=32,
        fraction=16,
    )
...
def test_committed_q1616_rtl_preserves_the_bounded_rk4_event_band() -> None:
    macro_steps = 20
    substeps = 100
    current = 15.0
    source_events = _hodgkin_huxley_hand_spike_count(macro_steps, current)
    universal = UniversalNeuron.from_schema("hodgkin_huxley")
