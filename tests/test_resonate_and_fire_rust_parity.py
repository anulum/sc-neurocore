...
# SC-NeuroCore — Standalone Rust/Python resonate-and-fire parity
...
def test_standalone_rust_configured_trace_matches_python(tmp_path: Path) -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/rust/safety/resonate_and_fire.rs"
    drive = _drive(128)
...
fn main() {{
    ...
    for current in drive {{
        ...
        println!("{{:.17e}} {{:.17e}} {{}}", state.x, state.y, spike);
    }}
}}
''',
        encoding="utf-8",
    ...
    reference = np.column_stack((expected["x"], expected["y"], expected["spikes"]))
