...
# SC-NeuroCore — Alpha-synapse benchmark evidence gates
...
from benchmarks import bench_model_alpha as benchmark
...
def _constant_result(value: float, n_steps: int) -> benchmark.BenchmarkResult:
    return {
        "v": np.full(n_steps, value, dtype=np.float64),
        "a_exc": np.full(n_steps, value, dtype=np.float64),
        "i_exc": np.full(n_steps, value, dtype=np.float64),
        "a_inh": np.full(n_steps, value, dtype=np.float64),
        "i_inh": np.full(n_steps, value, dtype=np.float64),
...
def test_binary_hashes_bind_loaded_native_artefacts() -> None:
    ...
    paths = {
        ...
        "go_shared_library": (benchmark.REPOSITORY / "src/sc_neurocore/accel/go/alpha/libalpha.so"),
        "mojo_shared_library": (
            benchmark.REPOSITORY / "src/sc_neurocore/accel/mojo/alpha/libalpha.so"
        ),
    }
...
def test_environment_separates_loaded_runtime_and_path_cli_provenance() -> None:
    ...
    assert go_binary["package"] == "github.com/anulum/sc-neurocore/accel/alpha"
