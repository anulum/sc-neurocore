...
# SC-NeuroCore — Adaptive-threshold benchmark evidence gates
...
from benchmarks import bench_model_adaptive_threshold_if as benchmark
...
def _constant_result(value: float, n_steps: int) -> benchmark.BenchmarkResult:
    return {
        ...
        "theta": np.full(n_steps, value, dtype=np.float64),
...
def test_environment_separates_loaded_runtime_and_path_cli_provenance() -> None:
    ...
    assert go_binary["package"] == "github.com/anulum/sc-neurocore/accel/adaptive_threshold_if"
...
def test_parity_failure_is_not_published(
    ...
    def measured(
        ...
        return [1], _constant_result(-60.8 if backend == "mojo" else -65.2, n_steps)
