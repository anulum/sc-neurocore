...
"""Full pipeline: DPINeuron. FULL PIPELINE WIRED + PERFORMANCE."""
...
class TestIsolation:
    def test_step_returns_binary(self):
        assert DPINeuron().step(50.0) in (0, 1)
...
class TestDynamics:
    def test_fires(self):
        n = DPINeuron()
        spikes = _run(n, 50.0, 5000)
        assert len(spikes) >= 100

    def test_rate_monotonic(self):
        n_low = DPINeuron()
        n_high = DPINeuron()
        s_low = len(_run(n_low, 20.0, 5000))
        s_high = len(_run(n_high, 100.0, 5000))
        assert s_high >= s_low
    ...
    def test_deterministic(self):
        ...
        for _ in range(2):
            n = DPINeuron()
            trace = [n.step(50.0) for _ in range(200)]
            traces.append(trace)
...
class TestPipeline:
    def test_population(self):
        assert Population(DPINeuron, n=5, label="t").n == 5

    def test_network_spikes(self):
        pop = Population(DPINeuron, n=5, label="t")
    ...
    def test_projection_wiring(self):
        src = Population(DPINeuron, n=5, label="s")
        tgt = Population(DPINeuron, n=5, label="t")
        drive = PoissonInput(n=5, rate_hz=500.0, weight=50.0, dt=0.001, seed=42)
