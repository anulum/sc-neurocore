...
# SC-NeuroCore — Mojo RK4 neuron integrator parity tests
...
from sc_neurocore.accel.mojo.rk4_neurons import (
    _HAS_MOJO_RK4_NEURONS,
    simulate_rk4_neuron,
)

if not _HAS_MOJO_RK4_NEURONS:
    pytest.skip(
        "librk4_neurons.so not built (run mojo build in accel/mojo/rk4_neurons)",
...
def test_mojo_rk4_neuron_simulator_matches_python(
    model: str,
    dt: float,
    currents: np.ndarray,
    ...
    for key in state_keys:
        np.testing.assert_allclose(np.asarray(actual[key]), expected[key], rtol=1e-9, atol=1e-9)


def test_mojo_rk4_neuron_simulator_rejects_unknown_model() -> None:
...
def test_mojo_rk4_neuron_simulator_matches_rust_when_available() -> None:
    ...
    mojo_out = simulate_rk4_neuron("izhikevich", currents, 1.0)

    np.testing.assert_array_equal(
        np.asarray(mojo_out["spikes"], dtype=np.uint64),
        np.asarray(rust_out["spikes"], dtype=np.uint64),
    )
    np.testing.assert_allclose(
        np.asarray(mojo_out["v"]), np.asarray(rust_out["v"]), rtol=1e-9, atol=1e-9
    )
    np.testing.assert_allclose(
        np.asarray(mojo_out["u"]), np.asarray(rust_out["u"]), rtol=1e-9, atol=1e-9
    )
