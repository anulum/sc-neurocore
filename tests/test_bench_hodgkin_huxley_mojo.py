...
# SC-NeuroCore — Hodgkin-Huxley Mojo benchmark contract tests

"""Production-path tests for the Hodgkin-Huxley Mojo benchmark harness."""
...
from benchmarks import bench_hodgkin_huxley_mojo as benchmark
...
_BACKENDS_AVAILABLE = hodgkin_huxley._HAS_RUST and hodgkin_huxley._ensure_mojo_loaded()
_RECEIPT = (
    benchmark.REPOSITORY / "src/sc_neurocore/neurons/reference_receipts/hodgkin_huxley_1952.json"
)


@pytest.mark.skipif(not _BACKENDS_AVAILABLE, reason="Hodgkin-Huxley backends unavailable")
def test_real_benchmark_writes_pinned_parity_evidence(
    ...
    assert payload["kernel"] == "hodgkin_huxley_five_runtime"
...
@pytest.mark.parametrize(
    ...
    [(np.array([0.0]), 1, 3), (np.array([3.0e-9]), 0, 4)],
)
def test_parity_contract_failures_have_distinct_exit_codes(
    ...
    def measured(
        ...
        if backend == "python":
            return 1.0, 1.0, np.array([0.0]), 0, (0.0,) * 4
        return 0.5, 0.5, mojo_trace, mojo_spikes, (0.0,) * 4
...
def test_backend_order_requires_python_reference_first(
    ...
    monkeypatch.setattr(
        benchmark,
        "_measure_backend",
        lambda _backend: (1.0, 1.0, np.array([0.0]), 0, (0.0,) * 4),
    )
