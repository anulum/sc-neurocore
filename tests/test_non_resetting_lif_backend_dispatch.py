...
"""Five-runtime dispatch tests for source MAT(1)."""
...
def _drive() -> np.ndarray:
    return np.asarray(
        [0.0] * 32 + [0.7] * 512 + [value for _ in range(64) for value in (0.2, 0.9)],
        dtype=np.float64,
    )
...
def test_available_backend_matches_complete_python_trace(backend: str) -> None:
    ...
    expected = simulate_non_resetting_lif(_drive(), backend="python")
    actual = simulate_non_resetting_lif(_drive(), backend=backend)
...
def test_invalid_drive_fails_before_dispatch() -> None:
    with pytest.raises(ValueError, match="finite"):
        simulate_non_resetting_lif([0.0, np.nan], backend="python")
