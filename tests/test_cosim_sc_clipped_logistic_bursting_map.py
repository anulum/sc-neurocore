...
# SC-NeuroCore — retained clipped-logistic map Python-to-Verilog co-simulation

"""Bounded Q16.16 trajectory evidence for the retained clipped-logistic fast/slow map."""
...
def _rtl_q1616_trace(n_steps: int, current: float) -> list[TraceRow]:
    ...
    assert HAS_IVERILOG, "iverilog is required for retained clipped-logistic co-simulation"
    neuron = UniversalNeuron.from_schema("sc_clipped_logistic_bursting_map")
    module_name = "sc_clipped_logistic_bursting_map_q1616_trace"
    ...
    testbench = "\n".join(
        [
            "`timescale 1ns / 1ps",
            "module tb_sc_clipped_logistic_bursting_map_q1616_trace;",
            "reg clk = 1'b0;",
            "reg rst_n = 1'b0;",
            "wire spike_out;",
            ...
            "        @(posedge clk); #1;",
            '        $display("SC_CLIPPED_LOGISTIC_TRACE %0d %0d %0d", spike_out, uut.x_reg, uut.y_reg);',
            "    end",
            "    $finish;",
            "end",
            "endmodule",
    ...
    rows = re.findall(
        r"^SC_CLIPPED_LOGISTIC_TRACE (-?\d+) (-?\d+) (-?\d+)$",
        simulation.stdout,
        re.MULTILINE,
    )
    scale = float(1 << 16)
    ...
    assert len(trace) == n_steps, (
        f"retained clipped-logistic RTL emitted {len(trace)} rows; expected {n_steps}:\n{simulation.stdout}"
    )
    return trace
...
def _hand_schema_trace(current: float, n_steps: int) -> tuple[list[TraceRow], dict[str, int]]:
    ...
    hand = SCClippedLogisticBurstingMapNeuron()
    toml_schema = UniversalNeuron.from_schema(SCHEMA_DIR / "sc_clipped_logistic_bursting_map.toml")
    json_schema = UniversalNeuron.from_schema(SCHEMA_DIR / "sc_clipped_logistic_bursting_map.json")
    trace: list[TraceRow] = []
