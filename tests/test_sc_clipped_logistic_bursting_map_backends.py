...
# SC-NeuroCore — Polyglot parity tests for the retained SC clipped-logistic bursting recurrence

"""Cross-backend parity for ``SCClippedLogisticBurstingMapNeuron.simulate``.
...
def _run(backend: str, *, a: float = 3.8, n: int = 4000, current: float = 0.05) -> tuple:
    neuron = SCClippedLogisticBurstingMapNeuron(a=a)
    trace, spikes = neuron.simulate(n, current, backend=backend)
    return trace, spikes, neuron.x, neuron.y


def _rust() -> bool:
    return sc_clipped_logistic_bursting_map._HAS_RUST


def _julia() -> bool:
    return sc_clipped_logistic_bursting_map._ensure_julia_loaded()


def _go() -> bool:
    return sc_clipped_logistic_bursting_map._ensure_go_loaded()


def _mojo() -> bool:
    return sc_clipped_logistic_bursting_map._ensure_mojo_loaded()
...
def test_bit_exact_trace(backend: str, available, a: float) -> None:
    if not available():
        pytest.skip(f"{backend} retained SC clipped-logistic backend unavailable")
...
@pytest.mark.skipif(not _mojo(), reason="Mojo retained SC clipped-logistic backend unavailable")
...
def test_mojo_per_step_within_two_ulp() -> None:
    ...
    for _ in range(5000):
        ...
        ref, _rs, rx, ry = SCClippedLogisticBurstingMapNeuron(x=x, y=y)._simulate_python(1, cur)
        got, _gs, gx, gy = SCClippedLogisticBurstingMapNeuron(x=x, y=y)._simulate_mojo(1, cur)
...
def test_invalid_backend_raises() -> None:
    with pytest.raises(ValueError, match="backend must be"):
        SCClippedLogisticBurstingMapNeuron().simulate(10, 0.0, backend="cuda")


def test_negative_n_steps_raises() -> None:
    with pytest.raises(ValueError, match="n_steps must be non-negative"):
        SCClippedLogisticBurstingMapNeuron().simulate(-1, 0.0)


def test_simulate_matches_repeated_step() -> None:
    # The N-step path must equal calling step() N times (same state evolution).
    trace_a, spikes_a = SCClippedLogisticBurstingMapNeuron().simulate(50, 0.05, backend="python")
    manual = []
    spikes_b = 0
    stepper = SCClippedLogisticBurstingMapNeuron()
    for _ in range(50):
...
def test_bursting_produces_spikes() -> None:
    ...
    _trace, spikes = SCClippedLogisticBurstingMapNeuron(a=3.8).simulate(
        2000, 0.05, backend="python"
    )
    assert spikes > 0


def test_long_run_is_finite() -> None:
    trace, _spikes = SCClippedLogisticBurstingMapNeuron(a=3.8).simulate(
        100_000, 0.05, backend="python"
    )
