...
# SC-NeuroCore — GLIF engine-binding contracts

"""Installed-extension contracts for the Allen GLIF5 batch binding."""
...
def _direct(
    ...
    trace, spikes, final_v, final_theta, final_i_asc1, final_i_asc2 = extension.py_glif_simulate(
...
def test_step_count_conversion_errors_are_stable(
    ...
    with pytest.raises(error) as captured:
        extension.py_glif_simulate(*_parameters(), n_steps, 30.0)
...
def test_production_rust_dispatcher_is_installed_and_bit_exact() -> None:
    ...
    rust_neuron = GLIFNeuron()
    python_neuron = GLIFNeuron()
