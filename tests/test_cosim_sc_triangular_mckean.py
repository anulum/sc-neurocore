...
RTL = ROOT / "hdl/formal/catalogue/sc_triangular_mckean.v"
SCALE = 1 << 32
...
def _rtl_trace(tmp: Path, currents: list[float]) -> np.ndarray:
    drives = "\n".join(
        f'current_t={_literal(round(x * SCALE))};@(posedge clk);#1;$display("SCM %0d %0d %0d",v,w,event_out);'
        for x in currents
    )
    tb = tmp / "tb.v"
    binary = tmp / "tb"
    ...
    tb.write_text(
        f"`timescale 1ns/1ps\nmodule tb;reg clk=0;reg rst_n=0;reg signed[63:0]current_t=0;wire signed[63:0]v,w;wire event_out;always #5 clk=~clk;sc_triangular_mckean uut(.clk(clk),.rst_n(rst_n),.current_t(current_t),.v_out(v),.w_out(w),.event_out(event_out));initial begin #23;rst_n=1;{drives}$finish;end endmodule\n"
    )
    subprocess.run(
    ...
    return np.asarray(
        [
            ...
            for v, w, event in re.findall(r"^SCM (-?\d+) (-?\d+) ([01])$", output, re.M)
        ]
    )


def test_sc_triangular_q3232_tracks_python(tmp_path: Path) -> None:
    currents = [0.3, 0.5, 0.8, 0.1] * 32
    neuron = SCTriangularMcKeanNeuron()
    expected = []
...
def test_yosys_synthesises_committed_rtl() -> None:
    completed = subprocess.run(
        [
            ...
            "-q",
            "-p",
            f"read_verilog {RTL}; synth -top sc_triangular_mckean -run begin:coarse; check; stat",
        ],
        cwd=ROOT,
        check=False,
        capture_output=True,
