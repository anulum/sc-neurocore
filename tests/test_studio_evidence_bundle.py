...
def test_write_studio_audit_quarantine_archive_writes_manifest_and_payload(
    ...
    context = _archive_context(tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda payload: payload.__setitem__("event_count", 2),
            "event count",
        ),
        (
            lambda payload: payload.__setitem__("retained_event_count", 0),
            "retained count",
        ),
        (
            lambda payload: payload.__setitem__("truncated", "false"),
            "truncation status",
        ),
        (
            lambda payload: payload.__setitem__("quarantine_reason", 7),
            "summary reason",
        ),
        (
            lambda payload: payload.__setitem__("events", ["invalid"]),
            "event rows",
        ),
        (
...
def test_studio_audit_quarantine_archive_route_requires_admin(
    tmp_path: Path,
) -> None:
    """Quarantine archive creation is denied without the admin role."""
