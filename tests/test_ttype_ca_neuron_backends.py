...
# SC-NeuroCore — Executed T-type backend and custody parity

"""Execute every declared T-type backend and bind public claims to live sources."""
...
def _python_trace() -> np.ndarray:
    ...
    for current in _DRIVE:
        ...
        rows.append((neuron.v, neuron.h, neuron.n, neuron.s, event))
...
def test_production_rust_binding_matches_complete_python_state() -> None:
    ...
    rust = cast(_RustTType, engine.TTypeCaNeuron())
    actual = []
    for current in _DRIVE:
        event = rust.step(current)
        state = rust.get_state()
        actual.append((state["v"], state["h"], state["n"], state["s"], event))
...
def test_standalone_safety_rust_matches_complete_python_state(tmp_path: Path) -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/rust/safety/ttype_ca_neuron.rs"
    literals = ", ".join(f"{current:.17e}_f64" for current in _DRIVE)
    program = tmp_path / "ttype_trace.rs"
    binary = tmp_path / "ttype_trace"
    program.write_text(
        f'''#[path = r#"{source}"#]
mod ttype;

use ttype::TTypeCaNeuron;

fn main() {{
    let mut state = TTypeCaNeuron::new();
    for current in [{literals}] {{
        let event = state.step(current).expect("finite configured drive");
        println!("{{:.17e}} {{:.17e}} {{:.17e}} {{:.17e}} {{}}", state.v, state.h, state.n, state.s, event);
    }}
}}
''',
        encoding="utf-8",
...
def test_go_backend_matches_complete_python_state(tmp_path: Path) -> None:
    ...
    program = tmp_path / "ttype_trace.go"
    program.write_text(
        f"""package main
...
def test_julia_backend_matches_complete_python_state() -> None:
    ...
    source = _ROOT / "src/sc_neurocore/accel/julia/neurons/ttype_ca_neuron.jl"
    literals = ", ".join(f"{current:.17e}" for current in _DRIVE)
    program = f'''
include(raw"{source}")
using .TtypeCaNeuronAccel
state = TTypeCaNeuronState()
for current in [{literals}]
    event = step!(state, current)
    println(state.v, " ", state.h, " ", state.n, " ", state.s, " ", event)
end
before = (state.v, state.h, state.n, state.s)
...
(state.v, state.h, state.n, state.s) == before || error("invalid drive mutated state")
'''
    completed = subprocess.run(
...
def test_descriptor_and_public_page_report_only_proven_backends() -> None:
    descriptor = tomllib.loads(
        (_ROOT / "src/sc_neurocore/neurons/model_descriptors/TTypeCaNeuron.toml").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert descriptor["provenance"]["doi"] == "10.1146/annurev.ph.58.030196.001553"

    page = (_ROOT / "docs/api/models/ttype_ca_neuron.md").read_text(encoding="utf-8")
    assert "Bhatt" not in page
    assert "Sejnowski" in page
    assert "engine/src/neurons/channels/t_type_ca.rs" in page
