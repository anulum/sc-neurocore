...
"""Full pipeline: ConnorStevensNeuron. FULL PIPELINE WIRED + PERFORMANCE."""
...
class TestIsolation:
    def test_step_returns_binary(self):
        assert ConnorStevensNeuron().step(20.0) in (0, 1)
...
class TestDynamics:
    ...
    def test_rate_monotonic(self):
        n_low = ConnorStevensNeuron()
        n_high = ConnorStevensNeuron()
        s_low = len(_run(n_low, 10.0, 5000))
        s_high = len(_run(n_high, 30.0, 5000))
        assert s_high >= s_low
...
class TestPerformance:
    def test_isolation_throughput(self):
        ...
        assert N / elapsed > 100

    def test_network_throughput(self):
        pop = Population(ConnorStevensNeuron, n=20, label="bench")
...
class TestPipeline:
    def test_population(self):
        assert Population(ConnorStevensNeuron, n=5, label="t").n == 5

    def test_network_spikes(self):
        pop = Population(ConnorStevensNeuron, n=5, label="t")
    ...
    def test_projection_wiring(self):
        src = Population(ConnorStevensNeuron, n=5, label="s")
        tgt = Population(ConnorStevensNeuron, n=5, label="t")
        drive = PoissonInput(n=5, rate_hz=500.0, weight=20.0, dt=0.001, seed=42)
