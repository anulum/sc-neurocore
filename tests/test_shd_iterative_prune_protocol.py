...
def _load_training_helpers():
    missing = object()
    original_modules = {
        mod_name: sys.modules.get(mod_name, missing) for mod_name in _TRAINING_STUB_MODULES
    }
    try:
        for mod_name in _TRAINING_STUB_MODULES:
            sys.modules[mod_name] = types.ModuleType(mod_name)

        sys.modules["torch"].nn = types.SimpleNamespace(Module=object)
        sys.modules["torch"].Tensor = object
        sys.modules["wandb"].init = lambda **_: None
        sys.modules["spikingjelly.activation_based"].neuron = types.SimpleNamespace(LIFNode=object)
        ...
        for mod_name, original_module in original_modules.items():
