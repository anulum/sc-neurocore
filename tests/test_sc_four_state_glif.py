...
# SC-NeuroCore — SCFourStateGLIFNeuron behavioural tests

"""Module-specific behavioural contract for SCFourStateGLIFNeuron.

The contract protects the historical SC four-state continuous flow, candidate-first RK4
...
def _state(neuron: SCFourStateGLIFNeuron) -> tuple[float, float, float, float]:
...
def _rk4_reference(
    neuron: SCFourStateGLIFNeuron, current: float
) -> tuple[float, float, float, float]:
    state = _state(neuron)
...
def test_subthreshold_step_matches_independent_rk4_reference() -> None:
    neuron = SCFourStateGLIFNeuron(v=-68.0, theta=-45.0, i_asc1=0.4, i_asc2=-0.2)
...
def test_after_spike_currents_decay_inside_continuous_flow() -> None:
    neuron = SCFourStateGLIFNeuron(i_asc1=5.0, i_asc2=5.0)
...
def test_spike_reset_is_candidate_first_and_additive() -> None:
    neuron = SCFourStateGLIFNeuron(v=-51.0, theta=-50.5, delta_theta=2.5, r_asc1=1.25, r_asc2=-0.25)
...
def test_reset_restores_rest_threshold_and_zero_currents() -> None:
    neuron = SCFourStateGLIFNeuron(v=-51.0, theta=-50.5, i_asc1=0.5, i_asc2=-0.2)
    assert neuron.step(40.0) == 1
...
def test_nonfinite_candidate_raises_before_mutation() -> None:
    neuron = SCFourStateGLIFNeuron(v=1e308, theta=-45.0, i_asc1=1e308, i_asc2=1e308)
    before = _state(neuron)
...
def test_voltage_coupling_raises_threshold_relative_to_uncoupled_case() -> None:
    uncoupled = SCFourStateGLIFNeuron(v=-65.0, a_theta=0.0)
    coupled = SCFourStateGLIFNeuron(v=-65.0, a_theta=0.2)
...
def test_population_wires_public_glif_surface() -> None:
    population = Population(SCFourStateGLIFNeuron, n=3, label="glif")
    spikes = [
        neuron.step(current)
