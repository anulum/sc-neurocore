...
"""Full pipeline: InhibitoryLIFNeuron. FULL PIPELINE WIRED + PERFORMANCE."""
...
class TestIsolation:
    def test_step_returns_binary(self):
        assert InhibitoryLIFNeuron().step(5.0) in (0, 1)
...
class TestPipeline:
    def test_population(self):
        assert Population(InhibitoryLIFNeuron, n=5, label="t").n == 5

    def test_network_spikes(self):
        pop = Population(InhibitoryLIFNeuron, n=5, label="t")
    ...
    def test_projection_wiring(self):
        src = Population(InhibitoryLIFNeuron, n=5, label="s")
        tgt = Population(InhibitoryLIFNeuron, n=5, label="t")
        drive = PoissonInput(n=5, rate_hz=500.0, weight=5.0, dt=0.001, seed=42)
