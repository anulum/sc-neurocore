...
# SC-NeuroCore — Pernarowski Q16.16 co-simulation and synthesis tests
...
from tests.cosim_pernarowski_support import (
    HAS_IVERILOG,
...
_RTL = _ROOT / "hdl/formal/catalogue/sc_pernarowski.v"
...
def test_yosys_synthesises_committed_rtl() -> None:
    completed = subprocess.run(
        [
            ...
            "-q",
            "-p",
            f"read_verilog {_RTL}; synth -top sc_pernarowski -run begin:coarse; check; stat",
        ],
        cwd=_ROOT,
        check=False,
        capture_output=True,
