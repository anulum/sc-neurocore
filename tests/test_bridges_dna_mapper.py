...
class TestDNAStrand:
    ...
    def test_delta_g_37(self):
        s = DNAStrand(name="s6", sequence="GCGCGCGCGCGC")
        dg = s.delta_g_37
        if callable(dg):
            dg = dg()
        assert isinstance(float(dg), float)
...
class TestSequenceDesigner:
    ...
    def test_generate_recognition(self):
        designer = SequenceDesigner(seed=42)
...
class TestStrandDisplacementCompiler:
    def test_compile_and_gate(self):
    ...
    def test_compile_mux(self):
        compiler = StrandDisplacementCompiler()
        gate = compiler.compile_mux("sel", "a", "b", "out")
        assert isinstance(gate, DNAGate)

    def test_compile_amplifier(self):
        compiler = StrandDisplacementCompiler()
        gate = compiler.compile_amplifier("in", "out")
        assert isinstance(gate, DNAGate)

    def test_compile_buffer(self):
        compiler = StrandDisplacementCompiler()
...
class TestKineticSimulator:
    def test_simulate_produces_trajectory(self):
        compiler = StrandDisplacementCompiler()
...
class TestGF4ErrorCorrection:
    ...
    def test_detects_errors(self):
        ...
        orig = corrupted[2]
        corrupted[2] = "G" if orig != "G" else "A"
...
class TestCrossHybridizationChecker:
    def test_check_returns_list(self):
        ...
        try:
            ...
            pytest.skip("CrossHybridizationChecker needs strands populated")
...
class TestDegradationModel:
    def test_predict_concentration_decays(self):
...
class TestDNAMapperBenchmark:
    def test_sequence_generation_throughput(self):
