...
def test_energy_lif_backend_matches_python(backend: str) -> None:
    if not backend_available(backend):
        pytest.skip(f"{backend} unavailable")
    drive = np.resize(np.array([80.0, 0.0, 120.0, 20.0]), 128)
    expected = simulate_energy_lif(drive, backend="python")
    actual = simulate_energy_lif(drive, backend=backend)
