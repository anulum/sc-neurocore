...
class TestStochasticHeatSolver:
    def test_construction(self):
        s = StochasticHeatSolver(length=100, num_walkers=500, alpha=0.1)
        assert len(s.walkers) == 500
        assert s.length == 100

    def test_step_moves_walkers(self):
        np.random.seed(42)
        s = StochasticHeatSolver(length=100, num_walkers=100, alpha=0.1)
    ...
    def test_walkers_stay_in_bounds(self):
        np.random.seed(0)
        s = StochasticHeatSolver(length=50, num_walkers=1000, alpha=0.1)
        for _ in range(100):
            s.step()
    ...
    def test_temperature_profile_shape(self):
        s = StochasticHeatSolver(length=20, num_walkers=500, alpha=0.1)
        profile = s.get_temperature_profile()
        assert profile.shape == (20,)

    def test_temperature_sums_to_one(self):
        s = StochasticHeatSolver(length=20, num_walkers=1000, alpha=0.1)
        profile = s.get_temperature_profile()
        assert abs(profile.sum() - 1.0) < 0.01

    def test_diffusion_spreads(self):
        np.random.seed(42)
        s = StochasticHeatSolver(length=100, num_walkers=10000, alpha=0.1)
        # Start all walkers at center
        s.walkers[:] = 50
        p0 = s.get_temperature_profile()
        for _ in range(200):
            s.step()
        p1 = s.get_temperature_profile()
        # Entropy should increase (more spread)
