...
def test_go_python_learning_parity():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            res = subprocess.run(
                ["go", "run", go_file], capture_output=True, text=True, env=env, cwd="src"
            )
            ...
            go_weight = float(out)
            assert math.isclose(py_weight, go_weight, rel_tol=1e-5), (
                f"Parity mismatch: Python={py_weight}, Go={go_weight}"
            )

        except FileNotFoundError:
            pytest.skip("Go not installed")
        except ValueError:
            pytest.skip(f"Unparsable Go output: {out}")
