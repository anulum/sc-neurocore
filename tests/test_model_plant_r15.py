...
def _run(neuron: PlantR15Neuron, current: float,
...
class TestPlantR15Isolation:
    ...
    def test_five_state_variables_evolve(self):
        ...
        for i, (name, v0, v1) in enumerate(
            zip(["v", "m", "h", "n", "ca"], initial, final)
        ):
...
class TestPlantR15Equilibrium:
    ...
    def test_transient_spike(self):
        ...
        assert len(spike_times) == 1, (
            f"Expected 1 transient spike, got {len(spike_times)}"
        )
...
class TestPlantR15Calcium:
    ...
    def test_calcium_accumulates_from_initial(self):
        ...
        for _ in range(5000):
            n.step(0.0)
        assert n.ca > ca_initial, (
            f"Ca={n.ca:.4f} <= initial {ca_initial}"
        )
    ...
    def test_calcium_at_equilibrium(self):
        ...
        for _ in range(10000):
            n.step(0.0)
        ca_2 = n.ca
        assert abs(ca_2 - ca_1) < 0.01, (
            f"Ca still drifting: {ca_1:.4f} → {ca_2:.4f}"
        )
...
class TestPlantR15Stability:
    ...
    def test_moderate_current_finite(self):
        ...
        for _ in range(50000):
            n.step(10.0)
        for name, val in [("v", n.v), ("m", n.m), ("h", n.h),
                          ("n", n.n), ("ca", n.ca)]:
...
class TestPlantR15Parameters:
    ...
    def test_g_kca_controls_burst_termination(self):
        ...
        assert len(s_low) >= len(s_high), (
            f"Low g_KCa: {len(s_low)} spikes, high: {len(s_high)}"
        )
    ...
    def test_tau_ca_affects_calcium_dynamics(self):
        ...
        assert n_fast.ca < n_slow.ca, (
            f"Fast Ca={n_fast.ca:.4f}, slow Ca={n_slow.ca:.4f}"
        )
