...
# SC-NeuroCore — DPI five-backend benchmark contract tests

"""Production-path tests for the controlled DPI benchmark."""
...
from benchmarks import bench_model_dpi_neuron as benchmark
...
def test_source_hashes_cover_declared_implementation_surfaces() -> None:
    ...
    schema_hashes = hashes["src/sc_neurocore/neurons/model_schemas/dpi_neuron"]
    assert isinstance(schema_hashes, dict)
    assert set(schema_hashes) == {"json", "toml"}
...
def test_committed_evidence_matches_live_sources_and_bounded_parity() -> None:
    ...
    assert payload["workload"] == {
        ...
        "state_atol": benchmark.STATE_ATOL,
    }
    assert set(payload["measured_order"]) == set(benchmark.BACKENDS)
    assert payload["dispatcher_order"] == list(benchmark.DISPATCH_ORDER)
    assert payload["verification"]["rust_safety"]["passed"] is True
...
def test_real_rust_safety_gate_executes_enrolled_module() -> None:
    """Execute the benchmark's actual Rust-safety verification command."""
...
def test_absolute_state_metric_handles_empty_and_signed_differences() -> None:
    """Measure the maintained scalar state rather than events alone."""
    assert benchmark._max_abs_diff(np.array([]), np.array([])) == 0.0
    assert benchmark._max_abs_diff(np.array([-1.0, 2.0]), np.array([1.0, 1.5])) == 2.0
...
def test_unavailable_backend_override_records_explicit_evidence(
    ...
    """Keep acknowledged missing runtimes visible rather than dropping them."""
...
def test_compiled_lane_cannot_precede_python_reference(
    ...
    """Fail closed if ordering no longer establishes the Python reference."""
