...
"""Full pipeline: LearnableNeuronModel. FULL PIPELINE + PERFORMANCE."""
...
class TestDynamics:
    def test_fires(self):
        n = LearnableNeuronModel()
        spikes = _run(n, 10.0, 5000)
        assert len(spikes) >= 100

    def test_rate_monotonic(self):
        n_low = LearnableNeuronModel()
        n_high = LearnableNeuronModel()
        s_low = len(_run(n_low, 5.0, 5000))
        s_high = len(_run(n_high, 20.0, 5000))
        assert s_high >= s_low
...
class TestPipeline:
    def test_population(self):
        assert Population(LearnableNeuronModel, n=5, label="t").n == 5

    def test_network(self):
        pop = Population(LearnableNeuronModel, n=5, label="t")
    ...
    def test_projection_wiring(self):
        src = Population(LearnableNeuronModel, n=5, label="s")
        tgt = Population(LearnableNeuronModel, n=5, label="t")
        drive = PoissonInput(n=5, rate_hz=500.0, weight=10.0, dt=0.001, seed=42)
